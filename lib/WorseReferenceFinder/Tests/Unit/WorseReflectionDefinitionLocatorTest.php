...
class WorseReflectionDefinitionLocatorTest extends DefinitionLocatorTestCase
{
    ...
    public function testExceptionOnNonPhpFile() : void
    {
        ...
        $this->locator()->locateDefinition(TextDocumentBuilder::create('asd')->language('asd')->build(), ByteOffset::fromInt(1234));
    }
    public function testExceptionOnUnresolvableSymbol() : void
    {
        ...
        $this->locator()->locateDefinition(TextDocumentBuilder::create($source)->language('php')->build(), ByteOffset::fromInt($offset));
    }
...
, '<?php class Foobar { public function b<>ar() {} }');
        $this->assertEquals($this->workspace->path('somefile.php'), (string) $location->first()->location()->uri()->path());
    }
...
, '<?php $f = Barfoo::barfoo(); $f->g<>et();');
        self::assertEquals($this->workspace->path('Foobar.php'), $location->first()->location()->uri()->path());
        self::assertEquals(39, $location->first()->location()->offset()->toInt());
    }
...
, '<?php $bar = new Barfoo(); $bar->bar<>foo();');
        self::assertEquals($this->workspace->path('Foobar.php'), $location->first()->location()->uri()->path());
        self::assertEquals(21, $location->first()->location()->offset()->toInt());
    }
...
, '<?php $bar = new Barfoo(); $bar->foobar()->baz<>();');
        self::assertEquals($this->workspace->path('Foobar.php'), $location->first()->location()->uri()->path());
        self::assertEquals(20, $location->first()->location()->offset()->toInt());
    }
...
\class_alias('Phpactor202301\\Phpactor\\WorseReferenceFinder\\Tests\\Unit\\WorseReflectionDefinitionLocatorTest', 'Phpactor\\WorseReferenceFinder\\Tests\\Unit\\WorseReflectionDefinitionLocatorTest', \false);
