...
class WorseReflectionDefinitionLocator implements DefinitionLocator
{
    ...
    public function locateDefinition(TextDocument $document, ByteOffset $byteOffset) : TypeLocations
    {
        ...
        try {
            $offset = $this->reflector->reflectOffset($sourceCode, $byteOffset->toInt());
    ...
    private function gotoDefinition(TextDocument $document, ReflectionOffset $offset) : TypeLocations
    {
        ...
        return match ($symbolContext->symbol()->symbolType()) {
            ...
            default => throw new CouldNotLocateDefinition(\sprintf('Do not know how to goto definition of symbol type "%s"', $symbolContext->symbol()->symbolType())),
        };
    }
    private function gotoClass(NodeContext $symbolContext) : TypeLocations
    {
        ...
        return new TypeLocations([new TypeLocation($className, new Location(TextDocumentUri::fromString($path), ByteOffset::fromInt($class->position()->start())))]);
    }
    private function gotoFunction(NodeContext $symbolContext) : TypeLocations
    {
        ...
        return new TypeLocations([new TypeLocation(TypeFactory::unknown(), new Location(TextDocumentUri::fromString($path), ByteOffset::fromInt($function->position()->start())))]);
    }
    private function gotoDeclaredConstant(NodeContext $symbolContext) : TypeLocations
    {
        ...
        return new TypeLocations([new TypeLocation(TypeFactory::unknown(), new Location(TextDocumentUri::fromString($path), ByteOffset::fromInt($constant->position()->start())))]);
    }
    private function gotoMember(NodeContext $symbolContext) : TypeLocations
    {
        ...
        foreach ($containerType->expandTypes()->classLike() as $namedType) {
            ...
            $locations[] = new TypeLocation($namedType, new Location(TextDocumentUri::fromString($path), ByteOffset::fromInt($member->position()->start())));
        }
