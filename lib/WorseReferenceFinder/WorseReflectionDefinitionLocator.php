...
class WorseReflectionDefinitionLocator implements DefinitionLocator
{
    ...
    public function locateDefinition(TextDocument $textDocument, ByteOffset $byteOffset) : TypeLocations
    {
        ...
        $this->cache->purge();
        try {
            $offset = $this->reflector->reflectOffset($textDocument, $byteOffset->toInt());
    ...
    private function gotoDefinition(TextDocument $document, ReflectionOffset $offset) : TypeLocations
    {
        ...
        return match ($nodeContext->symbol()->symbolType()) {
            ...
            default => throw new CouldNotLocateDefinition(\sprintf('Do not know how to goto definition of symbol type "%s"', $nodeContext->symbol()->symbolType())),
        };
    }
    private function gotoClass(NodeContext $nodeContext) : TypeLocations
    {
        ...
        if (!$className instanceof ClassType) {
            throw new CouldNotLocateDefinition(\sprintf('member container type is not a class type, it is a "%s"', \get_class($className)));
        }
        try {
            $class = $this->reflector->reflectClassLike($className->name());
        } catch (NotFound $e) {
            throw new CouldNotLocateDefinition($e->getMessage(), 0, $e);
        }
        $path = $class->sourceCode()->uri()?->path();
        if (null === $path) {
            throw new CouldNotLocateDefinition(\sprintf('The source code for class "%s" has no path associated with it.', $class->name()));
        }
        return new TypeLocations([new TypeLocation($className, new Location(TextDocumentUri::fromString($path), ByteOffset::fromInt($class->position()->start()->toInt())))]);
    }
    private function gotoFunction(NodeContext $nodeContext) : TypeLocations
    {
        ...
        if (null === $path) {
            throw new CouldNotLocateDefinition(\sprintf('The source code for function "%s" has no path associated with it.', $function->name()));
        }
        return new TypeLocations([new TypeLocation(TypeFactory::unknown(), new Location(TextDocumentUri::fromString($path), ByteOffset::fromInt($function->position()->start()->toInt())))]);
    }
    private function gotoDeclaredConstant(NodeContext $nodeContext) : TypeLocations
    {
        ...
        if (null === $path) {
            throw new CouldNotLocateDefinition(\sprintf('The source code for constant "%s" has no path associated with it.', $constant->name()));
        }
        return new TypeLocations([new TypeLocation(TypeFactory::unknown(), new Location(TextDocumentUri::fromString($path), ByteOffset::fromInt($constant->position()->start()->toInt())))]);
    }
        ...
        foreach ($containerType->expandTypes()->classLike() as $namedType) {
            ...
            switch ($symbolType) {
                ...
                case Symbol::PROPERTY:
                    ...
                    break;
                default:
                    throw new CouldNotLocateDefinition(\sprintf('Unhandled symbol type "%s"', $symbolType));
            }
            ...
            if (null === $path) {
                throw new CouldNotLocateDefinition(\sprintf('The source code for class "%s" has no path associated with it.', (string) $containingClass->name()));
            }
            $locations[] = new TypeLocation($namedType, new Location(TextDocumentUri::fromString($path), ByteOffset::fromInt($member->position()->start()->toInt())));
        }
    ...
    private function gotoMethodDeclaration(MemberDeclarationContext $nodeContext) : TypeLocations
    {
        ...
        $path = $member->declaringClass()->sourceCode()->uri()?->path();
        if (null === $path) {
            throw new CouldNotLocateDefinition(\sprintf('The source code for class "%s" has no path associated with it.', (string) $member->declaringClass()->name()));
        }
        return new TypeLocations([new TypeLocation($nodeContext->classType(), new Location(TextDocumentUri::fromString($path), $member->position()->start()))]);
    }
}
