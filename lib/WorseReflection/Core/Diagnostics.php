...
final class Diagnostics implements IteratorAggregate, Countable
{
    ...
    public function byClasses(string ...$classFqns) : self
    {
        ...
        return new self(\array_filter($this->diagnostics, function (Diagnostic $d) use($classFqns) {
    ...
    {
        return new self(\array_filter($this->diagnostics, fn(Diagnostic $d) => $d->range()->start()->toInt() >= $byteOffsetRange->start()->toInt() && $d->range()->end()->toInt() <= $byteOffsetRange->end()->toInt()));
    }
    /**
     * @return Diagnostics<T>
     */
 ...
 * @implements IteratorAggregate<T>
 */
\class_alias('Phpactor202301\\Phpactor\\WorseReflection\\Core\\Diagnostics', 'Phpactor\\WorseReflection\\Core\\Diagnostics', \false);
