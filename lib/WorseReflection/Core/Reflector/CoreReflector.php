...
class CoreReflector implements \Phpactor\WorseReflection\Core\Reflector\ClassReflector, \Phpactor\WorseReflection\Core\Reflector\SourceCodeReflector, \Phpactor\WorseReflection\Core\Reflector\FunctionReflector, \Phpactor\WorseReflection\Core\Reflector\ConstantReflector
{
    ...
    public function reflectClass($className) : ReflectionClass
    {
        ...
        if (\false === $class instanceof ReflectionClass) {
            throw new ClassNotFound(\sprintf('"%s" is not a class, it is a "%s"', $className->full(), \get_class($class)));
        }
        return $class;
    }
    /**
    ...
    public function reflectInterface($className, array $visited = []) : ReflectionInterface
    {
        ...
        if (\false === $class instanceof ReflectionInterface) {
            throw new ClassNotFound(\sprintf('"%s" is not an interface, it is a "%s"', $className->full(), \get_class($class)));
        }
        return $class;
    }
    /**
    ...
    public function reflectTrait($className, array $visited = []) : ReflectionTrait
    {
        ...
        if (\false === $class instanceof ReflectionTrait) {
            throw new ClassNotFound(\sprintf('"%s" is not a trait, it is a "%s"', $className->full(), \get_class($class)));
        }
        return $class;
    }
    public function reflectEnum($className) : ReflectionEnum
    {
        ...
        if (\false === $class instanceof ReflectionEnum) {
            throw new ClassNotFound(\sprintf('"%s" is not an enum, it is a "%s"', $className->full(), \get_class($class)));
        }
        return $class;
    }
    /**
    ...
    public function reflectClassLike($className, array $visited = []) : ReflectionClassLike
    {
        ...
        if (isset($visited[$className->__toString()])) {
            throw new CycleDetected(\sprintf('Cycle detected while resolving class "%s"', $className->full()));
        }
        $visited[$className->__toString()] = \true;
        ...
        if (\false === $classes->has((string) $className)) {
            throw new ClassNotFound(\sprintf('Unable to locate class "%s"', $className->full()));
        }
    ...
    public function reflectFunction($name) : ReflectionFunction
    {
        ...
        if (\false === $functions->has((string) $name)) {
            ...
            if (\false === $functions->has($name)) {
                throw new FunctionNotFound(\sprintf('Unable to locate function "%s"', $name));
            }
        }
    ...
    public function reflectConstant($name) : ReflectionDeclaredConstant
    {
        ...
        if (\false === $constants->has((string) $name)) {
            ...
            if (\false === $constants->has($name)) {
                throw new ConstantNotFound(\sprintf('Unable to locate constant "%s"', $name));
            }
        }
