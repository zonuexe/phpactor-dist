...
class SourceCode implements TextDocument
{
    ...
    public static function fromUnknown($value) : SourceCode
    {
        ...
        if ($value instanceof TextDocument) {
            if (null === $value->uri()) {
                return self::fromString($value->__toString());
            }
            return self::fromPathAndString($value->uri()->path(), $value->__toString());
        }
        if (\is_string($value)) {
            return self::fromString($value);
        }
        /** @phpstan-ignore-next-line */
        throw new InvalidArgumentException(\sprintf('Do not know how to create source code from type "%s"', \is_object($value) ? \get_class($value) : \gettype($value)));
    }
    ...
    public function mustGetUri() : TextDocumentUri
    {
        $uri = $this->uri();
        if (null === $uri) {
            throw new RuntimeException('URI has not been set on source code');
        }
        return $uri;
    }
