...
class ArrayLiteral extends \Phpactor\WorseReflection\Core\Type\ArrayType implements \Phpactor\WorseReflection\Core\Type\Literal, \Phpactor\WorseReflection\Core\Type\Generalizable, \Phpactor\WorseReflection\Core\Type\ArrayAccessType
{
    ...
    {
        if ($this->isList()) {
            return \sprintf('array{%s}', \implode(',', \array_map(fn(Type $type) => \sprintf('%s', $type->__toString()), \array_values($this->typeMap))));
        }
        return \sprintf('array{%s}', \implode(',', \array_map(fn($key, Type $type) => \sprintf('%s:%s', $key, $type->__toString()), \array_keys($this->typeMap), \array_values($this->typeMap))));
    }
    /**
     * @return Type[]
     */
    ...
    {
        return \array_map(fn(Type $type) => TypeUtil::valueOrNull($type), $this->typeMap);
    }
