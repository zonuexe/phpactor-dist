...
class ArrayShapeType extends ArrayType implements Generalizable, ArrayAccessType
{
    ...
    {
        if ($this->isList()) {
            return \sprintf('array{%s}', \implode(',', \array_map(fn(Type $t) => $t->__toString(), $this->typeMap)));
        }
        return \sprintf('array{%s}', \implode(',', \array_map(fn(string $key, Type $t) => \sprintf('%s:%s', $key, $t->__toString()), \array_keys($this->typeMap), $this->typeMap)));
    }
    ...
    {
        return new self(\array_map(function (Type $type) use($mapper) {
