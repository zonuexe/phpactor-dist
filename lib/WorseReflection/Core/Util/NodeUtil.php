...
class NodeUtil
{
    private const RESERVED_NAMES = ['iterable', 'resource'];
    /**
    ...
    {
        $out = [\sprintf('%s %d:%d - %s', \str_repeat('  ', $level) . $node->getNodeKindName(), $node->getStartPosition(), $node->getEndPosition(), \str_replace("\n", '\\n', $node->getText()))];
        $level++;
    ...
    public static function typeFromQualfiedNameLike(Reflector $reflector, Node $node, $nodeOrToken, ?ClassName $classContext = null) : Type
    {
        ...
        if ($nodeOrToken instanceof QualifiedNameList) {
            ...
            }, \iterator_to_array($nodeOrToken->getElements(), \true)), fn($name) => $name !== \false);
...
\class_alias('Phpactor202301\\Phpactor\\WorseReflection\\Core\\Util\\NodeUtil', 'Phpactor\\WorseReflection\\Core\\Util\\NodeUtil', \false);
