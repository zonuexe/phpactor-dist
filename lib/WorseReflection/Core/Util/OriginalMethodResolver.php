...
class OriginalMethodResolver
{
    ...
    private function doResolveOriginalMember(ReflectionClassLike $classLike, ReflectionMember $member) : ReflectionMember
    {
    ...
    private function resolveClass(ReflectionClass $classLike, ReflectionMember $member) : ReflectionMember
    {
        ...
        if ($parent !== null) {
            $member = $this->doResolveOriginalMember($classLike->parent(), $member);
        }
        foreach ($classLike->interfaces() as $interface) {
            $member = $this->doResolveOriginalMember($interface, $member);
        }
        return $member;
    }
    private function resolveInterface(ReflectionInterface $classLike, ReflectionMember $member) : ReflectionMember
    {
        foreach ($classLike->parents() as $parent) {
            $member = $this->doResolveOriginalMember($parent, $member);
        }
        return $member;
    }
}
