...
class MemberTypeResolver
{
    ...
    private function memberType(string $memberType, Type $containerType, NodeContext $info, string $name)
    {
        ...
        if (!$containerType instanceof ClassType) {
            return $info->withIssue(\sprintf('Containing type is not a class, got "%s"', (string) $containerType));
        }
        try {
            $class = $this->reflectClassOrNull($containerType, $name);
        } catch (NotFound $e) {
            $info = $info->withIssue(\sprintf('Could not find container class "%s" for "%s"', (string) $containerType, $name));
            return $info;
        }
        $info = $info->withContainerType(TypeFactory::reflectedClass($this->reflector, $class->name()));
        if (!\method_exists($class, $memberType)) {
            $info = $info->withIssue(\sprintf('Container class "%s" has no method "%s"', (string) $containerType, $memberType));
            return $info;
        }
        try {
            if (\false === $class->{$memberType}()->has($name)) {
                $info = $info->withIssue(\sprintf('Class "%s" has no %s named "%s"', (string) $containerType, $memberType, $name));
                return $info;
            }
        } catch (NotFound $e) {
