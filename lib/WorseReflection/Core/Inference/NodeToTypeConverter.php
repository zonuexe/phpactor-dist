...
class NodeToTypeConverter
{
    ...
    public function __construct(private Reflector $reflector, ?LoggerInterface $logger = null)
    {
    ...
    private function parentClass(Node $node) : Type
    {
        ...
        return TypeFactory::fromStringWithReflector($class->classBaseClause->baseClass->getResolvedName(), $this->reflector);
    }
    ...
    private function fromClassImports(Node $node, Type $type) : ?Type
    {
        ...
        if (isset($classImports[$type->name->head()->__toString()])) {
            $type->name = ClassName::fromString((string) $classImports[(string) $type->name->head()] . '\\' . (string) $type->name->tail());
            return $type;
        }
        return null;
    }
