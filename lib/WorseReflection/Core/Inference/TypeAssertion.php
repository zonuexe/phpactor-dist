...
final class TypeAssertion
{
    ...
    private bool $polarity = \true;
    /**
     * @param self::VARIABLE_TYPE_* $variableType
     */
    private function __construct(private string $variableType, string $name, private int $offset, Closure $true, Closure $false, private ?Type $classType = null)
    {
    ...
    {
        return \sprintf('%s: %s#%s %s', $this->variableType(), $this->name(), $this->offset(), $this->polarity() ? 'positive' : 'negative');
    }
