...
class IfStatementResolver implements Resolver
{
    public function resolve(NodeContextResolver $resolver, Frame $frame, Node $node) : NodeContext
    {
        ...
        $this->ifBranch($resolver, $frame, $node, $node->getStartPosition(), $this->resolveInitialEndPosition($node));
        // apply type assertions only within the if block elseif clauses
        foreach ($node->elseIfClauses as $clause) {
            $this->ifBranch($resolver, $frame, $clause, $clause->getStartPosition(), $clause->getEndPosition());
        }
        ...
        $this->ifBranchTermination($resolver, $frame, $node, $node->getStartPosition(), $node->getEndPosition());
        // handle terminateion for elseif clauses
        foreach ($node->elseIfClauses as $clause) {
            $this->ifBranchTermination($resolver, $frame, $clause, $node->getStartPosition(), $clause->getEndPosition());
        }
     ...
     */
    private function ifBranch(NodeContextResolver $resolver, Frame $frame, $node, int $start, int $end) : void
    {
        ...
        $frame->applyTypeAssertions($context->typeAssertions()->negate(), $start, $end);
    }
    /**
     * @param IfStatementNode|ElseIfClauseNode $node
     */
    private function ifBranchTermination(NodeContextResolver $resolver, Frame $frame, $node, int $start, int $end) : void
    {
        $context = $resolver->resolveNode($frame, $node->expression);
