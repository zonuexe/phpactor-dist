...
class SubscriptExpressionResolver implements Resolver
{
    public function resolve(NodeContextResolver $resolver, Frame $frame, Node $node) : NodeContext
    {
        ...
        if (null === $node->accessExpression) {
            $info = $info->withIssue(\sprintf('Subscript expression "%s" is incomplete', (string) $node->getText()));
            return $info;
        }
        $node = $node->accessExpression;
        $type = $info->type();
        if (!$type instanceof ArrayType) {
            $info = $info->withIssue(\sprintf('Not resolving subscript expression of type "%s"', (string) $info->type()));
            return $info;
        }
        $arrayLiteralType = $info->type();
        $info = $info->withType($type->iterableValueType());
        if (!$arrayLiteralType instanceof ArrayAccessType) {
            $info = $info->withIssue(\sprintf('Array value for symbol "%s" is not an array, is a "%s"', (string) $info->symbol(), $arrayLiteralType->__toString()));
            return $info;
        }
        ...
        $info = $info->withIssue(\sprintf('Did not resolve access expression for node type "%s"', \get_class($node)));
        return $info;
    }
}
