...
class MethodDeclarationResolver implements Resolver
{
    public function resolve(NodeContextResolver $resolver, Frame $frame, Node $node) : NodeContext
    {
        ...
        return new MemberDeclarationContext(Symbol::fromTypeNameAndPosition(Symbol::METHOD, (string) $node->name->getText($node->getFileContents()), ByteOffsetRange::fromInts($node->name->getStartPosition(), $node->name->getEndPosition())), TypeFactory::unknown(), $classSymbolContext->type());
    }
}
