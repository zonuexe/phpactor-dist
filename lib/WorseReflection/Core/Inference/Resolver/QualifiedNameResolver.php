...
class QualifiedNameResolver implements Resolver
{
    ...
    private function resolveContext(QualifiedName $node) : NodeContext
    {
        ...
        if ($type instanceof ReflectedClassType) {
            try {
                ...
                return new ClassLikeContext($context->symbol(), ByteOffsetRange::fromInts($node->getStartPosition(), $node->getEndPosition()), $class);
            } catch (NotFound) {
    ...
    private function resolveContextFromCall(NodeContextResolver $resolver, Frame $frame, CallExpression $parent, QualifiedName $node) : NodeContext
    {
        ...
        $range = ByteOffsetRange::fromInts($node->getStartPosition(), $node->getEndPosition());
        try {
