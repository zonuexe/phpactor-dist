...
use Phpactor202301\Phpactor\WorseReflection\Core\Inference\Variable as PhpactorVariable;
...
use Phpactor202301\Phpactor\WorseReflection\Core\Type\Comparable;
use Phpactor202301\Phpactor\WorseReflection\Core\Type\Concatable;
...
class BinaryExpressionResolver implements Resolver
{
    public function resolve(NodeContextResolver $resolver, Frame $frame, Node $node) : NodeContext
    {
        ...
        $context = NodeContextFactory::create($node->getText(), $node->getStartPosition(), $node->getEndPosition(), []);
    ...
    private function walkBinaryExpression(Type $left, Type $right, int $operator) : Type
    {
        ...
        if ($left instanceof ClassType) {
            switch ($operator) {
                case TokenKind::InstanceOfKeyword:
                    return TypeFactory::boolLiteral(\true);
            }
        }
        return new MissingType();
    }
    private function applyTypeAssertions(NodeContext $context, NodeContext $leftContext, NodeContext $rightContext, Node $leftOperand, Node $rightOperand, int $operator) : NodeContext
    {
        switch ($operator) {
            case TokenKind::OrKeyword:
            case TokenKind::BarBarToken:
                return $context->withTypeAssertions($leftContext->typeAssertions()->or($rightContext->typeAssertions()));
            case TokenKind::AndKeyword:
            case TokenKind::AmpersandAmpersandToken:
                return $context->withTypeAssertions($leftContext->typeAssertions()->and($rightContext->typeAssertions()));
        }
        if (!NodeUtil::canAcceptTypeAssertion($leftOperand, $rightOperand)) {
            return $context;
        }
        [$reciever, $recieverContext] = NodeUtil::canAcceptTypeAssertion($leftOperand) ? [$leftOperand, $leftContext] : [$rightOperand, $rightContext];
        [$transmitter, $transmittingContext] = NodeUtil::canAcceptTypeAssertion($rightOperand) ? [$leftOperand, $leftContext] : [$rightOperand, $rightContext];
        ...
        return match ($operator) {
            ...
            TokenKind::InstanceOfKeyword => $context->withTypeAssertion(TypeAssertion::forContext($recieverContext, function (Type $type) use($transmittingContext) {
    ...
    private function negate(NodeContext $context, Node $leftOperand, Node $rightOperand, int $operator) : NodeContext
    {
    ...
    private function addVariable(int $operator, Frame $frame, Node $leftOperand, NodeContext $context) : void
    {
        if (!$leftOperand instanceof Variable) {
            return;
        }
        if (!\in_array($operator, [TokenKind::DotEqualsToken, TokenKind::DotToken])) {
            return;
        }
        $name = NodeUtil::nameFromTokenOrNode($leftOperand, $leftOperand->name);
        $context = NodeContextFactory::create($name, $leftOperand->getStartPosition(), $leftOperand->getEndPosition(), ['symbol_type' => Symbol::VARIABLE, 'type' => $context->type()]);
        $frame->locals()->set(PhpactorVariable::fromSymbolContext($context));
    }
}
\class_alias('Phpactor202301\\Phpactor\\WorseReflection\\Core\\Inference\\Resolver\\BinaryExpressionResolver', 'Phpactor\\WorseReflection\\Core\\Inference\\Resolver\\BinaryExpressionResolver', \false);
