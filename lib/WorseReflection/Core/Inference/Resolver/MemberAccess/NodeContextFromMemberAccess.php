...
class NodeContextFromMemberAccess
{
     ...
     */
    public function __construct(private GenericMapResolver $resolver, private array $memberResolvers)
    {
    }
    public function infoFromMemberAccess(NodeContextResolver $resolver, Frame $frame, Type $classType, Node $node) : NodeContext
    {
        ...
        if (Symbol::PROPERTY === $memberTypeName && $node instanceof ScopedPropertyAccessExpression && \is_string($memberName) && !\str_starts_with($memberName, '$')) {
            $memberTypeName = Symbol::CONSTANT;
        }
        $context = NodeContextFactory::create((string) $memberName, $node->getStartPosition(), $node->getEndPosition(), ['symbol_type' => $memberTypeName]);
