    ...
    public function locate(Name $name) : SourceCode
    {
        ...
        foreach ($this->locators as $locator) {
            ...
            try {
                ...
                $this->logger->debug(\sprintf(' OK [%s] "%s" with locator "%s"', \number_format(\microtime(\true) - $start, 4), $name, \get_class($locator)));
                return $source;
            } catch (SourceNotFound $e) {
                $this->logger->debug(\sprintf('NOK [%s] "%s" with locator "%s" : %s', \number_format(\microtime(\true) - $start, 4), $name, \get_class($locator), $e->getMessage()));
                $exception = new SourceNotFound(\sprintf('Could not find source with "%s"', (string) $name), 0, $e);
            }
        }
        throw $exception;
    }
...
\class_alias('Phpactor202301\\Phpactor\\WorseReflection\\Core\\SourceCodeLocator\\ChainSourceLocator', 'Phpactor\\WorseReflection\\Core\\SourceCodeLocator\\ChainSourceLocator', \false);
