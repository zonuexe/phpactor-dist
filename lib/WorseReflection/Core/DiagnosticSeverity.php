...
final class DiagnosticSeverity
{
    ...
    {
        $validLevels = [self::INFORMATION, self::WARNING, self::ERROR, self::HINT];
        if (!\in_array($level, $validLevels)) {
            throw new RuntimeException(\sprintf('Severity must be one of DiagnosticSeverity::*, got: %d (valid values: %s)', $level, \implode(', ', $validLevels)));
        }
    }
