...
abstract class AbstractReflectionCollection implements \Phpactor\WorseReflection\Core\Reflection\Collection\ReflectionCollection
{
     ...
     */
    protected final function __construct(protected array $items)
    {
    }
    ...
    public function get(string $name)
    {
        if (!isset($this->items[$name])) {
            throw new ItemNotFound(\sprintf('Unknown item "%s", known items: "%s"', $name, \implode('", "', \array_keys($this->items))));
        }
    ...
    {
        if (empty($this->items)) {
            throw new ItemNotFound('Collection is empty, cannot get the first item');
        }
    ...
    {
        if (empty($this->items)) {
            throw new ItemNotFound('Collection is empty, cannot get the last item');
        }
