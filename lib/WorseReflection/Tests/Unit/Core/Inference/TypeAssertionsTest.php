...
use Phpactor202301\Phpactor\WorseReflection\Core\Inference\TypeAssertions;
...
class TypeAssertionsTest extends TestCase
{
    ...
    public function provideOr() : Generator
    {
        (yield [
            ...
            TypeAssertion::variable('foo', 0, fn(Type $t) => $t->addType(TypeFactory::string()), fn(Type $t) => $t->addType(TypeFactory::null())),
            // assert foo is STRING positively and int NULL negatively
            TypeAssertion::variable('foo', 0, fn(Type $t) => $t->addType(TypeFactory::int()), fn(Type $t) => $t->addType(TypeFactory::float())),
            // it's either mixed, int or string
            TypeFactory::union(TypeFactory::mixed(), TypeFactory::string(), TypeFactory::int()),
            // it's either mixed, int or string
            TypeFactory::union(TypeFactory::mixed(), TypeFactory::null(), TypeFactory::float()),
        ]);
    }
    /**
...
\class_alias('Phpactor202301\\Phpactor\\WorseReflection\\Tests\\Unit\\Core\\Inference\\TypeAssertionsTest', 'Phpactor\\WorseReflection\\Tests\\Unit\\Core\\Inference\\TypeAssertionsTest', \false);
