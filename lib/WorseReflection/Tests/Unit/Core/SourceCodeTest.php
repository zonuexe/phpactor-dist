...
class SourceCodeTest extends TestCase
{
    ...
    {
        $document = TextDocumentBuilder::create(self::SOURCE_CODE)->uri(self::EXAMPLE_PATH)->build();
    ...
    {
        $document = TextDocumentBuilder::create(self::SOURCE_CODE)->build();
        $sourceCode = SourceCode::fromUnknown($document);
        $this->assertEquals($sourceCode->__toString(), self::SOURCE_CODE);
    }
}
\class_alias('Phpactor202301\\Phpactor\\WorseReflection\\Tests\\Unit\\Core\\SourceCodeTest', 'Phpactor\\WorseReflection\\Tests\\Unit\\Core\\SourceCodeTest', \false);
