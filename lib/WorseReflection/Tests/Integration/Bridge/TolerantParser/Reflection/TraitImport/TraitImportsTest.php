...
namespace Phpactor202301\Phpactor\WorseReflection\Tests\Integration\Bridge\TolerantParser\Reflection\TraitImport;

use Closure;
use Phpactor202301\Microsoft\PhpParser\Node\Statement\ClassDeclaration;
use Phpactor202301\Phpactor\WorseReflection\Bridge\TolerantParser\Reflection\TraitImport\TraitAlias;
...
class TraitImportsTest extends IntegrationTestCase
{
    ...
    {
        (yield 'simple use' => ['<?php trait A {}; class B { use A; }', function (TraitImports $traitImports) : void {
            $this->assertCount(1, $traitImports);
            $this->assertTrue($traitImports->has('A'));
            $this->assertEquals('A', $traitImports->get('A')->name());
        }]);
        (yield 'incomplete statement' => ['<?php trait A {}; class B { use }', function (TraitImports $traitImports) : void {
            $this->assertCount(0, $traitImports);
        }]);
        (yield 'simple use with alias' => ['<?php trait A { function foo() {}}; class B { use A { foo as bar; }', function (TraitImports $traitImports) : void {
            $this->assertCount(1, $traitImports);
        ...
        (yield 'simple use with alias and visiblity' => ['<?php trait A { function foo() {}}; class B { use A { foo as private bar; bar as protected bar; zed as public aar;}', function (TraitImports $traitImports) : void {
            $traitImport = $traitImports->get('A');
        ...
        (yield 'does not support insteadof' => ['<?php trait A { function foo(){} function bar()}}' . 'trait B { function foo() {}} ' . 'class B { use A, B { B::foo insteadof A } }', function (TraitImports $traitImports) : void {
            $traitImport = $traitImports->get('A');
            $this->assertCount(0, $traitImport->traitAliases());
        }]);
        (yield 'multiple traits with single alias maping' => ['<?php trait A { function foo(){} }' . 'trait B { function bar(){} } ' . 'class B { use A, B { foo as foo1; bar as bar1 } }', function (TraitImports $traitImports) : void {
...
\class_alias('Phpactor202301\\Phpactor\\WorseReflection\\Tests\\Integration\\Bridge\\TolerantParser\\Reflection\\TraitImport\\TraitImportsTest', 'Phpactor\\WorseReflection\\Tests\\Integration\\Bridge\\TolerantParser\\Reflection\\TraitImport\\TraitImportsTest', \false);
