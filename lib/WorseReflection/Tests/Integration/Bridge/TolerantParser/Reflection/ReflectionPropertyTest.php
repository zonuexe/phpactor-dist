...
use Phpactor202301\Phpactor\WorseReflection\Core\Reflection\Collection\ReflectionPropertyCollection as PhpactorReflectionPropertyCollection;
...
class ReflectionPropertyTest extends IntegrationTestCase
{
    ...
    {
        (yield 'It reflects a property with union type' => ['<?php class Foobar { private int|string $property;}', 'Foobar', function (ReflectionPropertyCollection $properties) : void {
            $this->assertEquals('property', $properties->get('property')->name());
            $this->assertEquals(TypeFactory::union(...[TypeFactory::int(), TypeFactory::string()]), $properties->get('property')->inferredType());
        }]);
    }
    /**
     * @return Generator<string,array{string,string,Closure(ReflectionProperty): void}>
    ...
    {
        (yield 'It reflects a property' => [<<<'EOT'
<?php
...
EOT
, 'Foobar', function (ReflectionPropertyCollection $properties) : void {
            $this->assertEquals('property', $properties->get('property')->name());
...
, 'Foobar', function (ReflectionPropertyCollection $properties) : void {
        ...
        (yield 'Inherited properties' => [<<<'EOT'
<?php
...
\class_alias('Phpactor202301\\Foobar', 'Foobar', \false);
EOT
, 'Foobar', function (PhpactorReflectionPropertyCollection $properties) : void {
            $this->assertEquals(['property5', 'property2', 'property3', 'property4'], $properties->keys());
            self::assertEquals('ParentParentClass', $properties->get('property5')->declaringClass()->name()->head()->__toString());
...
EOT
, 'Foobar', function (ReflectionPropertyCollection $properties) : void {
            $this->assertEquals('Phpactor202301\\Acme\\Post', $properties->get('property1')->inferredType()->__toString());
            $this->assertFalse($properties->get('property1')->isStatic());
        }]);
        (yield 'Returns unknown type for (real) type' => [<<<'EOT'
<?php
...
EOT
, 'Foobar', function (ReflectionPropertyCollection $properties) : void {
            $this->assertEquals(TypeFactory::unknown(), $properties->get('property1')->type());
        }]);
        (yield 'Property with assignment' => [<<<'EOT'
<?php
...
, 'Foobar', function (ReflectionPropertyCollection $properties) : void {
        ...
        (yield 'Return true if property is static' => [<<<'EOT'
<?php
...
, 'Foobar', function (ReflectionPropertyCollection $properties) : void {
        ...
        (yield 'Property type from class @property annotation' => [<<<'EOT'
<?php
...
, 'Foobar', function (ReflectionPropertyCollection $properties) : void {
        ...
        (yield 'Property type from class @property annotation with imported name' => [<<<'EOT'
<?php

namespace Phpactor202301;

use Phpactor202301\Acme\Post;
use Phpactor202301\Bar\Foo;
/**
 * @property Foo $bar
 */
class Foobar
...
EOT
, 'Foobar', function (ReflectionPropertyCollection $properties) : void {
            $this->assertEquals('Phpactor202301\\Bar\\Foo', $properties->get('bar')->inferredType()->__toString());
        }]);
        (yield 'Property type from parent class @property annotation with imported name' => [<<<'EOT'
<?php
...
, 'Foobar', function (ReflectionPropertyCollection $properties) : void {
        ...
        (yield 'Typed property from imported class' => [<<<'EOT'
<?php
...
, 'Phpactor202301\\Test\\Barfoo', function (ReflectionPropertyCollection $properties) : void {
            $this->assertEquals('Phpactor202301\\Bar\\Foo', $properties->get('bar')->type()->__toString());
            $this->assertEquals('Phpactor202301\\Bar\\Foo', $properties->get('bar')->inferredType()->__toString());
            ...
            $this->assertEquals(TypeFactory::iterable(), $properties->get('it')->type());
        }]);
        (yield 'Nullable typed property' => [<<<'EOT'
<?php
...
EOT
, 'Phpactor202301\\Test\\Barfoo', function (ReflectionPropertyCollection $properties) : void {
            $this->assertEquals(TypeFactory::fromString('?string'), $properties->get('foo')->type());
        }]);
        (yield 'Property with intersection' => [<<<'EOT'
<?php
...
EOT
, 'Phpactor202301\\Test\\Barfoo', function (ReflectionPropertyCollection $properties) : void {
            $this->assertEquals(TypeFactory::intersection(TypeFactory::class('Phpactor202301\\Test\\Foo'), TypeFactory::class('Phpactor202301\\Test\\Bar'))->__toString(), $properties->get('foo')->type()->__toString());
        }]);
    }
}
\class_alias('Phpactor202301\\Phpactor\\WorseReflection\\Tests\\Integration\\Bridge\\TolerantParser\\Reflection\\ReflectionPropertyTest', 'Phpactor\\WorseReflection\\Tests\\Integration\\Bridge\\TolerantParser\\Reflection\\ReflectionPropertyTest', \false);
