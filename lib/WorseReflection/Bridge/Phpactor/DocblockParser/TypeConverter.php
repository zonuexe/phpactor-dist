...
use Phpactor202301\Phpactor\WorseReflection\Core\Type\ListType;
...
use Phpactor202301\Phpactor\WorseReflection\Core\Type\ParenthesizedType as PhpactorParenthesizedType;
...
class TypeConverter
{
    ...
    {
        return new UnionType(...\array_map(fn(Node $node) => $this->convert($node), \iterator_to_array($union->types->types())));
    }
    private function convertIntersection(IntersectionNode $type) : Type
    {
        return new IntersectionType(...\array_map(fn(Node $node) => $this->convert($node), \iterator_to_array($type->types->types())));
    }
    private function convertGeneric(GenericNode $type) : Type
    {
        if ($type->type instanceof ArrayNode) {
            ...
            if (\count($parameters) === 1) {
                return new ArrayType(null, $this->convert($parameters[0]));
            }
            if (\count($parameters) === 2) {
                return new ArrayType($this->convert($parameters[0]), $this->convert($parameters[1]));
            }
            return new ArrayType(new MissingType());
        }
        if ($type->type instanceof ScalarNode && $type->type->name->value === 'int') {
            $parameters = \array_values(\iterator_to_array($type->parameters()->types()));
            if (\count($parameters) === 2) {
                return new IntRangeType($this->convert($parameters[0]), $this->convert($parameters[1]));
            }
        }
        if ($type->type instanceof ListNode) {
            $parameters = \array_values(\iterator_to_array($type->parameters()->types()));
            if (\count($parameters) === 1) {
                return new ListType($this->convert($parameters[0]));
            }
        ...
        if ($type->type instanceof ClassNode && $type->type->name->value === 'iterable') {
            $parameters = \array_values(\iterator_to_array($type->parameters()->types()));
            if (\count($parameters) === 1) {
                return new PseudoIterableType(new ArrayKeyType(), $this->convert($parameters[0]));
            }
            if (\count($parameters) === 2) {
                return new PseudoIterableType($this->convert($parameters[0]), $this->convert($parameters[1]));
            }
        ...
        if ($classType instanceof ClassStringType) {
            ...
            if ($parameters->count()) {
                return new ClassStringType(ClassName::fromString($this->convert($parameters->types()->first())->__toString()));
            }
            return $classType;
        }
        ...
        return new GenericClassType($this->reflector, $classType->name(), \array_map(fn(TypeNode $node) => $this->convert($node), $parameters));
    }
    private function convertClass(ClassNode $typeNode) : Type
    {
        ...
        $type = new ReflectedClassType($this->reflector, ClassName::fromString($typeNode->name()->toString()));
    ...
    private function convertLiteralString(LiteralStringNode $type) : Type
    {
        ...
        $string = \trim($type->token->value, $quote);
...
\class_alias('Phpactor202301\\Phpactor\\WorseReflection\\Bridge\\Phpactor\\DocblockParser\\TypeConverter', 'Phpactor\\WorseReflection\\Bridge\\Phpactor\\DocblockParser\\TypeConverter', \false);
