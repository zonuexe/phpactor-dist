...
class DocblockMissingParamProvider implements DiagnosticProvider
{
    public function exit(NodeContextResolver $resolver, Frame $frame, Node $node) : iterable
    {
        ...
        foreach ($method->parameters() as $parameter) {
            ...
            (yield new \Phpactor\WorseReflection\Bridge\TolerantParser\Diagnostics\DocblockMissingParamDiagnostic(ByteOffsetRange::fromInts($parameter->position()->start()->toInt(), $parameter->position()->end()->toInt()), \sprintf('Method "%s" is missing @param $%s', $methodName, $parameter->name()), DiagnosticSeverity::WARNING(), $class->name()->__toString(), $methodName, $parameter->name(), $type));
        }
    }
    ...
    {
        (yield new DiagnosticExample(title: 'closure', source: <<<'PHP'
<?php
...
, valid: \false, assertion: function (Diagnostics $diagnostics) : void {
            $diagnostics = $diagnostics->byClass(\Phpactor\WorseReflection\Bridge\TolerantParser\Diagnostics\DocblockMissingParamDiagnostic::class);
            Assert::assertCount(1, $diagnostics);
            Assert::assertEquals('Method "foo" is missing @param $foobar', $diagnostics->at(0)->message());
        }));
        (yield new DiagnosticExample(title: 'generator', source: <<<'PHP'
<?php
 ...
 * @template TValue of string
 */
\class_alias('PhpactorDist\\Generator', 'Generator', \false);
class Foobar
{
...
, valid: \false, assertion: function (Diagnostics $diagnostics) : void {
        ...
        (yield new DiagnosticExample(title: 'array', source: <<<'PHP'
<?php
...
, valid: \false, assertion: function (Diagnostics $diagnostics) : void {
        ...
        (yield new DiagnosticExample(title: 'does not report diagnostic on method with @param', source: <<<'PHP'
<?php
...
, valid: \true, assertion: function (Diagnostics $diagnostics) : void {
        ...
        (yield new DiagnosticExample(title: 'variadic', source: <<<'PHP'
<?php
