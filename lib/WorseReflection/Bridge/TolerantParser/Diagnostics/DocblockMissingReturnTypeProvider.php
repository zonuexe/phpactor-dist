...
class DocblockMissingReturnTypeProvider implements DiagnosticProvider
{
    public function exit(NodeContextResolver $resolver, Frame $frame, Node $node) : iterable
    {
        ...
        if ($actualReturnType->isClosure()) {
            (yield new \Phpactor\WorseReflection\Bridge\TolerantParser\Diagnostics\DocblockMissingReturnTypeDiagnostic($method->nameRange(), \sprintf('Method "%s" is missing docblock return type: %s', $methodName, $actualReturnType->__toString()), DiagnosticSeverity::WARNING(), $class->name()->__toString(), $methodName, $actualReturnType->__toString()));
            return;
        }
    ...
    {
        (yield new DiagnosticExample(title: 'method without return type', source: <<<'PHP'
<?php
