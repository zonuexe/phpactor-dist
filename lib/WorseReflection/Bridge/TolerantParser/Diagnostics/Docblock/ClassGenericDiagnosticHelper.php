...
final class ClassGenericDiagnosticHelper
{
     ...
     */
    public function diagnosticsForExtends(ClassReflector $reflector, ByteOffsetRange $range, ReflectionClassLike $class, ?ReflectionClassLike $parentClass) : Generator
    {
     ...
     */
    private function fromReflectionClass(ClassReflector $reflector, ByteOffsetRange $range, ReflectionClassLike $class, ?ReflectionClassLike $parentClass, array $genericTypes, string $tagName) : Generator
    {
        if (!$parentClass) {
            return;
        }
        ...
        $defaultGenericType = new GenericClassType($reflector, $parentClass->name(), \array_map(fn(Type $type) => $type instanceof MissingType ? new MixedType() : $type, $templateMap->toArguments()));
        if (0 === \count($genericTypes)) {
            (yield new DocblockMissingClassGenericDiagnostic($range, $class->name(), $defaultGenericType, $tagName));
            return;
        }
        $extendTagType = $genericTypes[\array_key_first($genericTypes)];
        if ($parentClass->type()->upcastToGeneric()->accepts($extendTagType)->isFalse()) {
            (yield new DocblockIncorrectClassGenericDiagnostic($range, $extendTagType, $defaultGenericType, $tagName));
            return;
        }
