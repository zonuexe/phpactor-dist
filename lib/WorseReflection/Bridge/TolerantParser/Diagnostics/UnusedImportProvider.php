...
class UnusedImportProvider implements DiagnosticProvider
{
    ...
    {
        $docblock = $resolver->docblockFactory()->create($node->getLeadingCommentAndWhitespaceText(), new ReflectionScope($resolver->reflector(), $node));
        ...
        if ($node instanceof QualifiedName && !$node->parent instanceof NamespaceUseClause && !$node->parent instanceof NamespaceDefinition && !$node->parent instanceof NamespaceUseGroupClause) {
            ...
            $this->usedPrefixes[$usedPrefix] = \true;
            return [];
        }
    ...
    {
        (yield new DiagnosticExample(title: 'aliased import', source: <<<'PHP'
<?php

namespace PhpactorDist;

use PhpactorDist\Foobar as Barfoo;
use PhpactorDist\Bagggg as Bazgar;
new Barfoo();
...
, valid: \false, assertion: function (Diagnostics $diagnostics) : void {
        ...
        (yield new DiagnosticExample(title: 'aliased for used', source: <<<'PHP'
<?php
...
, valid: \true, assertion: function (Diagnostics $diagnostics) : void {
        ...
        (yield new DiagnosticExample(title: 'imported in one namespace but used in another', source: <<<'PHP'
<?php

namespace PhpactorDist\One;

use PhpactorDist\Foo;
namespace PhpactorDist\Two;
...
, valid: \false, assertion: function (Diagnostics $diagnostics) : void {
        ...
        (yield new DiagnosticExample(title: 'compact namespaced use', source: <<<'PHP'
<?php

namespace PhpactorDist\Foo\Foobar;

use PhpactorDist\Foo\Ham;
use PhpactorDist\Foo\Spam;
new Ham();
new Spam();
PHP
, valid: \true, assertion: function (Diagnostics $diagnostics) : void {
        ...
        (yield new DiagnosticExample(title: 'compact use unused', source: <<<'PHP'
<?php

namespace PhpactorDist;

use PhpactorDist\Foobar\Barfoo;
new Foobar();
PHP
, valid: \false, assertion: function (Diagnostics $diagnostics) : void {
            Assert::assertCount(1, $diagnostics);
        ...
        (yield new DiagnosticExample(title: 'gh-1866', source: <<<'PHP'
<?php
...
, valid: \true, assertion: function (Diagnostics $diagnostics) : void {
        ...
        (yield new DiagnosticExample(title: 'namespaced unused imports', source: <<<'PHP'
<?php

namespace PhpactorDist\Foo;

use PhpactorDist\Bar\Foobar;
use PhpactorDist\Bag\Boo;
new Boo();
PHP
, valid: \false, assertion: function (Diagnostics $diagnostics) : void {
            Assert::assertCount(1, $diagnostics);
        }));
        (yield new DiagnosticExample(title: 'namespaced used imports', source: <<<'PHP'
<?php
...
, valid: \true, assertion: function (Diagnostics $diagnostics) : void {
        ...
        (yield new DiagnosticExample(title: 'trait', source: <<<'PHP'
<?php
...
, valid: \true, assertion: function (Diagnostics $diagnostics) : void {
        ...
        (yield new DiagnosticExample(title: 'unused imort', source: <<<'PHP'
<?php

namespace PhpactorDist;

use PhpactorDist\Foobar;
PHP
, valid: \false, assertion: function (Diagnostics $diagnostics) : void {
            Assert::assertCount(1, $diagnostics);
            Assert::assertEquals('Name "Foobar" is imported but not used', $diagnostics->at(0)->message());
        }));
        (yield new DiagnosticExample(title: 'used by complex docblock', source: <<<'PHP'
<?php

namespace PhpactorDist\Bar;
...
, valid: \true, assertion: function (Diagnostics $diagnostics) : void {
        ...
        (yield new DiagnosticExample(title: 'used by doctrine annotation', source: <<<'PHP'
<?php

namespace PhpactorDist;

use PhpactorDist\Doctrine\ORM;
/**
 * @ORM\Foo $bar
 */
...
, valid: \true, assertion: function (Diagnostics $diagnostics) : void {
        ...
        (yield new DiagnosticExample(title: 'used by doctrine annotation aliased', source: <<<'PHP'
<?php

namespace PhpactorDist;

use PhpactorDist\Doctrine\ORM as Mapping;
/**
 * @Mapping\Foo $bar
 */
function fo($foo) : void
...
, valid: \true, assertion: function (Diagnostics $diagnostics) : void {
        ...
        (yield new DiagnosticExample(title: 'used by dcotrine attribute', source: <<<'PHP'
<?php

namespace PhpactorDist;

use PhpactorDist\Doctrine\Mapping;
#[Mapping\Id]
function fo($foo) : void
{
}
...
, valid: \true, assertion: function (Diagnostics $diagnostics) : void {
        ...
        (yield new DiagnosticExample(title: 'used by namespaced annotation', source: <<<'PHP'
<?php

namespace PhpactorDist;

use PhpactorDist\Foobar\Foo;
/**
 * @param Foo $bar
 */
function fo($foo) : void
...
, valid: \true, assertion: function (Diagnostics $diagnostics) : void {
        ...
        (yield new DiagnosticExample(title: 'used by throws annotation', source: <<<'PHP'
<?php
...
, valid: \true, assertion: function (Diagnostics $diagnostics) : void {
        ...
        (yield new DiagnosticExample(title: 'used by import', source: <<<'PHP'
<?php
...
, valid: \true, assertion: function (Diagnostics $diagnostics) : void {
    ...
    private function extractDocblockNames(Docblock $docblock, NodeContextResolver $resolver, Node $node) : void
    {
        ...
        foreach ($docblock->descendantElements(ClassNode::class) as $type) {
            $this->usedPrefixes[$prefix . $type->toString()] = \true;
        }
        foreach ($docblock->descendantElements(CallableNode::class) as $type) {
            ...
            if ($type->name->toString() === 'Closure') {
                $this->usedPrefixes[$prefix . 'Closure'] = \true;
            }
        }
    }
    /**
