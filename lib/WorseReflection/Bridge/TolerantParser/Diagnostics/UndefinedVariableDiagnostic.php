...
class UndefinedVariableDiagnostic implements Diagnostic
{
    ...
    public function message() : string
    {
        if (\count($this->suggestions) === 0) {
            return \sprintf('Undefined variable "$%s"', $this->varName);
        }
        $suggestString = \implode('", "$', $this->suggestions);
        if (\count($this->suggestions) === 1) {
            return \sprintf('Undefined variable "$%s", did you mean "$%s"', $this->varName, $suggestString);
        }
        return \sprintf('Undefined variable "$%s", did you mean one of "$%s"', $this->varName, $suggestString);
    }
    /**
     * @return list<string>
     */
