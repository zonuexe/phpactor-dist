...
class TolerantQualifiedNameResolver
{
    ...
    public static function getResolvedName($node, $namespaceDefinition = null)
    {
        ...
        if ($node->parent instanceof Node\Statement\NamespaceDefinition && $node->parent->name->getStartPosition() === $node->getStartPosition() || $node->parent instanceof Node\Statement\NamespaceUseDeclaration || $node->parent instanceof Node\NamespaceUseClause || $node->parent instanceof Node\NamespaceUseGroupClause || $node->parent instanceof Node\TraitSelectOrAliasClause || $node->parent instanceof TraitSelectOrAliasClause && ($node->parent->asOrInsteadOfKeyword == null || $node->parent->asOrInsteadOfKeyword->kind === TokenKind::AsKeyword)) {
            return null;
        }
        ...
        if (self::isConstantName($node)) {
            $resolvedName = self::tryResolveFromImportTable(
                $node,
                $constImportTable,
                /* case-sensitive */
                \true
