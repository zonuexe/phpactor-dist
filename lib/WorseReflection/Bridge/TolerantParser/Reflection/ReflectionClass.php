...
class ReflectionClass extends AbstractReflectionClass implements CoreReflectionClass
{
    ...
    public function members() : ReflectionMemberCollection
    {
        ...
        foreach ($this->hierarchy() as $reflectionClassLike) {
            ...
            $classLikeMembers = $classLikeMembers->merge($this->serviceLocator->methodProviders()->provideMembers($this->serviceLocator, $reflectionClassLike));
    ...
    public function ownMembers() : ReflectionMemberCollection
    {
        ...
        $this->ownMembers = ClassLikeReflectionMemberCollection::fromClassMemberDeclarations($this->serviceLocator, $this->node, $this);
        return $this->ownMembers;
    }
    ...
    public function parent() : ?CoreReflectionClass
    {
        ...
        try {
            ...
            $reflectedClass = $this->serviceLocator->reflector()->reflectClassLike($className, $this->visited);
            if (!$reflectedClass instanceof CoreReflectionClass) {
                $this->serviceLocator->logger()->warning(\sprintf('Class cannot extend interface. Class "%s" extends interface or trait "%s"', $this->name(), $reflectedClass->name()));
                return null;
            }
