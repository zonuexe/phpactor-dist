...
class ReflectionMethod extends \Phpactor\WorseReflection\Bridge\TolerantParser\Reflection\AbstractReflectionClassMember implements CoreReflectionMethod
{
    ...
    public function __construct(private ServiceLocator $serviceLocator, private ReflectionClassLike $class, private MethodDeclaration $node)
    {
    ...
    {
        $type = $this->typeContextualiser->contextualise($this->declaringClass(), $this->class(), $this->returnTypeResolver->resolve($this->class()));
        if ($type->isDefined()) {
            return $type;
        }
        return $this->type();
    ...
    {
        return $this->memberTypeResolver->resolve($this->node, $this->node->returnTypeList, $this->class()->name(), $this->node->questionToken ? \true : \false);
    }
