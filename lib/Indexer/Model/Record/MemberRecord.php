...
class MemberRecord implements \Phpactor\Indexer\Model\Record\HasFileReferences, Record, \Phpactor\Indexer\Model\Record\HasShortName
{
    ...
    {
        if (!\in_array($type, [self::TYPE_PROPERTY, self::TYPE_CONSTANT, self::TYPE_METHOD])) {
            throw new RuntimeException(\sprintf('Invalid member type "%s" use one of MemberType::TYPE_*', $type));
        }
        $this->type = $type;
    }
    ...
    public static function fromIdentifier(string $identifier) : self
    {
        if (!self::isIdentifier($identifier)) {
            throw new RuntimeException(\sprintf('Invalid member identifier "%s", must be <type>#<name> e.g. "property#foobar"', $identifier));
        }
