...
class IndexBuildCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output) : int
    {
        ...
        if (\is_string($subPath)) {
            $subPath = Path::join(Cast::toStringOrNull(\getcwd()), $subPath);
        }
    ...
    private function buildIndex(OutputInterface $output, ?string $subPath = null) : void
    {
        ...
        $output->write(\PHP_EOL);
        $output->write(\PHP_EOL);
        $output->writeln(\sprintf('<bg=green;fg=black;option>Done in %s seconds using %sb of memory</>', \number_format(\microtime(\true) - $start, 2), \number_format(\memory_get_usage(\true))));
    }
