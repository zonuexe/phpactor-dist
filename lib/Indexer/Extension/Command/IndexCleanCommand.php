...
use Phpactor202301\Symfony\Component\Finder\Finder;
...
, self::OPT_CLEAN_ALL));
    ...
    private function renderIndexTable(IndexInfos $indexList, OutputInterface $output) : void
    {
        ...
        foreach ($indexList as $index) {
            ...
            $table->addRow([$offset++, $index->name(), PhpactorFilesystem::formatSize($index->size()), \sprintf('%.1f days', $index->ageInDays()), \sprintf('%.1f days', $index->lastModifiedInDays())]);
        }
    ...
    {
        $finder = (new Finder())->directories()->in([$this->indexDirectory])->sortByName()->depth('==0');
