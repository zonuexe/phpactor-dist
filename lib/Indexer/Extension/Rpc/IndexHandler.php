...
class IndexHandler implements Handler
{
    ...
    {
        $resolver->setDefaults([self::PARAM_WATCH => \false, self::PARAM_INTERVAL => 5000]);
        $resolver->setTypes([self::PARAM_INTERVAL => 'integer']);
    }
    /**
    ...
    public function handle(array $arguments) : Response
    {
        ...
        $job->run();
        if ($arguments[self::PARAM_WATCH] === \true) {
        ...
        return EchoResponse::fromMessage(\sprintf('Indexed %s files', $job->size()));
    }
