...
namespace Phpactor202301\Phpactor\Indexer\Extension;

use Phpactor202301\Phpactor\AmpFsWatch\Watcher;
use Phpactor202301\Phpactor\AmpFsWatch\WatcherConfig;
use Phpactor202301\Phpactor\AmpFsWatch\Watcher\BufferedWatcher\BufferedWatcher;
...
use Phpactor202301\Phpactor\AmpFsWatch\Watcher\Watchman\WatchmanWatcher;
...
use Phpactor202301\Phpactor\Indexer\Adapter\ReferenceFinder\IndexedNameSearcher;
...
use Phpactor202301\Phpactor\Indexer\Adapter\Worse\IndexerConstantSourceLocator;
...
use Phpactor202301\Phpactor\Indexer\Extension\Command\IndexSearchCommand;
...
use Phpactor202301\Phpactor\Indexer\Extension\Command\IndexQueryCommand;
use Phpactor202301\Phpactor\Indexer\Extension\Command\IndexBuildCommand;
use Phpactor202301\Phpactor\Indexer\Extension\Command\IndexCleanCommand;
use Phpactor202301\Phpactor\Indexer\Extension\Rpc\IndexHandler;
...
class IndexerExtension implements Extension
{
    ...
    {
        $schema->setDefaults([self::PARAM_ENABLED_WATCHERS => ['inotify', 'watchman', 'find', 'php'], self::PARAM_INDEX_PATH => '%cache%/index/%project_id%', self::PARAM_INCLUDE_PATTERNS => ['/**/*.php'], self::PARAM_EXCLUDE_PATTERNS => ['/vendor/**/Tests/**/*', '/vendor/**/tests/**/*', '/vendor/composer/**/*'], self::PARAM_STUB_PATHS => [], self::PARAM_INDEXER_POLL_TIME => 5000, self::PARAM_INDEXER_BUFFER_TIME => 500, self::PARAM_INDEXER_FOLLOW_SYMLINKS => \false, self::PARAM_PROJECT_ROOT => '%project_root%', self::PARAM_REFERENCES_DEEP_REFERENCES => \true, self::PARAM_IMPLEMENTATIONS_DEEP_REFERENCES => \true]);
        $schema->setDescriptions([self::PARAM_ENABLED_WATCHERS => 'List of allowed watchers. The first watcher that supports the current system will be used', self::PARAM_INDEX_PATH => 'Path where the index should be saved', self::PARAM_STUB_PATHS => 'Paths to external folders to index. They will be indexed only once, if you want to take any changes into account you will have to reindex your project manually.', self::PARAM_INCLUDE_PATTERNS => 'Glob patterns to include while indexing', self::PARAM_EXCLUDE_PATTERNS => 'Glob patterns to exclude while indexing', self::PARAM_INDEXER_POLL_TIME => 'For polling indexers only: the time, in milliseconds, between polls (e.g. filesystem scans)', self::PARAM_INDEXER_BUFFER_TIME => 'For real-time indexers only: the time, in milliseconds, to buffer the results', self::PARAM_INDEXER_FOLLOW_SYMLINKS => 'To allow indexer to follow symlinks', self::PARAM_PROJECT_ROOT => 'The root path to use for scanning the index', self::PARAM_REFERENCES_DEEP_REFERENCES => 'Recurse over class implementations to resolve all references', self::PARAM_IMPLEMENTATIONS_DEEP_REFERENCES => 'Recurse over class implementations to resolve all class implementations (not just the classes directly implementing the subject)']);
    }
    ...
    private function registerWorseAdapters(ContainerBuilder $container) : void
    {
        ...
        }, [WorseReflectionExtension::TAG_SOURCE_LOCATOR => ['priority' => 128]]);
        ...
        }, [WorseReflectionExtension::TAG_SOURCE_LOCATOR => ['priority' => 100]]);
    }
    private function registerCommands(ContainerBuilder $container) : void
    {
        $container->register(IndexBuildCommand::class, function (Container $container) {
            return new IndexBuildCommand($container->get(Indexer::class), $container->get(Watcher::class));
        }, [ConsoleExtension::TAG_COMMAND => ['name' => 'index:build']]);
        $container->register(IndexCleanCommand::class, function (Container $container) {
            $indexPath = $container->get(FilePathResolverExtension::SERVICE_FILE_PATH_RESOLVER)->resolve($container->getParameters()['indexer.index_path']);
    ...
    private function registerModel(ContainerBuilder $container) : void
    {
        $container->register(IndexAgent::class, function (Container $container) {
            return $container->get(IndexAgentBuilder::class)->setReferenceEnhancer($container->get(WorseRecordReferenceEnhancer::class))->buildAgent();
        });
        $container->register(IndexAccess::class, function (Container $container) {
            ...
            return $container->get(IndexAgentBuilder::class)->buildAgent()->access();
        });
        ...
        $container->register(IndexAgentBuilder::class, function (Container $container) {
            ...
            $indexPath = $resolver->resolve($container->getParameter(self::PARAM_INDEX_PATH));
            return IndexAgentBuilder::create($indexPath, $this->projectRoot($container))->setExcludePatterns($container->get(self::SERVICE_INDEXER_EXCLUDE_PATTERNS))->setIncludePatterns($container->get(self::SERVICE_INDEXER_INCLUDE_PATTERNS))->setFollowSymlinks((bool) $container->getParameter(self::PARAM_INDEXER_FOLLOW_SYMLINKS))->setStubPaths($container->getParameter(self::PARAM_STUB_PATHS));
        });
        ...
        $container->register(WorseRecordReferenceEnhancer::class, function (Container $container) {
            return new WorseRecordReferenceEnhancer($container->get(WorseReflectionExtension::SERVICE_REFLECTOR), $this->logger($container));
        });
    }
    private function registerReferenceFinderAdapters(ContainerBuilder $container) : void
    {
        $container->register(IndexedImplementationFinder::class, function (Container $container) {
            return new IndexedImplementationFinder($container->get(QueryClient::class), $container->get(WorseReflectionExtension::SERVICE_REFLECTOR), $container->getParameter(self::PARAM_IMPLEMENTATIONS_DEEP_REFERENCES));
        ...
        $container->register(IndexedReferenceFinder::class, function (Container $container) {
            return new IndexedReferenceFinder($container->get(QueryClient::class), $container->get(WorseReflectionExtension::SERVICE_REFLECTOR), new ContainerTypeResolver($container->get(WorseReflectionExtension::SERVICE_REFLECTOR)), $container->getParameter(self::PARAM_REFERENCES_DEEP_REFERENCES));
        }, [ReferenceFinderExtension::TAG_REFERENCE_FINDER => []]);
        $container->register(IndexedNameSearcher::class, function (Container $container) {
            return new IndexedNameSearcher($container->get(SearchClient::class));
    ...
    private function registerRpc(ContainerBuilder $container) : void
    {
        ...
        $container->register(IndexHandler::class, function (Container $container) {
            return new IndexHandler($container->get(Indexer::class), $container->get(Watcher::class));
        }, [RpcExtension::TAG_RPC_HANDLER => ['name' => IndexHandler::NAME]]);
    }
    private function registerWatcher(ContainerBuilder $container) : void
    {
        $container->register(Watcher::class, function (Container $container) {
            ...
            foreach ($container->getServiceIdsForTag(self::TAG_WATCHER) as $serviceId => $attrs) {
                if (!isset($attrs['name'])) {
                    throw new RuntimeException(\sprintf('Watcher "%s" must provide the `name` attribute', $serviceId));
                }
                $watchers[$attrs['name']] = $serviceId;
            }
            if ($diff = \array_diff($enabledWatchers, \array_keys($watchers))) {
                throw new RuntimeException(\sprintf('Unknown watchers "%s" specified, available watchers: "%s"', \implode('", "', $diff), \implode('", "', \array_keys($watchers))));
            }
            ...
            return new PatternMatchingWatcher(new FallbackWatcher($watchers, $this->logger($container)), $container->get(self::SERVICE_INDEXER_INCLUDE_PATTERNS), $container->get(self::SERVICE_INDEXER_EXCLUDE_PATTERNS));
        });
        $container->register(WatcherConfig::class, function (Container $container) {
            ...
            return new WatcherConfig([$path->path()], $container->getParameter(self::PARAM_INDEXER_POLL_TIME));
        });
        ...
        $container->register(WatchmanWatcher::class, function (Container $container) {
            return new BufferedWatcher(new WatchmanWatcher($container->get(WatcherConfig::class), $this->logger($container)), $container->getParameter(self::PARAM_INDEXER_BUFFER_TIME));
        }, [self::TAG_WATCHER => ['name' => 'watchman']]);
        $container->register(InotifyWatcher::class, function (Container $container) {
            return new BufferedWatcher(new InotifyWatcher($container->get(WatcherConfig::class), $this->logger($container)), $container->getParameter(self::PARAM_INDEXER_BUFFER_TIME));
        }, [self::TAG_WATCHER => ['name' => 'inotify']]);
        $container->register(FsWatchWatcher::class, function (Container $container) {
            return new FsWatchWatcher($container->get(WatcherConfig::class), $this->logger($container));
        }, [self::TAG_WATCHER => ['name' => 'fswatch']]);
        $container->register(FindWatcher::class, function (Container $container) {
            return new FindWatcher($container->get(WatcherConfig::class), $this->logger($container));
        }, [self::TAG_WATCHER => ['name' => 'find']]);
        $container->register(PhpPollWatcher::class, function (Container $container) {
            return new PhpPollWatcher($container->get(WatcherConfig::class), $this->logger($container));
        }, [self::TAG_WATCHER => ['name' => 'php']]);
    }
...
\class_alias('Phpactor202301\\Phpactor\\Indexer\\Extension\\IndexerExtension', 'Phpactor\\Indexer\\Extension\\IndexerExtension', \false);
