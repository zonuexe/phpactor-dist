...
class IndexerExtension implements Extension
{
    ...
    public function configure(Resolver $schema) : void
    {
        ...
        $schema->setTypes([self::PARAM_ENABLED_WATCHERS => 'array', self::PARAM_INDEX_PATH => 'string', self::PARAM_INCLUDE_PATTERNS => 'array', self::PARAM_EXCLUDE_PATTERNS => 'array', self::PARAM_STUB_PATHS => 'array', self::PARAM_INDEXER_POLL_TIME => 'integer', self::PARAM_INDEXER_BUFFER_TIME => 'integer', self::PARAM_INDEXER_FOLLOW_SYMLINKS => 'boolean', self::PARAM_PROJECT_ROOT => 'string', self::PARAM_REFERENCES_DEEP_REFERENCES => 'boolean', self::PARAM_IMPLEMENTATIONS_DEEP_REFERENCES => 'boolean']);
    }
    ...
    private function registerCommands(ContainerBuilder $container) : void
    {
        ...
        $container->register(IndexCleanCommand::class, function (Container $container) {
            $indexPath = $container->get(FilePathResolverExtension::SERVICE_FILE_PATH_RESOLVER)->resolve($container->parameter(self::PARAM_INDEX_PATH)->string());
    ...
    private function registerModel(ContainerBuilder $container) : void
    {
        ...
        $container->register(WorseRecordReferenceEnhancer::class, function (Container $container) {
            return new WorseRecordReferenceEnhancer($container->expect(WorseReflectionExtension::SERVICE_REFLECTOR, Reflector::class), $this->logger($container), $container->has(TextDocumentLocator::class) ? $container->get(TextDocumentLocator::class) : new FilesystemTextDocumentLocator());
        });
    }
    private function registerReferenceFinderAdapters(ContainerBuilder $container) : void
    {
        $container->register(IndexedImplementationFinder::class, function (Container $container) {
            return new IndexedImplementationFinder($container->get(QueryClient::class), $container->get(WorseReflectionExtension::SERVICE_REFLECTOR), $container->parameter(self::PARAM_IMPLEMENTATIONS_DEEP_REFERENCES)->bool());
        ...
        $container->register(IndexedReferenceFinder::class, function (Container $container) {
            return new IndexedReferenceFinder($container->get(QueryClient::class), $container->get(WorseReflectionExtension::SERVICE_REFLECTOR), new ContainerTypeResolver($container->get(WorseReflectionExtension::SERVICE_REFLECTOR)), $container->parameter(self::PARAM_REFERENCES_DEEP_REFERENCES)->bool());
    ...
    private function registerWatcher(ContainerBuilder $container) : void
    {
        ...
        $container->register(WatcherConfig::class, function (Container $container) {
            ...
            return new WatcherConfig([$path->path()], $container->parameter(self::PARAM_INDEXER_POLL_TIME)->int());
        });
        ...
        $container->register(WatchmanWatcher::class, function (Container $container) {
            return new BufferedWatcher(new WatchmanWatcher($container->get(WatcherConfig::class), $this->logger($container)), $container->parameter(self::PARAM_INDEXER_BUFFER_TIME)->int());
        }, [self::TAG_WATCHER => ['name' => 'watchman']]);
        $container->register(InotifyWatcher::class, function (Container $container) {
            return new BufferedWatcher(new InotifyWatcher($container->get(WatcherConfig::class), $this->logger($container)), $container->parameter(self::PARAM_INDEXER_BUFFER_TIME)->int());
