...
use Phpactor202301\Phpactor\Indexer\Adapter\Php\InMemory\InMemorySearchIndex;
...
class ValidatingSearchIndexTest extends IntegrationTestCase
{
    ...
    protected function setUp() : void
    {
        ...
        $this->searchIndex = new ValidatingSearchIndex($this->innerSearchIndex, $this->index, new NullLogger());
    }
    ...
    {
        $record = ClassRecord::fromName('Foobar')->setFilePath($this->workspace()->path('nope.php'));
        $this->index->write($record);
        $this->innerSearchIndex->write($record);
        self::assertSearchCount(0, $this->searchIndex->search(new ShortNameBeginsWith('Foobar')));
        self::assertFalse($this->innerSearchIndex->has($record));
    ...
    public function testYieldsSearchResultIfFileExists() : void
    {
        ...
        $record = ClassRecord::fromName('Foobar')->setFilePath($this->workspace()->path('yep.php'));
        $this->index->write($record);
        $this->innerSearchIndex->write($record);
...
\class_alias('Phpactor202301\\Phpactor\\Indexer\\Tests\\Unit\\Model\\SearchIndex\\ValidatingSearchIndexTest', 'Phpactor\\Indexer\\Tests\\Unit\\Model\\SearchIndex\\ValidatingSearchIndexTest', \false);
