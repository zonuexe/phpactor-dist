...
class IndexQueryCommandTest extends IntegrationTestCase
{
    ...
    {
        $this->initProject();
        $process = new Process([__DIR__ . '/../../bin/console', 'index:query', $query], $this->workspace()->path());
        $process->mustRun();
    ...
    {
        (yield 'method' => ['method#testQueryIndex']);
        (yield 'constant' => ['constant#RECORD_TYPE']);
        (yield 'property' => ['property#workspace']);
        (yield 'class' => [__CLASS__]);
        (yield 'sprintf' => ['sprintf']);
    }
}
\class_alias('Phpactor202301\\Phpactor\\Indexer\\Tests\\Extension\\Command\\IndexQueryCommandTest', 'Phpactor\\Indexer\\Tests\\Extension\\Command\\IndexQueryCommandTest', \false);
