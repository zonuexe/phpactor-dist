...
class IndexedReferenceFinderTest extends IntegrationTestCase
{
    ...
    public function testFinder(string $manifest, int $expectedConfirmed, ?int $expectedTotal = 0) : void
    {
        ...
        $referenceFinder = new IndexedReferenceFinder($this->indexAgent()->query(), $this->createReflector());
        $locations = $referenceFinder->findReferences(TextDocumentBuilder::create($source)->build(), ByteOffset::fromInt((int) $offset));
    ...
    {
        (yield 'single class' => [<<<'EOT'
// File: project/subject.php
<?php new Fo<>o();
EOT
, 1]);
        (yield 'class references' => [<<<'EOT'
// File: project/subject.php
<?php class Fo<>o {}
...
EOT
, 2]);
        (yield 'class deep references' => [<<<'EOT'
// File: project/subject.php
<?php class Fo<>o {}
// File: project/class1.php
<?php
...
, 2]);
    ...
    {
        (yield 'single trait' => [<<<'EOT'
// File: project/subject.php
<?php class Foo { use Ba<>r; };
EOT
, 1]);
        (yield 'implementation' => [<<<'EOT'
// File: project/subject.php
<?php class Foo { use Ba<>r; };
...
EOT
, 3]);
        (yield 'namespaced static members' => [<<<'EOT'
// File: project/subject.php
...
\class_alias('Phpactor202301\\Phpactor\\Indexer\\Tests\\Adapter\\ReferenceFinder\\IndexedReferenceFinderTest', 'Phpactor\\Indexer\\Tests\\Adapter\\ReferenceFinder\\IndexedReferenceFinderTest', \false);
