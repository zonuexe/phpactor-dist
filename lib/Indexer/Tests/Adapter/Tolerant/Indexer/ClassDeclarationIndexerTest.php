...
class ClassDeclarationIndexerTest extends TolerantIndexerTestCase
{
    ...
    public function testImplementations(string $manifest, string $fqn, int $expectedCount) : void
    {
        ...
        $agent = $this->indexAgentBuilder('src')->setIndexers([new ClassDeclarationIndexer()])->buildAgent();
    ...
    {
        (yield 'no implementations' => ["// File: src/file1.php\n<?php class Barfoo {}", 'Foobar', 0]);
        (yield 'class implements' => ["// File: src/file1.php\n<?php class Barfoo implements Foobar{}", 'Foobar', 1]);
        (yield 'class implements multiple' => ["// File: src/file1.php\n<?php class Barfoo implements Baz, Foobar{}", 'Foobar', 1]);
        (yield 'abstract class implements' => ["// File: src/file1.php\n<?php abstract class Barfoo implements Foobar{}", 'Foobar', 1]);
    }
    /**
    ...
    public function testSearch(string $manifest, string $search, array $expectedRecords) : void
    {
        ...
        foreach ($expectedRecords as $record) {
            ...
            throw new RuntimeException(\sprintf('Record "%s" not found', $record->identifier()));
        }
    ...
    {
        (yield 'no results' => ["// File: src/file1.php\n<?php class Barfoo {}", 'Foobar', []]);
        (yield 'exact match' => ["// File: src/file1.php\n<?php class Barfoo implements Foobar{}", 'Barfoo', [ClassRecord::fromName('Barfoo')->setFilePath($this->workspace()->path('src/file1.php'))]]);
        (yield 'namespaced match' => ["// File: src/file1.php\n<?php namespace Bar; class Barfoo implements Foobar{}", 'Barfoo', [ClassRecord::fromName('Phpactor202301\\Bar\\Barfoo')->setFilePath($this->workspace()->path('src/file1.php'))]]);
    }
    /**
    ...
    public function testInvalidClass(string $manifest, string $exectedMessage) : void
    {
        ...
        $agent = $this->indexAgentBuilder('src')->setIndexers([new ClassDeclarationIndexer()])->setLogger($logger->reveal())->buildAgent();
        $agent->indexer()->getJob()->run();
        $this->addToAssertionCount(1);
    }
    /**
    ...
    {
        (yield 'no class name' => ["// File: src/file1.php\n<?php class {}", 'Name is empty']);
    }
}
\class_alias('Phpactor202301\\Phpactor\\Indexer\\Tests\\Adapter\\Tolerant\\Indexer\\ClassDeclarationIndexerTest', 'Phpactor\\Indexer\\Tests\\Adapter\\Tolerant\\Indexer\\ClassDeclarationIndexerTest', \false);
