...
class FileSearchIndex implements SearchIndex
{
    ...
    {
        $this->open();
        $info = [$record->recordType(), $record->identifier(), $record instanceof ClassRecord ? $record->type() : null, $record instanceof HasFlags ? $record->flags() : null];
    ...
    public function flush() : void
    {
        ...
        try {
            ...
            \mkdir(\dirname($this->path), 0777, \true);
            file_put_contents($this->path, $content);
        }
        $this->dirty = \false;
    }
