...
class FileRepository
{
    ...
    public function __construct(private string $path, private RecordSerializer $serializer, ?LoggerInterface $logger = null)
    {
    ...
    public function get(Record $record) : ?Record
    {
        ...
        try {
            ...
            $this->logger->warning(\sprintf('Record at path "%s" is corrupted, removing: %s', $path, $corrupted->getMessage()));
            $this->remove($record);
            return null;
        }
        ...
        if (!$deserialized instanceof $record) {
            $this->logger->warning(\sprintf('Invalid cache entry file: "%s", got instance of "%s"', $path, \get_class($deserialized)));
            return null;
        }
        return $deserialized;
    }
    ...
    public function remove(Record $record) : void
    {
        ...
        $this->logger->warning(\sprintf('Could not remove index file "%s"', $path));
    }
    ...
    {
        $this->lastUpdate = \file_exists($this->timestampPath()) ? (int) file_get_contents($this->timestampPath()) : 0;
    }
    ...
    private function pathFor(Record $record) : string
    {
        ...
        return \sprintf('%s/%s_%s/%s/%s.cache', $this->path, $record->recordType(), \substr($hash, 0, 1), \substr($hash, 1, 1), $hash);
    }
