...
class IndexerClassSourceLocator implements SourceCodeLocator
{
    ...
    public function locate(Name $name) : SourceCode
    {
        ...
        if (null === $filePath || !\file_exists($filePath)) {
            throw new SourceNotFound(\sprintf('Class "%s" is indexed, but it does not exist at path "%s"!', $name->full(), $filePath));
        }
        return SourceCode::fromPath($filePath);
    }
}
\class_alias('Phpactor202301\\Phpactor\\Indexer\\Adapter\\Worse\\IndexerClassSourceLocator', 'Phpactor\\Indexer\\Adapter\\Worse\\IndexerClassSourceLocator', \false);
