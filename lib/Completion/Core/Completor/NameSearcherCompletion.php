...
final class NameSearcherCompletion
{
    ...
    protected function completeName(string $name, ?TextDocumentUri $sourceUri = null, ?Node $node = null) : Generator
    {
        ...
        foreach ($this->nameSearcher->search($name) as $result) {
            ...
            (yield $this->createSuggestion($name, $result, $wasQualified, $node, $this->createSuggestionOptions($result, $sourceUri, $node, $wasQualified)));
        }
        return \true;
    }
    /**
    ...
    {
        $options = ['short_description' => $result->name()->__toString(), 'type' => $this->suggestionType($result), 'class_import' => null, 'name_import' => null, 'priority' => $this->prioritizer->priority($result->uri(), $sourceUri)];
        // needed?
    ...
    private function suggestChildSegment(&$visitedSegments, string $search, NameSearchResult $result, TextDocumentUri $sourceUri, string $segment) : Generator
    {
        ...
        $visitedSegments[$segment] = \true;
        (yield Suggestion::createWithOptions($segment, ['short_description' => NameUtil::join(NameUtil::relativeToSearch($result->name()->__toString(), $search), $segment), 'type' => Suggestion::TYPE_MODULE, 'priority' => $this->prioritizer->priority($result->uri(), $sourceUri)]));
    }
}
