...
class Suggestion
{
    ...
    {
        $defaults = ['short_description' => '', 'documentation' => '', 'type' => null, 'class_import' => null, 'name_import' => null, 'label' => null, 'range' => null, 'snippet' => null, 'priority' => null];
        if ($diff = \array_diff(\array_keys($options), \array_keys($defaults))) {
            throw new RuntimeException(\sprintf('Invalid options for suggestion: "%s" valid options: "%s"', \implode('", "', $diff), \implode('", "', \array_keys($defaults))));
        }
        $options = \array_merge($defaults, $options);
        return new self($name, $options['type'], $options['short_description'], $options['name_import'] ? $options['name_import'] : $options['class_import'], $options['label'], $options['documentation'], $options['range'], $options['snippet'], $options['priority']);
    }
    /**
