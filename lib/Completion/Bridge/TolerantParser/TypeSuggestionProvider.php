...
class TypeSuggestionProvider
{
    ...
    private function nameResults(string $search) : Generator
    {
        ...
        foreach ($this->nameSearcher->search($search) as $result) {
            ...
            (yield Suggestion::createWithOptions($result->name()->head(), ['short_description' => $result->name()->__toString(), 'name_import' => $wasAbsolute ? null : $result->name()->__toString(), 'type' => Suggestion::TYPE_CLASS, 'priority' => Suggestion::PRIORITY_MEDIUM]));
        }
    }
    /**
     * @return Generator<Suggestion>
    ...
    private function nameImports(Node $node) : Generator
    {
        ...
        foreach ($namespaceImports as $alias => $resolvedName) {
            (yield Suggestion::createWithOptions($alias, ['type' => Suggestion::TYPE_CLASS, 'short_description' => \sprintf('%s', $resolvedName->__toString()), 'priority' => Suggestion::PRIORITY_HIGH]));
        }
    }
    /**
     * @return Generator<Suggestion>
    ...
    private function builtInTypes() : Generator
    {
        foreach (self::BUILT_IN_TYPES as $type) {
            (yield Suggestion::createWithOptions($type, ['type' => Suggestion::TYPE_KEYWORD, 'priority' => Suggestion::PRIORITY_HIGH]));
        }
    }
