    ...
    private function doSignatureHelp(TextDocument $textDocument, ByteOffset $offset) : SignatureHelp
    {
        ...
        if ($callable instanceof MemberAccessExpression) {
            ...
            if ($symbolContext->symbol()->symbolType() !== Symbol::METHOD) {
                throw new CouldNotHelpWithSignature(\sprintf('Could not provide signature member type "%s"', $symbolContext->symbol()->symbolType()));
            }
            $containerType = $symbolContext->containerType()->expandTypes()->classLike()->firstOrNull();
            if (!$containerType instanceof ClassType) {
                throw new CouldNotHelpWithSignature(\sprintf('Container type is not a class: "%s"', $symbolContext->symbol()->name()));
            }
