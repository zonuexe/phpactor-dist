...
class WorseSignatureHelper implements SignatureHelper
{
    ...
    public function __construct(private Reflector $reflector, private ObjectFormatter $formatter, ?Parser $parser = null)
    {
    ...
    private function doSignatureHelp(TextDocument $textDocument, ByteOffset $offset) : SignatureHelp
    {
        ...
        if ($callable instanceof MemberAccessExpression) {
            ...
            if ($nodeContext->symbol()->symbolType() !== Symbol::METHOD) {
                throw new CouldNotHelpWithSignature(\sprintf('Could not provide signature member type "%s"', $nodeContext->symbol()->symbolType()));
            }
            $containerType = $nodeContext->containerType()->expandTypes()->classLike()->firstOrNull();
            if (!$containerType instanceof ClassType) {
                throw new CouldNotHelpWithSignature(\sprintf('Container type is not a class: "%s"', $nodeContext->symbol()->name()));
            }
    ...
    private function signatureHelperForObjectCreation(ObjectCreationExpression $node, int $position) : SignatureHelp
    {
        ...
        if (!$name instanceof QualifiedName) {
            throw new CouldNotHelpWithSignature(\sprintf('Only provide help for qualified names, got "%s"', \get_class($name)));
        }
        $offset = $this->reflector->reflectOffset(NodeToTextDocumentConverter::convert($node), $name->getStartPosition());
    ...
    {
        $callNode = $nodeAtPosition;
        if (($nodeAtPosition instanceof CallExpression || $nodeAtPosition instanceof ObjectCreationExpression) && null === $nodeAtPosition->argumentExpressionList && null !== $nodeAtPosition->openParen && $nodeAtPosition->openParen->getEndPosition() == $offset->toInt()) {
