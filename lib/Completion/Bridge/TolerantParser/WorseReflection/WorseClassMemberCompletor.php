...
{
    public function __construct(private Reflector $reflector, private ObjectFormatter $formatter, private ObjectFormatter $snippetFormatter, private ObjectRenderer $objectRenderer)
    {
    }
    ...
    private function populateSuggestions(NodeContext $nodeContext, Type $type, bool $static, bool $completeOnlyName, bool $isInstance) : Generator
    {
        ...
        if (!$isParent && $static) {
            (yield Suggestion::createWithOptions('class', ['type' => Suggestion::TYPE_CONSTANT, 'short_description' => $type->name(), 'priority' => Suggestion::PRIORITY_HIGH]));
        }
        try {
        ...
        foreach ($members->methods() as $method) {
            ...
            (yield Suggestion::createWithOptions($method->name(), ['type' => Suggestion::TYPE_METHOD, 'short_description' => fn() => $this->formatter->format($method), 'documentation' => function () use($method) {
                return $this->objectRenderer->render(new ItemDocumentation(\sprintf('%s::%s', $method->class()->name(), $method->name()), $method->docblock()->formatted(), $method));
        ...
        if ($classReflection instanceof ReflectionClass) {
            ...
            foreach ($members->properties() as $property) {
                ...
                (yield Suggestion::createWithOptions($name, ['type' => Suggestion::TYPE_PROPERTY, 'short_description' => fn() => $this->formatter->format($property), 'documentation' => function () use($property) {
                    return $this->objectRenderer->render(new ItemDocumentation(\sprintf('%s::%s', $property->class()->name(), $property->name()), $property->docblock()->formatted(), $property));
                }]));
            }
        }
        if (\false === $isInstance && $classReflection instanceof ReflectionClass || $classReflection instanceof ReflectionInterface) {
            foreach ($members->constants() as $constant) {
                if ($publicOnly && \false === $constant->visibility()->isPublic()) {
                    continue;
                }
                (yield Suggestion::createWithOptions($constant->name(), ['type' => Suggestion::TYPE_CONSTANT, 'short_description' => fn() => $this->formatter->format($constant), 'documentation' => fn() => $constant->docblock()->formatted()]));
            }
        }
        if ($classReflection instanceof ReflectionEnum) {
            foreach ($members->enumCases() as $case) {
                (yield Suggestion::createWithOptions($case->name(), ['type' => Suggestion::TYPE_ENUM, 'short_description' => fn() => $this->formatter->format($case), 'documentation' => fn() => $case->docblock()->formatted()]));
            }
        }
    }
}
