...
class DocblockCompletor implements TolerantCompletor
{
    ...
    const TAGS_WITH_VAR = ['@param'];
    const TAGS_WITH_TYPE_ARG = ['@param', '@var', '@return', '@method', '@property', '@implements', '@extends'];
    public function __construct(private TypeSuggestionProvider $typeSuggestionProvider, private Parser $parser)
    {
    }
    /**
    ...
    public function complete(Node $node, TextDocument $source, ByteOffset $byteOffset) : Generator
    {
        ...
        foreach (self::SUPPORTED_TAGS as $supportedTag) {
            if (\str_starts_with($supportedTag, $tag)) {
                (yield Suggestion::createWithOptions($supportedTag, ['type' => Suggestion::TYPE_KEYWORD]));
            }
        }
        return \true;
    }
    ...
    private function varCompletion(Node $node, ByteOffset $offset, string $tag, string $var) : Generator
    {
        ...
        foreach ($node->parameters->getElements() as $parameter) {
            ...
            (yield Suggestion::createWithOptions('$' . $parameter->getName(), ['type' => Suggestion::TYPE_VARIABLE]));
        }
    }
}
