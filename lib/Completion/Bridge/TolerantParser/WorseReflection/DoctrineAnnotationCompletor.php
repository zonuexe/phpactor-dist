...
class DoctrineAnnotationCompletor extends NameSearcherCompletor implements Completor
{
    ...
    public function __construct(NameSearcher $nameSearcher, private Reflector $reflector, Parser $parser = null)
    {
    ...
    {
        return \array_merge(parent::createSuggestionOptions($result, null, $node, $wasAbsolute), ['snippet' => (string) $result->name()->head() . '($1)$0']);
    }
    ...
    private function findNodeForPhpdocAtPosition(SourceFileNode $sourceNodeFile, int $position) : ?Node
    {
        ...
        foreach ($sourceNodeFile->getDescendantNodes() as $node) {
            if ($node->getFullStartPosition() < $position && $position < $node->getStartPosition()) {
