...
abstract class AbstractParameterCompletor
{
    ...
    public function __construct(protected Reflector $reflector, private ObjectFormatter $formatter, VariableCompletionHelper $variableCompletionHelper = null)
    {
    ...
    protected function populateResponse(Node $callableExpression, ReflectionFunctionLike $functionLikeReflection, array $variables) : Generator
    {
        ...
        foreach ($variables as $variable) {
            if ($variable->type()->isDefined() && \false === $this->isVariableValidForParameter($variable, $parameter)) {
                // parameter has no types and is not valid for this position, ignore it
                continue;
            }
            (yield Suggestion::createWithOptions('$' . $variable->name(), ['type' => Suggestion::TYPE_VARIABLE, 'priority' => Suggestion::PRIORITY_HIGH, 'short_description' => \sprintf('%s => param #%d %s', $this->formatter->format($variable->type()), $paramIndex, $this->formatter->format($parameter))]));
        }
        return \true;
    }
    private function paramIndex(Node $node) : int
    {
        ...
        if (\str_ends_with(\trim($argumentList->getText()), ',')) {
            return $index + 1;
        }
        return $index;
    }
