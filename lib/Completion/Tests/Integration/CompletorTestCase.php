...
abstract class CompletorTestCase extends IntegrationTestCase
{
    ...
    public function assertCouldNotComplete(string $source) : void
    {
        ...
        $suggestions = $completor->complete(TextDocumentBuilder::create($source)->language('php')->uri('file:///tmp/test')->build(), ByteOffset::fromInt($offset));
        $array = \iterator_to_array($suggestions);
        $this->assertEmpty($array);
        $this->assertTrue($suggestions->getReturn());
    }
    protected abstract function createCompletor(string $source) : Completor;
    protected function assertComplete(string $source, array $expected, bool $isComplete = \true) : void
    {
        [$source, $offset] = ExtractOffset::fromSource($source);
        $completor = $this->createCompletor($source);
        $suggestionGenerator = $completor->complete(TextDocumentBuilder::create($source)->language('php')->uri('file:///tmp/test')->build(), ByteOffset::fromInt((int) $offset));
...
\class_alias('Phpactor202301\\Phpactor\\Completion\\Tests\\Integration\\CompletorTestCase', 'Phpactor\\Completion\\Tests\\Integration\\CompletorTestCase', \false);
