...
class ParserDocblockUpdater implements DocBlockUpdater
{
    ...
    private function edits(Docblock $docblock, TagPrototype $prototype) : array
    {
        if ($prototype instanceof ReturnTagPrototype) {
            return $this->updateTag($docblock, $prototype, \sprintf('@return %s', $prototype->type->__toString()));
        }
        if ($prototype instanceof ParamTagPrototype) {
            return $this->updateTag($docblock, $prototype, \sprintf('@param %s $%s', $prototype->type->__toString(), $prototype->name));
        }
        if ($prototype instanceof ExtendsTagPrototype) {
            return $this->updateTag($docblock, $prototype, \sprintf('@extends %s', $prototype->type->short()), 0);
        }
        if ($prototype instanceof ImplementsTagPrototype) {
            return $this->updateTag($docblock, $prototype, \sprintf('@implements %s', $prototype->type->short()), 0);
        }
        throw new RuntimeException(\sprintf('Do not know how to update tag "%s"', \get_class($prototype)));
    }
    /**
     * @return array<int,TextEdit>
     */
    ...
    {
        // create
        if (\strlen(\trim($docblock->toString())) === 0) {
            $indent = $this->textFormat->indent('', $indent);
            return [TextEdit::create($docblock->start(), 0, \sprintf("\n%s/**\n%s * %s\n%s */\n%s", $indent, $indent, $tagText, $indent, $indent))];
        }
        // update
        $edits = [];
        foreach ($docblock->tags() as $tag) {
            if ($prototype->matches($tag)) {
                $edits[] = TextEdit::create($tag->start(), $prototype->endOffsetFor($tag) - $tag->start(), $tagText);
            }
        }
        if ($edits) {
            return $edits;
        ...
        if ($line = $docblock->lastMultilineContentToken()) {
            return [TextEdit::create($line->end(), 0, \sprintf("* %s\n%s", $tagText, \str_repeat(' ', $docblock->indentationLevel())))];
        }
        if ($open = $docblock->phpDocOpen()) {
            if (!\str_contains($docblock->toString(), "\n")) {
                return [TextEdit::create($open->end(), 0, \sprintf(' %s', $tagText))];
            }
            return [TextEdit::create($open->end(), 0, \sprintf("\n%s* %s", \str_repeat(' ', $docblock->indentationLevel()), $tagText))];
        }
        return [];
    }
}
