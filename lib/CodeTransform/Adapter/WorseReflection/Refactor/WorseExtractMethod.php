...
class WorseExtractMethod implements ExtractMethod
{
    ...
    public function canExtractMethod(SourceCode $source, int $offsetStart, int $offsetEnd) : bool
    {
        ...
        if ($endNode instanceof CompoundStatementNode && $endNode->openBrace->getEndPosition() < $offsetEnd && $endNode->closeBrace->getStartPosition() >= $offsetEnd && \count($endNode->statements) > 0) {
        ...
        if ($startNode instanceof CompoundStatementNode && $startNode->openBrace->getEndPosition() <= $offsetStart && $startNode->closeBrace->getStartPosition() > $offsetStart && \count($startNode->statements) > 0) {
            $stmt = \current($startNode->statements);
            if (!$stmt instanceof Node) {
                return \false;
            }
            while ($stmt && $stmt->getStartPosition() < $offsetStart) {
                $stmt = \next($startNode->statements);
            }
    ...
    public function extractMethod(SourceCode $source, int $offsetStart, int $offsetEnd, string $name) : TextDocumentEdits
    {
        ...
        return new TextDocumentEdits(TextDocumentUri::fromString($source->path()), $this->updater->textEditsFor($prototype, Code::fromString((string) $source))->add(TextEdit::create($offsetStart, $offsetEnd - $offsetStart, $replacement)));
    }
    ...
    private function returnVariables(Assignments $locals, ReflectionMethod $reflectionMethod, string $source, int $offsetStart, int $offsetEnd) : array
    {
        ...
        $tailDependencies = $this->variableNames($tail = \mb_substr($source, $offsetEnd, $reflectionMethod->position()->end() - $offsetEnd));
        $returnVariables = [];
        foreach ($tailDependencies as $variable) {
            $variables = $locals->byName($variable)->assignmentsOnly()->lessThanOrEqualTo($offsetEnd)->greaterThanOrEqualTo($offsetStart);
            if ($variables->count()) {
    ...
    {
        return $this->reflector->reflectOffset((string) $source, $offsetEnd)->frame()->locals();
    }
    ...
    {
        foreach ($node->getDescendantNodes(function (Node $n) {
...
\class_alias('Phpactor202301\\Phpactor\\CodeTransform\\Adapter\\WorseReflection\\Refactor\\WorseExtractMethod', 'Phpactor\\CodeTransform\\Adapter\\WorseReflection\\Refactor\\WorseExtractMethod', \false);
