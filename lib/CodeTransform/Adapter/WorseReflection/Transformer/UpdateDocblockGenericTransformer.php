        ...
        return call(function () use($code) {
            ...
            foreach ($diagnostics as $diagnostic) {
                ...
                $tag = match ($diagnostic->isExtends()) {
                    \true => new ExtendsTagPrototype($diagnostic->missingGenericType()),
                    \false => new ImplementsTagPrototype($diagnostic->missingGenericType()),
                };
                $classBuilder->docblock($this->docblockUpdater->set($classBuilder->getDocblock() ? $classBuilder->getDocblock()->__toString() : $class->docblock()->raw(), $tag));
            }
    ...
    public function diagnostics(SourceCode $code) : Promise
    {
        return call(function () use($code) {
            ...
            foreach ($missings as $missing) {
                $diagnostics[] = new Diagnostic($missing->range(), $missing->message(), Diagnostic::WARNING);
            }
