    ...
    public function diagnostics(SourceCode $source) : Promise
    {
        return new Success((function () use($source) {
            ...
            foreach ($classes->concrete() as $class) {
                ...
                $diagnostics[] = new Diagnostic(ByteOffsetRange::fromInts($class->position()->start()->toInt(), $class->position()->start()->toInt() + 5 + \strlen($class->name()->__toString())), \sprintf('Missing methods "%s"', \implode('", "', \array_map(function (ReflectionMethod $method) {
                    return $method->name();
