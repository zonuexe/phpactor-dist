...
{
    public function __construct(private Reflector $reflector, private Updater $updater, private string $visibility, private bool $promote = \false)
    {
    }
    /**
    ...
    public function diagnostics(SourceCode $source) : Promise
    {
        ...
        foreach ($this->candidateClasses($source) as $class) {
            ...
            foreach ($constructMethod->parameters()->notPromoted() as $parameter) {
                ...
                $diagnostics[] = new Diagnostic(ByteOffsetRange::fromInts($parameter->position()->start()->toInt(), $parameter->position()->end()->toInt() + 5 + \strlen($class->name()->__toString())), \sprintf('Parameter "%s" may not have been assigned', $parameter->name()), Diagnostic::WARNING);
            }
        }
