...
class TolerantImportName implements ImportName
{
    ...
    public function __construct(private Updater $updater, Parser $parser = null, private bool $importGlobals = \false)
    {
    ...
    private function assertNotAlreadyImported(Node $node, NameImport $nameImport) : void
    {
        ...
        if (null === $nameImport->alias() && $existingImport !== null) {
            throw new NameAlreadyImportedException($nameImport, $existingName, $existingImport->getFullyQualifiedNameText());
        }
    ...
    private function updateReferences(Node $node, NameImport $nameImport, TextEdits $edits) : TextEdits
    {
        ...
        return $edits->add(TextEdit::create($node->getStartPosition(), $node->getEndPosition() - $node->getStartPosition(), $alias));
    }
