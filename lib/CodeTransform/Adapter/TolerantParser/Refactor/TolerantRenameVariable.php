...
class TolerantRenameVariable implements RenameVariable
{
    ...
    private function variableNodeFromSource(SourceFileNode $sourceNode, int $offset) : Node
    {
        ...
        if (\false === $node instanceof Variable && \false === $node instanceof UseVariableName && \false === $node instanceof Parameter) {
            throw new TransformException(\sprintf('Expected Variable or Parameter node, got "%s"', \get_class($node)));
        }
        return $node;
    }
    ...
    private function scopeNode(Node $variable, string $scope) : Node
    {
        ...
        if (null === $scopeNode) {
            throw new TransformException('Could not determine scope node, this should not happen as ' . 'there should always be a SourceFileNode.');
        }
        return $scopeNode;
    }
    ...
    {
        if (\false === $node instanceof UseVariableName && \false === $node instanceof Variable && \false === $node instanceof Parameter) {
            return null;
        }
        ...
        if ($node instanceof Variable || $node instanceof UseVariableName) {
            return TextEdit::create($node->getStartPosition(), $node->getEndPosition() - $node->getStartPosition(), '$' . $newName);
        }
        if ($node instanceof Parameter) {
            /** @var Parameter $node */
            return TextEdit::create($node->variableName->getStartPosition(), $node->variableName->getEndPosition() - $node->variableName->getStartPosition(), '$' . $newName);
        }
    }
}
