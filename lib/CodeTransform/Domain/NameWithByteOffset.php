...
final class NameWithByteOffset
{
    ...
    private const VALID_TYPES = [self::TYPE_CLASS, self::TYPE_FUNCTION];
    private string $type;
    public function __construct(private Name $name, private ByteOffset $byteOffset, string $type = self::TYPE_CLASS)
    {
        if (!\in_array($type, self::VALID_TYPES)) {
            throw new RuntimeException(\sprintf('Invalid type "%s", valid types "%s"', $type, \implode('", "', self::VALID_TYPES)));
        }
        $this->type = $type;
    }
