...
abstract class AbstractTolerantImportNameTest extends TolerantTestCase
{
    ...
    public abstract function provideImportClass() : Generator;
    ...
    {
        try {
            $this->importName('<?php namespace Foo; use Foo1\\Bar; use Foo2\\Bar as Foo2Bar;', 55, NameImport::forClass('Phpactor202301\\Foo2\\Bar'));
            ...
            self::assertSame('Phpactor202301\\Foo2\\Bar', $error->existingFQN());
        }
    }
    public function testThrowsNameAlreadyImportedNameInUse() : void
    {
        try {
            $this->importName('<?php namespace Foo; use Foo1\\Bar;', 34, NameImport::forClass('Phpactor202301\\Foo2\\Bar'));
            self::fail('Expected NameAlreadyImportedException has not been raised');
        } catch (NameAlreadyImportedException $error) {
            self::assertSame('Class "Bar" is already imported', $error->getMessage());
            self::assertSame('Bar', $error->name());
            self::assertSame('Bar', $error->existingName());
            self::assertSame('Phpactor202301\\Foo1\\Bar', $error->existingFQN());
        }
    }
    public function testThrowsNameAlreadyImportedOnlyAliasName() : void
    {
        try {
            $this->importName('<?php namespace Foo; use Foo2\\Bar as Foo2Bar;', 45, NameImport::forClass('Phpactor202301\\Foo2\\Bar'));
            self::fail('Expected NameAlreadyImportedException has not been raised');
        } catch (NameAlreadyImportedException $error) {
            self::assertSame('Class "Bar" is already imported', $error->getMessage());
            self::assertSame('Bar', $error->name());
    ...
    {
        try {
            $this->importName('<?php use function in_array;', 55, NameImport::forFunction('in_array'));
            self::fail('Expected NameAlreadyImportedException has not been raised');
        } catch (NameAlreadyImportedException $error) {
    ...
    {
        try {
            $this->importName('<?php use function in_array as foo_in_array;', 55, NameImport::forFunction('in_array'));
            self::fail('Expected NameAlreadyImportedException has not been raised');
        } catch (NameAlreadyImportedException $error) {
            self::assertSame('Function "in_array" is already imported', $error->getMessage());
            self::assertSame('in_array', $error->name());
    ...
    {
        try {
            $this->importName('<?php namespace Barfoo; class Foobar extends Foobar', 47, NameImport::forClass('Phpactor202301\\BazBar\\Foobar'));
            self::fail('Expected NameAlreadyImportedException has not been raised');
        } catch (NameAlreadyImportedException $error) {
            ...
            self::assertSame('Phpactor202301\\Barfoo\\Foobar', $error->existingFQN());
        }
    }
    public function testThrowsExceptionIfImportedClassHasSameNameAsCurrentInterfaceName() : void
    {
        $this->expectException(NameAlreadyImportedException::class);
        $this->importName('<?php namespace Barfoo; interface Foobar extends Foobar', 50, NameImport::forClass('Phpactor202301\\BazBar\\Foobar'));
    }
...
EOT;
        $this->importName($source, 64, NameImport::forClass('Phpactor202301\\Barfoo\\Barfoo'));
    }
    /**
    ...
    public abstract function provideImportFunction() : Generator;
    protected abstract function importName(string $source, int $offset, NameImport $nameImport, bool $importGlobals = \true) : TextEdits;
...
\class_alias('Phpactor202301\\Phpactor\\CodeTransform\\Tests\\Adapter\\TolerantParser\\Refactor\\AbstractTolerantImportNameTest', 'Phpactor\\CodeTransform\\Tests\\Adapter\\TolerantParser\\Refactor\\AbstractTolerantImportNameTest', \false);
