...
namespace Phpactor202301\Phpactor\CodeTransform\Tests\Adapter\TolerantParser\ClassToFile\Transformer;
...
class ClassNameFixerTransformerTest extends AdapterTestCase
{
    ...
    public function testFixClassName(string $filePath, string $test, int $diagnosticCount) : void
    {
        ...
        $source = SourceCode::fromStringAndPath($workspace->getContents($filePath), $this->workspace()->path($filePath));
        ...
        $this->assertEquals(\trim($expected), \trim($transformed->apply($source)));
    }
    public function provideFixClassName()
    {
        (yield 'no op' => ['FileOne.php', 'fixNamespace0.test', 0]);
        (yield 'fix file with missing namespace' => ['PathTo/FileOne.php', 'fixNamespace1.test', 1]);
        (yield 'fix file with namespace' => ['PathTo/FileOne.php', 'fixNamespace2.test', 1]);
        (yield 'fix class name' => ['FileOne.php', 'fixNamespace3.test', 1]);
        (yield 'fix class name with same line bracket' => ['FileOne.php', 'fixNamespace4.test', 1]);
        (yield 'fix class name and namespace' => ['Phpactor/Test/Foobar/FileOne.php', 'fixNamespace5.test', 2]);
    }
...
namespace Phpactor202301\PathTo;
...
\class_alias('Phpactor202301\\Phpactor\\CodeTransform\\Tests\\Adapter\\TolerantParser\\ClassToFile\\Transformer\\ClassNameFixerTransformerTest', 'Phpactor\\CodeTransform\\Tests\\Adapter\\TolerantParser\\ClassToFile\\Transformer\\ClassNameFixerTransformerTest', \false);
