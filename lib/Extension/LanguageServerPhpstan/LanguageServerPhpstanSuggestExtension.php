    ...
    public function load(ContainerBuilder $container) : void
    {
        $container->register('language_server_phpstan.suggest', function (Container $container) {
            return new PhpactorComposerSuggestor($container->expect(ConfigurationExtension::SERVICE_PHPACTOR_CONFIG_LOCAL, JsonConfig::class), $container->get(ComposerInspector::class), function (JsonConfig $config, ComposerInspector $inspector) {
                if ($config->has(\Phpactor\Extension\LanguageServerPhpstan\LanguageServerPhpstanExtension::PARAM_ENABLED)) {
                    return Changes::none();
                }
                if (!$inspector->package('phpstan/phpstan')) {
                    return Changes::none();
                }
                return Changes::from([new PhpactorConfigChange('Phpstan detected, enable PHPStan extension?', function (bool $enable) {
                    return [\Phpactor\Extension\LanguageServerPhpstan\LanguageServerPhpstanExtension::PARAM_ENABLED => $enable];
                })]);
            });
