    ...
    public function parse(string $jsonString) : array
    {
        ...
        foreach ($decoded['files'] ?? [] as $fileDiagnostics) {
            foreach ($fileDiagnostics['messages'] as $message) {
                ...
                $diagnostics[] = new Diagnostic(message: $message['message'], range: new Range(new Position($lineNo, 1), new Position($lineNo, 100)), severity: DiagnosticSeverity::ERROR, source: 'phpstan');
            }
        }
        return $diagnostics;
    }
