...
class ClassToFileExtension implements Extension
{
    ...
    {
        $schema->setDefaults([self::PARAM_PROJECT_ROOT => '%project_root%', self::PARAM_BRUTE_FORCE_CONVERSION => \true]);
        $schema->setDescriptions([self::PARAM_PROJECT_ROOT => 'Root path of the project (e.g. where composer.json is)', self::PARAM_BRUTE_FORCE_CONVERSION => 'If composer not found, fallback to scanning all files (very time consuming depending on project size)']);
    }
    public function load(ContainerBuilder $container) : void
    {
        $container->register(self::SERVICE_CONVERTER, function (Container $container) {
            return new ClassToFileFileToClass($container->get('class_to_file.class_to_file'), $container->get('class_to_file.file_to_class'));
        });
