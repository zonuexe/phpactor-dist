...
{
    public function __construct(private RequestHandler $handler, private string $replayPath, private bool $storeReplay = \false, private $inputStream = \STDIN)
    {
        parent::__construct();
    }
    ...
    public function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $request = \json_decode($stdin, \true);
        if (null === $request) {
            throw new InvalidArgumentException(\sprintf('Could not decode JSON: %s', $stdin));
        }
        ...
        $output->write((string) \json_encode(['version' => RpcVersion::asString(), 'action' => $response->name(), 'parameters' => $response->parameters()], $flags), \false, OutputInterface::OUTPUT_RAW);
        return 0;
    }
    ...
    private function resolveInput(bool $replay) : string
    {
        if ($replay) {
            if (\false === $this->storeReplay) {
                throw new RuntimeException('You must explicitly enable replay, set `rpc.store_replay` to `true` in your config.');
            }
    ...
    private function lastRequest()
    {
        ...
        if (\false === \file_exists($path)) {
            throw new RuntimeException(\sprintf('Replace file does not exist at "%s"', $path));
        }
