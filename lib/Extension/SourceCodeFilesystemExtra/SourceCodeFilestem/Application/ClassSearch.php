...
class ClassSearch
{
    public function __construct(private FilesystemRegistry $filesystemRegistry, private FileToClass $fileToClass, private Reflector $reflector)
    {
    }
    public function classSearch(string $filesystemName, string $name)
    {
        ...
        foreach ($files as $file) {
            ...
            $result = ['file_path' => (string) $file->path(), 'class' => null, 'class_name' => null, 'class_namespace' => null];
    ...
    {
        $declared = \array_merge(\get_declared_classes(), \get_declared_traits(), \get_declared_interfaces());
