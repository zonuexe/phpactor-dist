...
namespace Phpactor202301\Phpactor\Extension\LanguageServer\Tests\ridge\Converter;
...
class LocationConverterTest extends IntegrationTestCase
{
    ...
    public function testConvertsPhpactorLocationsToLspLocations() : void
    {
        ...
        $locations = new Locations([Location::fromPathAndOffset($this->workspace()->path('test.php'), 2)]);
        $expected = [new LspLocation('file://' . $this->workspace()->path('test.php'), new Range(new Position(0, 2), new Position(0, 2)))];
    ...
    public function testIgnoresNonExistingFiles() : void
    {
        ...
        $locations = new Locations([Location::fromPathAndOffset($this->workspace()->path('test.php'), 2), Location::fromPathAndOffset($this->workspace()->path('test-no.php'), 2)]);
        $expected = [new LspLocation('file://' . $this->workspace()->path('test.php'), new Range(new Position(0, 2), new Position(0, 2)))];
        $converter = new LocationConverter(new FilesystemWorkspaceLocator());
        self::assertEquals($expected, $converter->toLspLocations($locations));
    }
    ...
    {
        (yield 'out of upper range' => ['12345', 10, $this->createRange(0, 5, 0, 5)]);
    }
    /**
     * @return Generator<mixed>
     */
    ...
    {
        (yield '4 byte char 1st char' => ['😼😼😼😼😼', 4, $this->createRange(0, 4, 0, 4)]);
        (yield '4 byte char 2nd char' => ['😼😼😼😼😼', 5, $this->createRange(0, 8, 0, 8)]);
        (yield '4 byte char 4th char' => ['😼😼😼😼😼', 16, $this->createRange(0, 16, 0, 16)]);
    }
    /**
     * @return Generator<mixed>
     */
    ...
    {
        (yield 'single line' => ['12345678', 2, $this->createRange(0, 2, 0, 2)]);
        (yield 'second line' => ["12\n345\n678", 4, $this->createRange(1, 1, 1, 1)]);
        (yield 'third line first char' => ["12\n345\n678", 8, $this->createRange(2, 1, 2, 1)]);
    }
...
\class_alias('Phpactor202301\\Phpactor\\Extension\\LanguageServer\\Tests\\ridge\\Converter\\LocationConverterTest', 'Phpactor\\Extension\\LanguageServer\\Tests\\ridge\\Converter\\LocationConverterTest', \false);
