...
class AnalyseCommand extends Command
{
    ...
    {
        $start = (float) \microtime(\true);
        /**
         * @var array<string,Diagnostics<Diagnostic>> $results
         */
        $results = [];
        $path = $input->getArgument(self::ARG_PATH);
        $count = \count(\iterator_to_array($this->analyser->fileList($path), \true));
    ...
    private function renderTable(OutputInterface $output, array $results, float $start) : void
    {
        ...
        foreach ($results as $file => $diagnostics) {
            ...
            foreach ($diagnostics as $diagnostic) {
                $errorCount++;
                $table->addRow([\sprintf('%s:%s', $diagnostic->range()->start()->toInt(), $diagnostic->range()->end()->toInt()), $diagnostic->severity()->toString(), $diagnostic->message()]);
            }
            $table->render();
            $output->writeln('');
        }
        $output->writeln(\sprintf('%s problems in %s seconds with %sb memory', \number_format($errorCount), \number_format(\microtime(\true) - $start, 4), \number_format(\memory_get_peak_usage())));
    }
    /**
     * @param array<string,Diagnostics<Diagnostic>> $results
     */
    private function renderJson(OutputInterface $output, array $results) : void
    {
        foreach ($results as $file => $diagnostics) {
            foreach ($diagnostics as $diagnostic) {
                $output->writeln((string) \json_encode(['file' => $file, 'range' => ['start' => $diagnostic->range()->start()->toInt(), 'end' => $diagnostic->range()->end()->toInt()], 'message' => $diagnostic->message(), 'severity' => $diagnostic->severity()]));
            }
        }
    }
}
