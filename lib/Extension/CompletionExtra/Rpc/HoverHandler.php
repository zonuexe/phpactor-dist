...
class HoverHandler implements Handler
{
    ...
    {
        $offset = $this->reflector->reflectOffset(TextDocumentBuilder::create(Cast::toString($arguments[self::PARAM_SOURCE]))->build(), Cast::toInt($arguments[self::PARAM_OFFSET]));
        $type = $offset->nodeContext()->type();
        $nodeContext = $offset->nodeContext();
        $info = $this->messageFromSymbolContext($nodeContext);
        $info = $info ?: \sprintf('%s %s', $nodeContext->symbol()->symbolType(), $nodeContext->symbol()->name());
