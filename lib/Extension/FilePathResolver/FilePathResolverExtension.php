...
class FilePathResolverExtension implements Extension
{
    ...
    {
        $schema->setDefaults([self::PARAM_PROJECT_ROOT => \getcwd(), self::PARAM_APP_NAME => 'phpactor', self::PARAM_APPLICATION_ROOT => null, self::PARAM_ENABLE_CACHE => \true, self::PARAM_ENABLE_LOGGING => \true]);
    }
    ...
    public static function calculateProjectId(string $projectRoot) : string
    {
        if (empty($projectRoot)) {
            throw new RuntimeException('Project root must be a non-empty string');
        }
        return \sprintf('%s-%s', \basename($projectRoot), \substr(\md5(TextDocumentUri::fromString($projectRoot)->path()), 0, 6));
    }
    private function registerPathResolver(ContainerBuilder $container) : void
    {
        $container->register(self::SERVICE_FILE_PATH_RESOLVER, function (Container $container) {
            ...
            if ($container->getParameter(self::PARAM_ENABLE_LOGGING)) {
                $resolver = new LoggingPathResolver($resolver, LoggingExtension::channelLogger($container, self::LOG_CHANNEL), LogLevel::DEBUG);
            }
            return $resolver;
        });
    }
