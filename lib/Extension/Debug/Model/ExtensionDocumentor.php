...
class ExtensionDocumentor implements Documentor
{
    ...
    private function documentExtension(string $extensionClass) : ?string
    {
        ...
        if (\false === $documentedName) {
            throw new RuntimeException(\sprintf('Invalid extension class name "%s"', $extensionClass));
        }
        $help = ['.. _' . $documentedName . ':', "\n", $documentedName, \str_repeat('-', \mb_strlen($documentedName)), "\n"];
        $extension = new $extensionClass();
        if (!$extension instanceof Extension) {
            throw new RuntimeException(\sprintf('Expected "%s" to be an instanceof Phpactor\\Container\\Extension', \get_class($extension)));
        }
