...
class Navigator
{
    ...
    private function destination(string $path, string $destinationName)
    {
        ...
        if (\false === isset($destinations[$destinationName])) {
            throw new RuntimeException(\sprintf('Destination "%s" does not exist, known destinations: "%s"', $destinationName, \implode('", "', \array_keys($destinations))));
        }
    ...
    private function variant(string $destinationName)
    {
        if (!isset($this->autoCreateConfig[$destinationName])) {
            throw new RuntimeException(\sprintf('Destination "%s" has no new class variant set', $destinationName));
        }
