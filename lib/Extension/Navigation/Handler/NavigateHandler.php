...
class NavigateHandler extends AbstractHandler
{
    ...
    {
        $resolver->setDefaults([self::PARAM_SOURCE_PATH => null, self::PARAM_DESTINATION => null, self::PARAM_CONFIRM_CREATE => null]);
    }
    public function handle(array $arguments)
    {
        if (null === $arguments[self::PARAM_SOURCE_PATH]) {
            throw new RuntimeException(\sprintf('Param %s is required', self::PARAM_SOURCE_PATH));
        }
        ...
        $this->requireInput(ChoiceInput::fromNameLabelChoices(self::PARAM_DESTINATION, 'Destination:', \array_combine(\array_keys($destinations), \array_keys($destinations))));
        ...
        if ($canCreate) {
            $this->requireInput(ConfirmInput::fromNameAndLabel(self::PARAM_CONFIRM_CREATE, \sprintf('File "%s" does not exist, generate new?: ', $path)));
        }
        if ($this->hasMissingArguments($arguments)) {
            return $this->createInputCallback($arguments);
        }
