...
namespace Phpactor202301\Phpactor\Extension\LanguageServerIndexer\Tests\Unit;
...
class IndexerHandlerTest extends IntegrationTestCase
{
    ...
    {
        $container = $this->container([LanguageServerExtension::PARAM_FILE_EVENTS => \false]);
        $this->tester = $container->get(LanguageServerBuilder::class)->tester(new InitializeParams(rootUri: $this->workspace()->path(), capabilities: new ClientCapabilities(window: ['workDoneProgress' => \true])));
    }
...
EOT
    ...
    {
        $this->tester->notifyAndWait('phpactor/indexer/reindex', ['soft' => \false]);
        self::assertContains('indexer', $this->tester->services()->listRunning());
    }
...
EOT
        ...
        $tester = $this->container(['indexer.enabled_watchers' => ['will_die']])->get(LanguageServerBuilder::class)->tester(new InitializeParams(rootUri: $this->workspace()->path(), capabilities: new ClientCapabilities(window: ['workDoneProgress' => \true])));
        $tester->initialize();
...
\class_alias('Phpactor202301\\Phpactor\\Extension\\LanguageServerIndexer\\Tests\\Unit\\IndexerHandlerTest', 'Phpactor\\Extension\\LanguageServerIndexer\\Tests\\Unit\\IndexerHandlerTest', \false);
