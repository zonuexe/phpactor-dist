...
class ConsoleExtension implements Extension
{
    ...
    public function load(ContainerBuilder $container) : void
    {
        $container->register(self::SERVICE_COMMAND_LOADER, function (Container $container) {
            ...
            foreach ($container->getServiceIdsForTag(self::TAG_COMMAND) as $commandId => $attrs) {
                if (!isset($attrs['name'])) {
                    throw new InvalidArgumentException(\sprintf('Command with service ID "%s" must have the "name" attribute', $commandId));
                }
    ...
    {
        $schema->setDefaults([self::PARAM_VERBOSITY => OutputInterface::VERBOSITY_NORMAL, self::PARAM_DECORATED => null]);
        $schema->setDescriptions([self::PARAM_VERBOSITY => 'Verbosity level', self::PARAM_DECORATED => 'Whether to decorate messages (null for auto-guessing)']);
