    ...
    private function openConfig() : stdClass
    {
        if (!\file_exists($this->configPath)) {
            if (\false === file_put_contents($this->configPath, $this->createConfig())) {
                throw new RuntimeException(\sprintf('Could not write Phpactor config file to "%s"', $this->configPath));
            }
        }
        $config = file_get_contents($this->configPath);
        if (\false === $config) {
            throw new RuntimeException(\sprintf('Could not read config file "%s"', $this->configPath));
        }
        $json = json_decode($config);
        if (null === $json) {
            throw new RuntimeException(\sprintf('Could not decode JSON file "%s"', $this->configPath));
        }
        return $json;
    }
    /**
