...
class StatusHandler implements Handler
{
    ...
    {
        $resolver->setDefaults([self::PARAM_TYPE => self::TYPE_FORMATTED]);
    }
    ...
    {
        $status['diagnostics'] = \array_merge(\array_fill_keys($status['good'], \true), \array_fill_keys($status['bad'], \false));
        unset($status['good']);
        unset($status['bad']);
    ...
    {
        $info = ['Info', '----', 'Version: ' . $diagnostics['phpactor_version'], 'PHP: ' . \sprintf('%s (supporting %s)', \phpversion(), $diagnostics['php_version']), 'Phpactor dir: ' . \realpath(__DIR__ . '/../../../../'), 'Work dir: ' . $diagnostics['cwd'] . \PHP_EOL, 'Diagnostics', '-----------', $this->buildSupportMessage($diagnostics), 'Config files', '------------', $this->buildConfigFileMessage()];
    ...
    {
        return \implode(\PHP_EOL, [\implode(\PHP_EOL, \array_map(function (string $message) {
            return '[✔] ' . $message;
        }, $diagnostics['good'])), \implode(\PHP_EOL, \array_map(function (string $message) {
            return '[✘] ' . $message;
        }, $diagnostics['bad']))]);
    }
