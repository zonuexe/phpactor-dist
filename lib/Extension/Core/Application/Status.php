...
class Status
{
    ...
    public function __construct(private FilesystemRegistry $registry, private PathCandidates $paths, private string $workingDirectory, private PhpVersionResolver $phpVersionResolver, ExecutableFinder $executableFinder = null, private bool $warnOnDevelop = \true)
    {
    ...
    public function check() : array
    {
        ...
        $diagnostics = ['filesystems' => $filesystems, 'cwd' => $this->workingDirectory, 'php_version' => $this->phpVersionResolver->resolve(), 'config_files' => [], 'good' => [], 'bad' => []];
        ...
        if ($path = $this->executableFinder->find('git')) {
            $process = new Process(['git', 'log', '-1', '--pretty=format:"%h (%ad) %f REF(%D)REF', '--date=relative'], __DIR__ . '/../../../..');
            $process->run();
    ...
    private function versionInfo(Process $process) : array
    {
        if ($process->getExitCode() !== 0) {
            return ['phpactor_version' => 'ERROR: ' . $process->getErrorOutput(), 'phpactor_is_develop' => \false];
        }
        if (!\preg_match('{^(.*)REF(.*?)REF}', $process->getOutput(), $matches)) {
            return ['phpactor_version' => $process->getOutput(), 'phpactor_is_develop' => \false];
        }
