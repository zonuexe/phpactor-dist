...
final class ChainPrompt implements \Phpactor\Extension\Core\Console\Prompt\Prompt
{
    ...
    public function prompt(string $prompt, string $prefill) : string
    {
        ...
        throw new RuntimeException(\sprintf('Could not prompt for "%s". ' . 'Appropriate prompt implementation for your platform / environment could not be found (tried "%s"). ' . 'Try specifying the command in full', $prompt, \implode('", "', \array_keys($this->prompts))));
    }
