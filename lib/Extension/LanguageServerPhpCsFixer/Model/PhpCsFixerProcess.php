...
class PhpCsFixerProcess
{
     ...
     */
    public function __construct(private string $binPath, private LoggerInterface $logger, private array $env = [])
    {
    }
    /**
    ...
    public function fix(string $content, array $options = []) : Promise
    {
        return call(function () use($content, $options) {
            ...
            if ($exitCode !== 0 && $exitCode !== self::EXIT_SOME_FILES_INVALID && $exitCode !== self::EXIT_FILES_NEEDS_FIXING && $exitCode !== (self::EXIT_SOME_FILES_INVALID | self::EXIT_FILES_NEEDS_FIXING)) {
                throw new PhpCsFixerError($exitCode, $process->getCommand(), (yield buffer($process->getStderr())), $stdout);
            }
            return $stdout;
        });
    }
