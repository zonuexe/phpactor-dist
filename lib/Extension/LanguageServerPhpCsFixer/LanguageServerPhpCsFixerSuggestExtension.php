    ...
    public function load(ContainerBuilder $container) : void
    {
        $container->register('language_server_php_cs_fixer.suggest', function (Container $container) {
            return new PhpactorComposerSuggestor($container->expect(ConfigurationExtension::SERVICE_PHPACTOR_CONFIG_LOCAL, JsonConfig::class), $container->get(ComposerInspector::class), function (JsonConfig $config, ComposerInspector $inspector) {
                if ($config->has(\Phpactor\Extension\LanguageServerPhpCsFixer\LanguageServerPhpCsFixerExtension::PARAM_ENABLED)) {
                    return Changes::none();
                }
                if (!$inspector->package('friendsofphp/php-cs-fixer')) {
                    return Changes::none();
                }
                return Changes::from([new PhpactorConfigChange('PHP-CS-Fixer detected, enable the PHP-CS-Fixer extension?', function (bool $enable) {
                    return [\Phpactor\Extension\LanguageServerPhpCsFixer\LanguageServerPhpCsFixerExtension::PARAM_ENABLED => $enable];
                })]);
            });
