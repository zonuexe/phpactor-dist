    ...
    public function toTextEdits(string $diffText) : array
    {
        ...
        foreach ($parsedDiffs as $diff) {
            foreach ($diff->getChunks() as $chunk) {
                $consumer = new \Phpactor\Extension\LanguageServerPhpCsFixer\Util\DiffLinesConsumer($chunk);
                while ($consumer->current()) {
                    ...
                    if (($added = $consumer->eatAdded()) !== null) {
                        $edits[] = new TextEdit(new Range(new Position($startLine, 0), new Position($startLine, 0)), $this->linesToString($added));
                    }
                    if (($removed = $consumer->eatRemoved()) !== null) {
                        $added = $consumer->eatAdded();
                        $edits[] = new TextEdit(new Range(new Position($startLine, 0), new Position($startLine + \count($removed), 0)), $this->linesToString($added));
                    }
                }
            }
        }
