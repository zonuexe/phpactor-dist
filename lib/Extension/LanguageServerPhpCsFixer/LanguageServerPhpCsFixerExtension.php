...
class LanguageServerPhpCsFixerExtension implements OptionalExtension
{
    ...
    {
        $container->register(PhpCsFixerProcess::class, function (Container $container) {
            ...
            return new PhpCsFixerProcess($path, LoggingExtension::channelLogger($container, 'php-cs-fixer'), $container->getParameter(self::PARAM_ENV));
        });
        ...
        $container->register(PhpCsFixerDiagnosticsProvider::class, function (Container $container) {
            return new PhpCsFixerDiagnosticsProvider($container->get(PhpCsFixerProcess::class), $container->getParameter(self::PARAM_SHOW_DIAGNOSTICS), LoggingExtension::channelLogger($container, 'php-cs-fixer'));
        }, [LanguageServerExtension::TAG_DIAGNOSTICS_PROVIDER => DiagnosticProviderTag::create('php-cs-fixer'), LanguageServerExtension::TAG_CODE_ACTION_PROVIDER => []]);
        $container->register(FormatCommand::class, function (Container $container) {
            return new FormatCommand($container->get(PhpCsFixerProcess::class), $container->get(ClientApi::class), $container->get(LanguageServerExtension::SERVICE_SESSION_WORKSPACE), LoggingExtension::channelLogger($container, 'php-cs-fixer'));
        }, [LanguageServerExtension::TAG_COMMAND => ['name' => 'php_cs_fixer.fix']]);
    }
    public function configure(Resolver $schema) : void
    {
        $schema->setDefaults([self::PARAM_PHP_CS_FIXER_BIN => '%project_root%/vendor/bin/php-cs-fixer', self::PARAM_ENV => ['XDEBUG_MODE' => 'off', 'PHP_CS_FIXER_IGNORE_ENV' => \true], self::PARAM_SHOW_DIAGNOSTICS => \true]);
        $schema->setDescriptions([self::PARAM_PHP_CS_FIXER_BIN => 'Path to the php-cs-fixer executable', self::PARAM_ENV => 'Environment for PHP CS Fixer (e.g. to set PHP_CS_FIXER_IGNORE_ENV)', self::PARAM_SHOW_DIAGNOSTICS => 'Whether PHP CS Fixer diagnostics are shown']);
    }
