...
use PhpactorDist\SebastianBergmann\Diff\Diff;
use PhpactorDist\SebastianBergmann\Diff\Line;
use PhpactorDist\SebastianBergmann\Diff\Parser;
use LogicException;
...
class PhpCsFixerDiagnosticsProvider implements DiagnosticsProvider, CodeActionProvider
{
    ...
    public function __construct(private PhpCsFixerProcess $phpCsFixer, private bool $showDiagnostics, private LoggerInterface $logger)
    {
    }
    /**
    ...
    public function provideDiagnostics(TextDocumentItem $textDocument, CancellationToken $cancel) : Promise
    {
        ...
        return \PhpactorDist\Amp\call(function () use($textDocument, $cancel) {
    ...
    public function provideActionsFor(TextDocumentItem $textDocument, Range $range, CancellationToken $cancel) : Promise
    {
        return \PhpactorDist\Amp\call(function () use($textDocument, $cancel) {
            ...
            $actions = [CodeAction::fromArray(['title' => $title, 'kind' => 'source.fixAll.phpactor.phpCsFixer', 'diagnostics' => $diagnostics, 'command' => new Command($title, 'php_cs_fixer.fix', [$textDocument->uri])])];
            return $actions;
        });
    }
    ...
    {
        return \PhpactorDist\Amp\call(function () use($textDocument) {
            $outputJson = (yield $this->phpCsFixer->fix($textDocument->text, ['--dry-run', '--verbose', '--format', 'json']));
            ...
            foreach ($rules as $rule) {
                ...
                if (\count($fileDiff) !== 1) {
                    $this->logger->warning(\sprintf("Expected php-cs-fixer to provide 1 diff, got %s. Skipping diagnostics for file '%s'", \count($fileDiff), $textDocument->uri));
                    continue;
                }
        ...
        foreach ($fileDiff->getChunks() as $chunk) {
            ...
            foreach ($chunk->getLines() as $index => $line) {
                ...
                if ($lastChangedLine) {
                    ...
                    $endPos = $lineLength ? new Position($lineNo - 1, $lineLength) : new Position($lineNo, 0);
                    if ($replacedLines) {
                ...
                if ($line->getType() === Line::ADDED && $prevLine->getType() === Line::UNCHANGED) {
    ...
    {
        return \PhpactorDist\Amp\call(function () use($rule, $range) {
            return Diagnostic::fromArray(['message' => (yield $this->explainRule($rule)), 'range' => $range, 'severity' => DiagnosticSeverity::WARNING, 'source' => $this->name(), 'code' => $rule]);
        });
    }
    /**
    ...
    private function explainRule(string $rule) : Promise
    {
        ...
        return \PhpactorDist\Amp\call(function () use($rule) {
