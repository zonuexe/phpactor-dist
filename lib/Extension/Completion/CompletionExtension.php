...
class CompletionExtension implements Extension
{
    ...
    {
        $schema->setDefaults([self::PARAM_DEDUPE => \true, self::PARAM_DEDUPE_MATCH_FQN => \true, self::PARAM_LIMIT => null]);
        $schema->setDescriptions([self::PARAM_DEDUPE => 'If results should be de-duplicated', self::PARAM_DEDUPE_MATCH_FQN => 'If ``' . self::PARAM_DEDUPE . '``, consider the class FQN in addition to the completion suggestion', self::PARAM_LIMIT => 'Sets a limit on the number of completion suggestions for any request']);
    }
    ...
    private function registerCompletion(ContainerBuilder $container) : void
    {
        $container->register(self::SERVICE_REGISTRY, function (Container $container) {
            ...
            foreach ($completors as $type => $completors) {
                ...
                if ($container->getParameter(self::PARAM_DEDUPE)) {
                    $completors = new DedupeCompletor($completors, $container->getParameter(self::PARAM_DEDUPE_MATCH_FQN));
                }
        ...
        $container->register(self::SERVICE_SIGNATURE_HELPER, function (Container $container) {
            ...
            return new ChainSignatureHelper(LoggingExtension::channelLogger($container, 'CT'), $helpers);
        });
    }
}
