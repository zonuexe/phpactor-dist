...
namespace Phpactor202301\Phpactor\Extension\Completion\Tests\Unit;
...
class CompletionExtensionTest extends TestCase
{
    ...
    public function testCreatesChainedCompletor() : void
    {
        ...
        $this->completor1->complete($document, ByteOffset::fromInt(self::EXAMPLE_OFFSET))->will(function () {
            return (function () {
        ...
        $results = \iterator_to_array($completor->complete($document, ByteOffset::fromInt(self::EXAMPLE_OFFSET)));
    ...
    {
        $document = TextDocumentBuilder::create(self::EXAMPLE_SOURCE)->build();
        $this->signatureHelper1->signatureHelp($document, ByteOffset::fromInt(self::EXAMPLE_OFFSET))->will(function () {
            return (function () {
                return new SignatureHelp([], 0);
            })();
        });
        $signatureHelper = $this->createContainer()->get(CompletionExtension::SERVICE_SIGNATURE_HELPER);
        $help = $signatureHelper->signatureHelp($document, ByteOffset::fromInt(self::EXAMPLE_OFFSET));
    ...
    private function createContainer() : Container
    {
        ...
        $builder->register('short_desc_formatter_array', function () {
            return [$this->formatter1->reveal(), $this->formatter1->reveal()];
        }, [CompletionExtension::TAG_SHORT_DESC_FORMATTER => []]);
        ...
        $extension->load($builder);
        return $builder->build(['logging.enabled' => \false, CompletionExtension::PARAM_DEDUPE => \false, CompletionExtension::PARAM_DEDUPE_MATCH_FQN => \false, CompletionExtension::PARAM_LIMIT => 10]);
    }
}
\class_alias('Phpactor202301\\Phpactor\\Extension\\Completion\\Tests\\Unit\\CompletionExtensionTest', 'Phpactor\\Extension\\Completion\\Tests\\Unit\\CompletionExtensionTest', \false);
