...
class ClassMemberReferences
{
    ...
    public function findOrReplaceReferences(string $scope, string $class = null, string $memberName = null, string $memberType = null, string $replace = null, bool $dryRun = \false)
    {
    ...
    public function replaceInSource(string $source, string $class, string $memberName, string $memberType, string $replacement)
    {
        ...
        $referenceList = $this->memberFinder->findMembers(SourceCode::fromString($source), $query);
        return (string) $this->replaceReferencesInCode($source, $referenceList->withClasses(), $replacement);
    }
    private function referencesInFile(Filesystem $filesystem, $filePath, string $className = null, string $memberName = null, string $memberType = null, string $replace = null, bool $dryRun = \false)
    {
        $code = $filesystem->getContents($filePath);
        $query = $this->createQuery($className, $memberName, $memberType);
        $referenceList = $this->memberFinder->findMembers(SourceCode::fromString($code), $query);
        $confidentList = $referenceList->withClasses();
        $riskyList = $referenceList->withoutClasses();
        $result = ['references' => [], 'risky_references' => [], 'replacements' => []];
        ...
        if ($replace) {
            ...
            $replacedReferences = $this->memberFinder->findMembers(SourceCode::fromString($updatedSource), $query);
    ...
    private function serializeReference(string $code, MemberReference $reference)
    {
        ...
        return ['start' => $reference->position()->start(), 'end' => $reference->position()->end(), 'line' => $line, 'line_no' => $lineNumber, 'col_no' => $colNumber, 'reference' => (string) $reference->methodName(), 'class' => $reference->hasClass() ? (string) $reference->class() : null];
    }
