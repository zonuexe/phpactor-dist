...
class ClassMover
{
    public function __construct(private ClassFileNormalizer $classFileNormalizer, private ClassMoverFacade $classMover, private FilesystemRegistry $filesystemRegistry, private PathFinder $pathFinder)
    {
    }
     ...
     */
    public function move(ClassMoverLogger $logger, string $filesystemName, string $src, string $dest, bool $moveRelatedFiles) : void
    {
    ...
    {
        return $this->moveFile($logger, $filesystemName, $this->classFileNormalizer->classToFile($srcName), $this->classFileNormalizer->classToFile($destName), $moveRelatedFiles);
    }
    ...
    private function doMoveFile(ClassMoverLogger $logger, string $filesystemName, string $srcPath, string $destPath) : void
    {
        ...
        if (!\file_exists(\dirname($destPath->path()))) {
            \mkdir(\dirname($destPath->path()), 0777, \true);
        }
    ...
    private function replaceReferences(ClassMoverLogger $logger, Filesystem $filesystem, string $srcName, string $destName) : void
    {
        foreach ($filesystem->fileList()->existing()->phpFiles() as $filePath) {
            ...
            $edits = $this->classMover->replaceReferences($references, $destName);
