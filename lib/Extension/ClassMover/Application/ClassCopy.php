...
class ClassCopy
{
    ...
    public function __construct(private ClassFileNormalizer $classFileNormalizer, private ClassMoverFacade $classMover, private Filesystem $filesystem)
    {
    }
    /**
    ...
    {
        return $this->copyFile($logger, $this->classFileNormalizer->classToFile($srcName), $this->classFileNormalizer->classToFile($destName));
    }
    ...
    private function updateReferences(ClassCopyLogger $logger, CopyReport $copyReport) : void
    {
        foreach ($copyReport->srcFiles() as $srcPath) {
            ...
            $edits = $this->classMover->replaceReferences($references, $destClassName);
