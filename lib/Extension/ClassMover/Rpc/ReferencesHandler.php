...
class ReferencesHandler extends AbstractHandler
{
    ...
    public function __construct(private Reflector $reflector, private ClassReferences $classReferences, private ClassMemberReferences $classMemberReferences, private FilesystemRegistry $registry, private string $defaultFilesystem = SourceCodeFilesystemExtension::FILESYSTEM_GIT)
    {
    }
    ...
    {
        $resolver->setDefaults([self::PARAMETER_MODE => self::MODE_FIND, self::PARAMETER_FILESYSTEM => $this->defaultFilesystem, self::PARAMETER_REPLACEMENT => null]);
        $resolver->setRequired([self::PARAMETER_PATH, self::PARAMETER_OFFSET, self::PARAMETER_SOURCE]);
    }
    ...
    private function replaceReferences(NodeContext $nodeContext, string $filesystem, string $replacement, string $path, string $source)
    {
        ...
        if ($source) {
            ...
            if (\file_exists($path)) {
                $actions[] = OpenFileResponse::fromPath($path)->withForcedReload(\true);
    ...
    private function classReferences(string $filesystem, NodeContext $nodeContext, string $source = null, string $replacement = null)
    {
        ...
        $updatedSource = null;
        if ($source) {
            $updatedSource = $this->classReferences->replaceInSource($source, $classType, $replacement);
        }
    ...
    private function sortReferences(array $fileReferences) : array
    {
        ...
        \array_walk($fileReferences, function (array &$fileReference) : void {
            if (empty($fileReference['references'])) {
                return;
                // Do nothing if there is no references
            }
