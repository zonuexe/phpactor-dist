...
class ClassMoverExtension implements Extension
{
    ...
    private function registerRpc(ContainerBuilder $container) : void
    {
        $container->register('class_mover.handler.class_references', function (Container $container) {
            return new ReferencesHandler($container->get(WorseReflectionExtension::SERVICE_REFLECTOR), $container->get('application.class_references'), $container->get('application.method_references'), $container->get(SourceCodeFilesystemExtension::SERVICE_REGISTRY));
    ...
    private function registerClassMover(ContainerBuilder $container) : void
    {
        $container->register('class_mover.member_finder', function (Container $container) {
            return new WorseTolerantMemberFinder($container->get(WorseReflectionExtension::SERVICE_REFLECTOR));
        });
    ...
    private function registerApplicationServices(ContainerBuilder $container) : void
    {
        ...
        $container->register('application.class_copy', function (Container $container) {
            return new ClassCopy($container->get('application.helper.class_file_normalizer'), $container->get(ClassMover::class), $container->get('source_code_filesystem.registry')->get('git'));
        });
        $container->register('application.class_references', function (Container $container) {
            return new ClassReferences($container->get('application.helper.class_file_normalizer'), $container->get('class_mover.class_finder'), $container->get('class_mover.ref_replacer'), $container->get('source_code_filesystem.registry'));
        });
        $container->register('application.method_references', function (Container $container) {
            return new ClassMemberReferences($container->get('application.helper.class_file_normalizer'), $container->get('class_mover.member_finder'), $container->get('class_mover.member_replacer'), $container->get('source_code_filesystem.registry'), $container->get(WorseReflectionExtension::SERVICE_REFLECTOR));
        });
    }
    private function registerConsoleCommands(ContainerBuilder $container) : void
    {
        ...
        $container->register('command.class_references', function (Container $container) {
            return new ReferencesClassCommand($container->get('application.class_references'), $container->get('console.dumper_registry'));
        ...
        $container->register('command.member_references', function (Container $container) {
            return new ReferencesMemberCommand($container->get('application.method_references'), $container->get('console.dumper_registry'));
