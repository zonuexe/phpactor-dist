...
class GotoImplementationHandler extends AbstractHandler
{
    ...
    {
        $resolver->setDefaults([self::PARAM_LANGUAGE => 'php', self::PARAM_TARGET => OpenFileResponse::TARGET_FOCUSED_WINDOW]);
        $resolver->setRequired([self::PARAM_OFFSET, self::PARAM_SOURCE, self::PARAM_PATH]);
        $resolver->setEnums([self::PARAM_TARGET => OpenFileResponse::VALID_TARGETS]);
        $resolver->setTypes([self::PARAM_OFFSET => 'integer', self::PARAM_LANGUAGE => 'string', self::PARAM_TARGET => 'string']);
        $resolver->setDescriptions([self::PARAM_OFFSET => 'Number of character into the buffer', self::PARAM_SOURCE => 'Content of the current file', self::PARAM_PATH => 'Path of the current file', self::PARAM_LANGUAGE => 'Language of the current file', self::PARAM_TARGET => 'Where should the reference be opened']);
    }
    public function handle(array $arguments)
    {
        $document = TextDocumentBuilder::create($arguments[self::PARAM_SOURCE])->uri($arguments[self::PARAM_PATH])->language($arguments[self::PARAM_LANGUAGE])->build();
    ...
    private function locationsToReferences(Locations $locations) : array
    {
        ...
        foreach ($locations as $location) {
            ...
            $fileReferences = FileReferences::fromPathAndReferences($location->uri()->path(), [Reference::fromStartEndLineNumberLineAndCol($location->offset()->toInt(), $location->offset()->toInt(), $lineCol->line(), $line, $lineCol->col())]);
    ...
    private function fileContents(Location $location)
    {
        if (!\file_exists($location->uri()->path())) {
            throw new RuntimeException(\sprintf('Could not open file "%s"', $location->uri()->path()));
        }
