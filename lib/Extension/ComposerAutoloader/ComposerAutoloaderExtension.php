...
class ComposerAutoloaderExtension implements Extension
{
    ...
    public function load(ContainerBuilder $container) : void
    {
        $container->register(self::SERVICE_AUTOLOADERS, function (Container $container) {
            ...
            $autoloaderPaths = \array_filter(\array_map(function ($path) use($container) {
                $path = $container->get(FilePathResolverExtension::SERVICE_FILE_PATH_RESOLVER)->resolve($path);
    ...
    {
        LoggingExtension::channelLogger($container, self::LOG_CHANNEL)->warning(\sprintf('Could not find autoloader "%s"', $autoloaderPath));
    }
