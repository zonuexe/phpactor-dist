...
class ConfigurationExtension implements Extension
{
    ...
    private function registerCommands(ContainerBuilder $container) : void
    {
        ...
        $container->register(ConfigSuggestCommand::class, function (Container $container) {
            return new ConfigSuggestCommand($container->get(Configurator::class));
        }, [ConsoleExtension::TAG_COMMAND => ['name' => 'config:auto']]);
    ...
    private function registerMisc(ContainerBuilder $container) : void
    {
        $container->register(ConfigManipulator::class, function (Container $container) {
            return new ConfigManipulator(\realpath(__DIR__ . '/../../..') . '/phpactor.schema.json', $container->parameter(FilePathResolverExtension::PARAM_PROJECT_ROOT)->string() . '/.phpactor.json');
        });
    }
}
