...
class ClassReflector
{
    ...
    public function reflect(string $classOrFile) : array
    {
        ...
        $return = ['class' => (string) $reflection->name(), 'class_namespace' => (string) $reflection->name()->namespace(), 'class_name' => (string) $reflection->name()->short(), 'methods' => [], 'properties' => [], 'constants' => []];
        foreach ($reflection->methods() as $method) {
            \assert($method instanceof ReflectionMethod);
            $methodInfo = [(string) $method->visibility() . ' function ' . $method->name()];
            $return['methods'][$method->name()] = ['name' => $method->name(), 'abstract' => $method->isAbstract(), 'visibility' => (string) $method->visibility(), 'parameters' => [], 'static' => $method->isStatic() ? 1 : 0];
            $paramInfos = [];
            foreach ($method->parameters() as $parameter) {
                ...
                if ($parameter->default()->isDefined()) {
                    $paramInfo[] = ' = ' . \str_replace(\PHP_EOL, '', \var_export($parameter->default()->value(), \true));
                }
                $paramInfos[] = \implode(' ', $paramInfo);
                $return['methods'][$method->name()]['parameters'][$parameter->name()] = ['name' => $parameter->name(), 'has_type' => $parameter->type()->isDefined(), 'type' => $parameter->type()->__toString(), 'has_default' => $parameter->default()->isDefined(), 'default' => $parameter->default()->value()];
            }
