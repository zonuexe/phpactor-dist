...
class SymfonyDiContextClassResolver implements ContextClassResolver
{
    ...
    public function resolve(string $className) : string
    {
        ...
        throw new CouldNotResolverContextClass(\sprintf('Could not resolve context from Symfony container "%s"', $this->xmlPath));
    }
    private function loadIndex() : void
    {
        ...
        if (!\file_exists($this->xmlPath)) {
            throw new RuntimeException(\sprintf('Symfony DI XML file "%s" does not exist', $this->xmlPath));
        }
