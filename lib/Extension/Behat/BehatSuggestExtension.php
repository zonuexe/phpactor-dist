...
class BehatSuggestExtension implements Extension
{
    public function load(ContainerBuilder $container) : void
    {
        $container->register('behat.suggest', function (Container $container) {
            ...
            return new PhpactorComposerSuggestor($container->expect(ConfigurationExtension::SERVICE_PHPACTOR_CONFIG_LOCAL, JsonConfig::class), $container->get(ComposerInspector::class), function (JsonConfig $config, ComposerInspector $inspector) use($pathResolver) {
                if ($config->has(\Phpactor\Extension\Behat\BehatExtension::PARAM_ENABLED)) {
                    return Changes::none();
                }
                if (!$inspector->package('behat/behat')) {
                    return Changes::none();
                }
                $changes = [new PhpactorConfigChange('Behat BDD framework detected, enable Behat extension?', function (bool $enable) {
                    return [\Phpactor\Extension\Behat\BehatExtension::PARAM_ENABLED => $enable];
                })];
                $xmlPath = 'var/cache/test/App_KernelTestDebugContainer.xml';
                $fullXmlPath = $pathResolver->resolve('%project_root%/' . $xmlPath);
                if (!$config->has(\Phpactor\Extension\Behat\BehatExtension::PARAM_SYMFONY_XML_PATH)) {
                    if (\file_exists($fullXmlPath)) {
                        $changes[] = new PhpactorConfigChange('Enable Behat Symfony integration?', function (bool $enable) use($xmlPath) {
                            return [\Phpactor\Extension\Behat\BehatExtension::PARAM_SYMFONY_XML_PATH => $xmlPath];
                        });
                    }
                }
