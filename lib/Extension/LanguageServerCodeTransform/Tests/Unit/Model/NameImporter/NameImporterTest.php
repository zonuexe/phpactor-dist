...
namespace Phpactor202301\LanguageServerCodeTransform\Unit\Model\NameImporter;
...
use Phpactor202301\Phpactor\CodeTransform\Domain\Refactor\ImportClass\NameImport as ImportClassNameImport;
use Phpactor202301\Phpactor\CodeTransform\Domain\Refactor\ImportName as RefactorImportName;
...
class NameImporterTest extends TestCase
{
    ...
    protected function setUp() : void
    {
        ...
        $this->textEdits = TextEdits::one(TextEdit::create(23, 6, 'huhuhu'));
        $this->lspTextEdits = TextEditConverter::toLspTextEdits($this->textEdits, self::EXAMPLE_CONTENT);
        $this->sourceCode = SourceCode::fromStringAndPath(self::EXAMPLE_CONTENT, TextDocumentUri::fromString(self::EXAMPLE_PATH_URI)->path());
        $this->subject = new NameImporter($this->importNameProphecy->reveal());
    }
    public function provideTestImportData() : Generator
    {
        (yield 'function' => ['\\in_array', 'function', ImportClassNameImport::forFunction('\\in_array')]);
        (yield 'class' => [self::class, 'class', ImportClassNameImport::forClass(self::class)]);
    }
    /**
     * @dataProvider provideTestImportData
     */
    public function testImport(string $fqn, string $importType, ImportClassNameImport $importClassNameImport) : void
    {
        $this->importNameProphecy->importName($this->sourceCode, $this->byteOffset, $importClassNameImport)->willReturn($this->textEdits);
        $result = $this->subject->__invoke($this->document, self::EXAMPLE_OFFSET, $importType, $fqn, \true);
    ...
    public function testImportTransformException() : void
    {
        ...
        $this->importNameProphecy->importName($this->sourceCode, $this->byteOffset, ImportClassNameImport::forClass(Exception::class))->willThrow($error);
        $result = $this->subject->__invoke($this->document, self::EXAMPLE_OFFSET, 'class', Exception::class, \true);
    ...
    public function testImportAliasAlreadyUsedException() : void
    {
        ...
        $this->importNameProphecy->importName($this->sourceCode, $this->byteOffset, $import)->willThrow($aliasAlreadyUsedException);
        $aliasedNameImport = ImportClassNameImport::forClass(Exception::class, 'AliasedException');
        $this->importNameProphecy->importName($this->sourceCode, $this->byteOffset, $aliasedNameImport)->willReturn($this->textEdits);
        $result = $this->subject->__invoke($this->document, self::EXAMPLE_OFFSET, 'class', Exception::class, \true);
        self::assertTrue($result->isSuccess());
    ...
    {
        $import = ImportClassNameImport::forClass(Exception::class);
        $nameAlreadyImportedException = new NameAlreadyImportedException($import, 'Exception', Exception::class);
        $this->importNameProphecy->importName($this->sourceCode, $this->byteOffset, ImportClassNameImport::forClass(Exception::class))->willThrow($nameAlreadyImportedException);
        $result = $this->subject->__invoke($this->document, self::EXAMPLE_OFFSET, 'class', Exception::class, \true);
        self::assertTrue($result->isSuccess());
    ...
    {
        $import = ImportClassNameImport::forClass(Exception::class);
        $nameAlreadyImportedException = new NameAlreadyImportedException($import, 'RuntimeException', RuntimeException::class);
        $this->importNameProphecy->importName($this->sourceCode, $this->byteOffset, $import)->willThrow($nameAlreadyImportedException);
