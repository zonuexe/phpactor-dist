...
class CreateUnresolvableClassProvider implements CodeActionProvider
{
    ...
    public function provideActionsFor(TextDocumentItem $textDocument, Range $range, CancellationToken $cancel) : Promise
    {
        return call(function () use($textDocument, $range) {
            $diagnostics = ((yield $this->reflector->diagnostics(TextDocumentConverter::fromLspTextItem($textDocument))))->byClass(UnresolvableNameDiagnostic::class)->containingRange(RangeConverter::toPhpactorRange($range, $textDocument->text));
            $actions = [];
            foreach ($diagnostics as $diagnostic) {
                ...
                foreach ($this->classToFile->classToFileCandidates(ClassName::fromString($diagnostic->name())) as $candidate) {
                    ...
                    foreach ($this->generators as $name => $_) {
                        ...
                        $actions[] = CodeAction::fromArray(['title' => $title, 'kind' => self::KIND, 'diagnostics' => [ProtocolFactory::diagnostic(RangeConverter::toLspRange($diagnostic->range(), $textDocument->text), $diagnostic->message())], 'command' => new Command($title, CreateClassCommand::NAME, [TextDocumentUri::fromString($candidate->__toString())->__toString(), $name])]);
                    }
                }
            }
            return $actions;
