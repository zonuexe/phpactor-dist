...
class CorrectUndefinedVariableCodeAction implements CodeActionProvider
{
    ...
    public function provideActionsFor(TextDocumentItem $textDocument, Range $range, CancellationToken $cancel) : Promise
    {
        return call(function () use($textDocument, $cancel) {
            $actions = [];
            foreach (((yield $this->reflector->diagnostics(TextDocumentConverter::fromLspTextItem($textDocument))))->byClass(UndefinedVariableDiagnostic::class) as $diagnostic) {
                ...
                foreach ($diagnostic->suggestions() as $suggestion) {
                    ...
                    $actions[] = new CodeAction(title: \sprintf('Correct undefined variable "$%s" to "$%s"', $diagnostic->undefinedVariableName(), $suggestion), kind: CodeActionKind::QUICK_FIX, diagnostics: null, isPreferred: null, disabled: null, edit: new WorkspaceEdit(documentChanges: [new TextDocumentEdit(new OptionalVersionedTextDocumentIdentifier($textDocument->uri, $textDocument->version), [new TextEdit(range: RangeConverter::toLspRange($diagnostic->range(), $textDocument->text), newText: '$' . $suggestion)])]), command: null);
                }
            }
            return $actions;
        });
