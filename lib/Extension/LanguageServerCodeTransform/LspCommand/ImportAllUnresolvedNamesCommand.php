...
class ImportAllUnresolvedNamesCommand implements Command
{
    ...
    public function __invoke(string $uri) : Promise
    {
        return call(function () use($uri) {
            ...
            foreach (((yield $this->candidateFinder->unresolved($item)))->onlyUniqueNames() as $unresolvedName) {
                ...
                if (null === $candidate) {
                    $this->client->window()->showMessage()->warning(\sprintf('Class "%s" has no candidates', $unresolvedName->name()->__toString()));
                    continue;
                }
                (yield $this->importName->__invoke($uri, $unresolvedName->byteOffset()->toInt(), $unresolvedName->type(), $candidate->candidateFqn()));
            }
        });
    }
    /**
    ...
    private function resolveCandidate(NameWithByteOffset $unresolved, array $candidates) : Promise
    {
        return call(function () use($unresolved, $candidates) {
            ...
            $choice = (yield $this->client->window()->showMessageRequest()->info(\sprintf('Ambiguous class "%s":', $unresolved->name()->__toString()), ...\array_map(function (NameCandidate $candidate) {
