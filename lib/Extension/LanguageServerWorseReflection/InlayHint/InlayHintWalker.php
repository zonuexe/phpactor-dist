...
class InlayHintWalker implements Walker
{
     ...
     */
    private array $hints = [];
    public function __construct(private ByteOffsetRange $range, private \Phpactor\Extension\LanguageServerWorseReflection\InlayHint\InlayHintOptions $options)
    {
    }
    ...
    private function fromCall(FrameResolver $resolver, Frame $frame, CallExpression $node) : void
    {
        ...
        foreach ($node->argumentExpressionList?->getValues() ?? [] as $index => $argument) {
            ...
            $this->hints[] = new InlayHint(position: PositionConverter::intByteOffsetToPosition($argument->getStartPosition(), $node->getFileContents()), label: $parameter->name(), kind: InlayHintKind::PARAMETER, textEdits: null, tooltip: $parameter->type()->__toString());
        }
    }
    private function fromVariable(FrameResolver $resolver, Frame $frame, Variable $node) : void
    {
        ...
        $this->hints[] = new InlayHint(position: PositionConverter::intByteOffsetToPosition($node->getStartPosition(), $node->getFileContents()), label: $variable->type()->short(), tooltip: $variable->type()->__toString(), kind: InlayHintKind::TYPE, textEdits: null);
    }
