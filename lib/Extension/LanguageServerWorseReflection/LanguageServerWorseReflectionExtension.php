...
class LanguageServerWorseReflectionExtension implements Extension
{
    ...
    {
        $schema->setDefaults([self::PARAM_UPDATE_INTERVAL => 100, self::PARAM_INLAY_HINTS_ENABLE => \false, self::PARAM_INLAY_HINTS_TYPES => \false, self::PARAM_INLAY_HINTS_PARAMS => \true]);
        $schema->setDescriptions([self::PARAM_UPDATE_INTERVAL => 'Minimum interval to update the workspace index as documents are updated (in milliseconds)', self::PARAM_INLAY_HINTS_ENABLE => 'Enable inlay hints (experimental)', self::PARAM_INLAY_HINTS_TYPES => 'Show inlay type hints for variables', self::PARAM_INLAY_HINTS_PARAMS => 'Show inlay hints for parameters']);
    }
    private function registerSourceLocator(ContainerBuilder $container) : void
    {
        ...
        $container->register(WorkspaceIndex::class, function (Container $container) {
            return new WorkspaceIndex(ReflectorBuilder::create()->build(), $container->parameter(self::PARAM_UPDATE_INTERVAL)->int());
        });
        $container->register(WorseDiagnosticProvider::class, function (Container $container) {
            return new WorseDiagnosticProvider($container->expect(WorseReflectionExtension::SERVICE_REFLECTOR, Reflector::class));
        }, [LanguageServerExtension::TAG_DIAGNOSTICS_PROVIDER => DiagnosticProviderTag::create('code-action', \true)]);
        $container->register(InlayHintHandler::class, function (Container $container) {
            ...
            return new InlayHintHandler(new InlayHintProvider($container->expect(WorseReflectionExtension::SERVICE_REFLECTOR, SourceCodeReflector::class), new InlayHintOptions($container->parameter(self::PARAM_INLAY_HINTS_TYPES)->bool(), $container->parameter(self::PARAM_INLAY_HINTS_PARAMS)->bool())), $container->expect(LanguageServerExtension::SERVICE_SESSION_WORKSPACE, Workspace::class));
