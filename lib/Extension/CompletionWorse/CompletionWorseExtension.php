...
namespace Phpactor202301\Phpactor\Extension\CompletionWorse;

use Closure;
use Phpactor202301\Phpactor\Completion\Bridge\TolerantParser\DebugTolerantCompletor;
use Phpactor202301\Phpactor\Completion\Bridge\TolerantParser\LimitingCompletor;
use Phpactor202301\Phpactor\Completion\Bridge\TolerantParser\ReferenceFinder\AttributeCompletor;
use Phpactor202301\Phpactor\Completion\Bridge\TolerantParser\ReferenceFinder\ClassLikeCompletor;
use Phpactor202301\Phpactor\Completion\Bridge\TolerantParser\ReferenceFinder\ExpressionNameCompletor;
use Phpactor202301\Phpactor\Completion\Bridge\TolerantParser\ReferenceFinder\TypeCompletor;
use Phpactor202301\Phpactor\Completion\Bridge\TolerantParser\ReferenceFinder\UseNameCompletor;
...
use Phpactor202301\Phpactor\Completion\Bridge\TolerantParser\WorseReflection\KeywordCompletor;
use Phpactor202301\Phpactor\Completion\Bridge\TolerantParser\WorseReflection\ImportedNameCompletor;
use Phpactor202301\Phpactor\Completion\Bridge\TolerantParser\WorseReflection\WorseConstantCompletor;
...
use Phpactor202301\Phpactor\Completion\Bridge\TolerantParser\WorseReflection\WorseSignatureHelper;
...
use Phpactor202301\Phpactor\Completion\Bridge\TolerantParser\WorseReflection\WorseFunctionCompletor;
use Phpactor202301\Phpactor\Completion\Bridge\TolerantParser\WorseReflection\WorseParameterCompletor;
use Phpactor202301\Phpactor\Completion\Bridge\TolerantParser\WorseReflection\WorseNamedParameterCompletor;
...
EOT
    ...
    private function registerCompletion(ContainerBuilder $container) : void
    {
        foreach ($this->getTolerantCompletors() as $name => [$_, $completor]) {
            $container->register(\sprintf('worse_completion.completor.%s', $name), $completor, [self::TAG_TOLERANT_COMPLETOR => ['name' => $name]]);
        }
        foreach ($this->getOtherCompletors() as $name => [$_, $completor]) {
            $container->register(\sprintf('worse_completion.completor.%s', $name), $completor, [CompletionExtension::TAG_COMPLETOR => ['name' => $name]]);
        }
        $container->register(SuggestionDocumentor::class, function (Container $container) {
            return new WorseSuggestionDocumentor($container->get(WorseReflectionExtension::SERVICE_REFLECTOR), $container->get(ObjectRendererExtension::SERVICE_MARKDOWN_RENDERER));
        });
        $container->register(TypeSuggestionProvider::class, function (Container $container) {
            return new TypeSuggestionProvider($container->get(NameSearcher::class));
        });
        $container->register(ChainTolerantCompletor::class, function (Container $container) {
            ...
            }, $container->get(self::SERVICE_COMPLETOR_MAP))), $container->get('worse_reflection.tolerant_parser'));
        ...
        $container->register(self::SERVICE_COMPLETOR_MAP, function (Container $container) {
            ...
            foreach ($container->getServiceIdsForTag(self::TAG_TOLERANT_COMPLETOR) as $serviceId => $attrs) {
                if (!isset($attrs['name'])) {
                    throw new RuntimeException(\sprintf('Completor "%s" must declare an "name" attribute', $serviceId));
                }
                $name = $attrs['name'];
                if (isset($completors[$name])) {
                    throw new RuntimeException(\sprintf('Completor name "%s" (service ID "%s") already registered', $name, $serviceId));
                }
        ...
        $container->register('completion_worse.short_desc.formatters', function (Container $container) {
            return [new TypeFormatter(), new MethodFormatter(), new ParameterFormatter(), new ParametersFormatter(), new ClassFormatter(), new PropertyFormatter(), new FunctionFormatter(), new VariableFormatter(), new InterfaceFormatter(), new TraitFormatter(), new ConstantFormatter(), new EnumCaseFormatter()];
    ...
    {
        return ['doctrine_annotation' => ['Completion for annotations provided by the Doctrine annotation library', function (Container $container) {
            return new DoctrineAnnotationCompletor($container->get(NameSearcher::class), $container->get(WorseReflectionExtension::SERVICE_REFLECTOR), $container->get(WorseReflectionExtension::SERVICE_PARSER));
        }]];
    }
    /**
     * @return array<string,array{string,Closure(Container): mixed}>
    ...
    {
        return ['imported_names' => ['Completion for names imported into the current namespace', function (Container $container) {
            return new ImportedNameCompletor();
        }], 'worse_parameter' => ['Completion for method or function parameters', function (Container $container) {
            return new WorseParameterCompletor($container->get(WorseReflectionExtension::SERVICE_REFLECTOR), $container->get(CompletionExtension::SERVICE_SHORT_DESC_FORMATTER));
        }], 'named_parameter' => ['Completion for named parameters', function (Container $container) {
            return new WorseNamedParameterCompletor($container->get(WorseReflectionExtension::SERVICE_REFLECTOR), $container->get(CompletionExtension::SERVICE_SHORT_DESC_FORMATTER));
        }], 'constructor' => ['Completion for constructors', function (Container $container) {
            return new WorseConstructorCompletor($container->get(WorseReflectionExtension::SERVICE_REFLECTOR), $container->get(CompletionExtension::SERVICE_SHORT_DESC_FORMATTER));
        }], 'class_member' => ['Completion for class members', function (Container $container) {
            return new WorseClassMemberCompletor($container->get(WorseReflectionExtension::SERVICE_REFLECTOR), $container->get(CompletionExtension::SERVICE_SHORT_DESC_FORMATTER), $container->get(CompletionExtension::SERVICE_SNIPPET_FORMATTER), $container->get(ObjectRendererExtension::SERVICE_MARKDOWN_RENDERER));
        }], 'scf_class' => ['Brute force completion for class names (not recommended)', function (Container $container) {
            return $this->limitCompletor($container, new ScfClassCompletor($container->get(SourceCodeFilesystemExtension::SERVICE_REGISTRY)->get('composer'), $container->get('class_to_file.file_to_class')));
        }], 'local_variable' => ['Completion for local variables', function (Container $container) {
            return new WorseLocalVariableCompletor(new VariableCompletionHelper($container->get(WorseReflectionExtension::SERVICE_REFLECTOR)), $container->get(CompletionExtension::SERVICE_SHORT_DESC_FORMATTER));
        }], 'declared_function' => ['Completion for functions defined in the Phpactor runtime', function (Container $container) {
            return new WorseFunctionCompletor($container->get(WorseReflectionExtension::SERVICE_REFLECTOR), $container->get(CompletionExtension::SERVICE_SHORT_DESC_FORMATTER), $container->get(CompletionExtension::SERVICE_SNIPPET_FORMATTER));
        }], 'declared_constant' => ['Completion for constants defined in the Phpactor runtime', function (Container $container) {
            return new WorseConstantCompletor();
        }], 'declared_class' => ['Completion for classes defined in the Phpactor runtime', function (Container $container) {
            return new WorseDeclaredClassCompletor($container->get(WorseReflectionExtension::SERVICE_REFLECTOR), $container->get(CompletionExtension::SERVICE_SHORT_DESC_FORMATTER));
        }], 'expression_name_search' => ['Completion for class names, constants and functions at expression positions that are located in the index', function (Container $container) {
            return $this->limitCompletor($container, new ExpressionNameCompletor($container->get(NameSearcher::class), new ObjectFormatter($container->get(self::SERVICE_COMPLETION_WORSE_SNIPPET_FORMATTERS)), $container->get(DocumentPrioritizer::class)));
        }], 'use' => ['Completion for use imports', function (Container $container) {
            return $this->limitCompletor($container, new UseNameCompletor($container->get(NameSearcher::class), $container->get(DocumentPrioritizer::class)));
        }], 'attribute' => ['Completion for attribute class names', function (Container $container) {
            return $this->limitCompletor($container, new AttributeCompletor($container->get(NameSearcher::class), $container->get(DocumentPrioritizer::class)));
        }], 'class_like' => ['Completion for class like contexts', function (Container $container) {
            return $this->limitCompletor($container, new ClassLikeCompletor($container->get(NameSearcher::class), $container->get(DocumentPrioritizer::class)));
        }], 'type' => ['Completion for types', function (Container $container) {
            return $this->limitCompletor($container, new TypeCompletor($container->get(TypeSuggestionProvider::class)));
        }], 'keyword' => ['Completion for keywords (not very accurate)', function (Container $container) {
            return new KeywordCompletor();
        }], 'docblock' => ['Docblock completion', function (Container $container) {
            return new DocblockCompletor($container->get(TypeSuggestionProvider::class), $container->get(WorseReflectionExtension::SERVICE_PARSER));
        }]];
    }
    private function registerSignatureHelper(ContainerBuilder $container) : void
    {
        $container->register('completion_worse.signature_helper', function (Container $container) {
            return new WorseSignatureHelper($container->get(WorseReflectionExtension::SERVICE_REFLECTOR), $container->get(CompletionExtension::SERVICE_SHORT_DESC_FORMATTER));
...
\class_alias('Phpactor202301\\Phpactor\\Extension\\CompletionWorse\\CompletionWorseExtension', 'Phpactor\\Extension\\CompletionWorse\\CompletionWorseExtension', \false);
