...
final class OffsetExtractorResult
{
    ...
    public function offsets(string $name = null) : array
    {
        ...
        if (!isset($this->offsets[$name])) {
            throw new RuntimeException(\sprintf('No offset registered with name "%s", known names "%s"', $name, \implode('", "', \array_keys($this->offsets))));
        }
    ...
    public function offset(string $name = null) : ByteOffset
    {
        ...
        if (!\count($offsets)) {
            throw new RuntimeException(\sprintf('No "%s" offsets found in source code', $name));
        }
    ...
    public function ranges(string $name = null) : array
    {
        ...
        if (!isset($this->ranges[$name])) {
            throw new RuntimeException(\sprintf('No range registered with name "%s", known names "%s"', $name, \implode('", "', \array_keys($this->ranges))));
        }
    ...
    public function range(string $name = null) : ByteOffsetRange
    {
        ...
        if (!\count($ranges)) {
            throw new RuntimeException(\sprintf('No "%s" ranges found in source code', $name));
        }
        $range = \reset($ranges);
        return $range;
    }
}
\class_alias('Phpactor202301\\Phpactor\\Extension\\LanguageServerRename\\Tests\\Util\\OffsetExtractorResult', 'Phpactor\\Extension\\LanguageServerRename\\Tests\\Util\\OffsetExtractorResult', \false);
