...
namespace Phpactor202301\Phpactor\Extension\LanguageServerRename\Tests\Util;
...
class OffsetExtractorTest extends TestCase
{
    ...
    {
        $extractor = OffsetExtractor::create()->registerOffset('selection', '<>')->parse('Test string with<> selector');
    ...
    {
        $extractor = OffsetExtractor::create()->registerOffset('selection', '<>')->parse('Test string 🐱   <> selector');
        $selection = $extractor->offset('selection');
        $newSource = $extractor->source();
    ...
    public function testExceptionWhenNoOffsetIsFound() : void
    {
        ...
        $extractor = OffsetExtractor::create()->registerOffset('selection', '<>')->parse('Test string without selector');
    ...
    public function testExceptionWhenNoOffsetIsRegistered() : void
    {
        ...
        $extractor = OffsetExtractor::create()->parse('Test string without selector');
        $extractor->offset('selection');
    }
    public function testOffsets() : void
    {
        $extractor = OffsetExtractor::create()->registerOffset('selection', '<>')->parse('Test string with<> two select<>ors');
        $selection = $extractor->offsets('selection');
        $newSource = $extractor->source();
        $this->assertEquals([ByteOffset::fromInt(16), ByteOffset::fromInt(27)], $selection);
    ...
    {
        $extractor = OffsetExtractor::create()->registerRange('textEdit', '{{', '}}')->parse('Test string {{with}} selector');
    ...
    {
        $extractor = OffsetExtractor::create()->registerRange('textEdit', '{{', '}}')->parse('Test string {{with}} two {{selectors}}');
        $textEdit = $extractor->ranges('textEdit');
        $newSource = $extractor->source();
        $this->assertEquals([ByteOffsetRange::fromInts(12, 16), ByteOffsetRange::fromInts(21, 30)], $textEdit);
        $this->assertEquals('Test string with two selectors', $newSource);
    }
    ...
    public function testExceptionWhenNoRangeIsFound() : void
    {
        ...
        $extractor = OffsetExtractor::create()->registerRange('selection', '<', '>')->parse('Test string without selector');
...
\class_alias('Phpactor202301\\Phpactor\\Extension\\LanguageServerRename\\Tests\\Util\\OffsetExtractorTest', 'Phpactor\\Extension\\LanguageServerRename\\Tests\\Util\\OffsetExtractorTest', \false);
