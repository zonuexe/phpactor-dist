...
use Phpactor202301\Phpactor\LanguageServerProtocol\TextEdit as PhpactorTextEdit;
...
class RenameHandlerTest extends IntegrationTestCase
{
    ...
    public function testPrepareRenameReturnsNullIfItCouldNotPrepareAnything() : void
    {
        ...
        $response = $this->tester->requestAndWait(PrepareRenameRequest::METHOD, new PrepareRenameParams(ProtocolFactory::textDocumentIdentifier(self::EXAMPLE_FILE), ProtocolFactory::position(0, 0)));
    ...
    public function testRename() : void
    {
        ...
        $this->bootContainerWithRangeAndResults(ByteOffsetRange::fromInts(0, 0), [new LocatedTextEdit($expectedUri, TextEdit::create(ByteOffset::fromInt(1), 0, self::EXAMPLE_NEW_NAME))]);
        $this->tester->textDocument()->open(self::EXAMPLE_FILE, '<?php');
        $response = $this->tester->requestAndWait(RenameRequest::METHOD, new RenameParams(ProtocolFactory::textDocumentIdentifier(self::EXAMPLE_FILE), ProtocolFactory::position(0, 0), self::EXAMPLE_NEW_NAME));
        $this->tester->assertSuccess($response);
    ...
    {
        $container = $this->container(['range' => $range, 'results' => $results]);
        $this->tester = $container->get(LanguageServerBuilder::class)->tester(ProtocolFactory::initializeParams($this->workspace()->path()));
        $this->renamer = $container->get(InMemoryRenamer::class);
    }
}
\class_alias('Phpactor202301\\Phpactor\\Extension\\LanguageServerRename\\Tests\\Unit\\Handler\\RenameHandlerTest', 'Phpactor\\Extension\\LanguageServerRename\\Tests\\Unit\\Handler\\RenameHandlerTest', \false);
