...
class ContextMenuHandler implements Handler
{
    ...
    public function __construct(private Reflector $reflector, private InterestingOffsetFinder $offsetFinder, private ClassFileNormalizer $classFileNormalizer, private ContextMenu $menu, private Container $container)
    {
    }
    ...
    {
        $resolver->setRequired([self::PARAMETER_SOURCE, self::PARAMETER_OFFSET]);
        $resolver->setDefaults([self::PARAMETER_ACTION => null, self::PARAMETER_CURRENT_PATH => null]);
    }
    ...
    private function resolveAction(ReflectionOffset $offset, Symbol $symbol, array $arguments)
    {
        if (\false === $this->menu->hasContext($symbol->symbolType())) {
            return EchoResponse::fromMessage(\sprintf('No context actions available for symbol type "%s"', $symbol->symbolType()));
        }
