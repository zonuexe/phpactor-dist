...
class ContextMenu
{
    ...
    public function forContext(string $context) : array
    {
        if (!isset($this->contexts[$context])) {
            throw new RuntimeException(\sprintf('Context "%s" does not exist', $context));
        }
        ...
        foreach ($this->contexts as $name => $actions) {
            ...
            foreach ($actions as $actionName) {
                if (!isset($this->actions[$actionName])) {
                    throw new RuntimeException(\sprintf('Action "%s" used in context "%s" does not exist, known actions: "%s"', $actionName, $name, \implode('", "', \array_keys($this->actions))));
                }
                $action = $this->actions[$actionName];
                if (isset($keys[$action->key()])) {
                    throw new RuntimeException(\sprintf('Key "%s" in context "%s" mapped by action "%s" is already used by action "%s"', $action->key(), $name, $actionName, $keys[$action->key()]));
                }
