...
abstract class AbstractClassGenerateHandler extends AbstractHandler
{
    ...
    {
        $resolver->setDefaults([self::PARAM_NEW_PATH => null, self::PARAM_VARIANT => null, self::PARAM_OVERWRITE_EXISTING => null]);
    ...
    public function handle(array $arguments)
    {
        ...
        if (null === $arguments[self::PARAM_VARIANT]) {
            $this->requireInput(ChoiceInput::fromNameLabelChoicesAndDefault(self::PARAM_VARIANT, 'Variant: ', (array) \array_combine($this->generators->names(), $this->generators->names())));
        }
        ...
        if ($arguments[self::PARAM_NEW_PATH] && null === $arguments[self::PARAM_OVERWRITE_EXISTING] && \file_exists($arguments[self::PARAM_NEW_PATH]) && 0 !== \filesize($arguments[self::PARAM_NEW_PATH])) {
            $this->requireInput(ConfirmInput::fromNameAndLabel(self::PARAM_OVERWRITE_EXISTING, 'File exists and is not empty, overwrite?'));
        }
    ...
    private function writeFileContents(array $arguments, SourceCode $code) : void
    {
        ...
        if (!\file_exists($dirName)) {
            if (!@\mkdir($dirName, 0777, \true)) {
                throw new RuntimeException(\sprintf('Could not create directory at "%s"', $dirName));
            }
        }
        if (!\file_put_contents($newPath, (string) $code)) {
            throw new RuntimeException(\sprintf('Could not save file contents to "%s"', $newPath));
        }
    }
}
