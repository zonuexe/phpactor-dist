...
namespace Phpactor202301\Phpactor\Extension\CodeTransform;
...
use Phpactor202301\Phpactor\CodeBuilder\Adapter\WorseReflection\TypeRenderer\WorseTypeRenderer80;
...
use Phpactor202301\Phpactor\CodeBuilder\Adapter\WorseReflection\TypeRenderer\WorseTypeRendererFactory;
...
use Phpactor202301\Phpactor\CodeBuilder\Adapter\Twig\TwigExtension;
...
use Phpactor202301\Phpactor\CodeTransform\Adapter\TolerantParser\ClassToFile\Transformer\ClassNameFixerTransformer;
...
use Phpactor202301\Phpactor\CodeTransform\Adapter\TolerantParser\Refactor\TolerantExtractExpression;
use Phpactor202301\Phpactor\CodeTransform\Adapter\WorseReflection\GenerateFromExisting\InterfaceFromExistingGenerator;
use Phpactor202301\Phpactor\CodeTransform\Adapter\TolerantParser\Refactor\TolerantRenameVariable;
use Phpactor202301\Phpactor\CodeTransform\Adapter\WorseReflection\Helper\WorseMissingMethodFinder;
use Phpactor202301\Phpactor\CodeTransform\Adapter\WorseReflection\Refactor\WorseExtractMethod;
use Phpactor202301\Phpactor\CodeTransform\Adapter\WorseReflection\Refactor\WorseFillObject;
use Phpactor202301\Phpactor\CodeTransform\Adapter\WorseReflection\Refactor\WorseGenerateConstructor;
use Phpactor202301\Phpactor\CodeTransform\Adapter\WorseReflection\Refactor\WorseGenerateMutator;
...
use Phpactor202301\Phpactor\CodeTransform\Adapter\WorseReflection\Refactor\WorseOverrideMethod;
use Phpactor202301\Phpactor\CodeTransform\Adapter\WorseReflection\Helper\WorseInterestingOffsetFinder;
use Phpactor202301\Phpactor\CodeTransform\Adapter\WorseReflection\Refactor\WorseGenerateAccessor;
use Phpactor202301\Phpactor\CodeTransform\Adapter\WorseReflection\Refactor\WorseGenerateMethod;
use Phpactor202301\Phpactor\CodeTransform\Adapter\WorseReflection\Transformer\AddMissingProperties;
use Phpactor202301\Phpactor\CodeTransform\Adapter\WorseReflection\Refactor\WorseExtractConstant;
...
class CodeTransformExtension implements Extension
{
    ...
    {
        $schema->setDefaults([self::PARAM_NEW_CLASS_VARIANTS => [], self::PARAM_TEMPLATE_PATHS => [
            // Ordered by priority
            '%project_config%/templates',
            '%config%/templates',
        ], self::PARAM_INDENTATION => '    ', self::PARAM_GENERATE_ACCESSOR_PREFIX => '', self::PARAM_GENERATE_ACCESSOR_UPPER_CASE_FIRST => \false, self::PARAM_GENERATE_MUTATOR_PREFIX => 'set', self::PARAM_GENERATE_MUTATOR_UPPER_CASE_FIRST => \true, self::PARAM_GENERATE_MUTATOR_FLUENT => \false, self::PARAM_IMPORT_GLOBALS => \false, self::PARAM_OBJECT_FILL_HINT => \true, self::PARAM_OBJECT_FILL_NAMED => \true]);
        $schema->setDescriptions([self::PARAM_NEW_CLASS_VARIANTS => 'Variants which should be suggested when class-create is invoked', self::PARAM_TEMPLATE_PATHS => 'Paths in which to look for code templates', self::PARAM_INDENTATION => 'Indentation chars to use in code generation and transformation', self::PARAM_GENERATE_ACCESSOR_PREFIX => 'Prefix to use for generated accessors', self::PARAM_GENERATE_ACCESSOR_UPPER_CASE_FIRST => 'If the first letter of a generated accessor should be made uppercase', self::PARAM_GENERATE_MUTATOR_PREFIX => 'Prefix to use for generated mutators', self::PARAM_GENERATE_MUTATOR_UPPER_CASE_FIRST => 'If the first letter of a generated mutator should be made uppercase', self::PARAM_GENERATE_MUTATOR_FLUENT => 'If the mutator should be fluent', self::PARAM_IMPORT_GLOBALS => 'Import functions even if they are in the global namespace', self::PARAM_OBJECT_FILL_NAMED => 'Object fill refactoring: use named parameters', self::PARAM_OBJECT_FILL_HINT => 'Object fill refactoring: show hint as a comment']);
    }
    ...
    private function registerGeneratorImplementations(ContainerBuilder $container) : void
    {
        $container->register('code_transform_extra.class_generator.variants', function (Container $container) {
            $generators = ['default' => new ClassGenerator($container->get('code_transform.renderer')), 'interface' => new ClassGenerator($container->get('code_transform.renderer'), 'interface'), 'trait' => new ClassGenerator($container->get('code_transform.renderer'), 'trait'), 'enum' => new ClassGenerator($container->get('code_transform.renderer'), 'enum')];
        ...
        }, [CodeTransformExtension::TAG_FROM_EXISTING_GENERATOR => ['name' => 'interface']]);
    }
    ...
    private function registerRenderer(ContainerBuilder $container) : void
    {
        ...
        $container->register('code_transform.renderer', function (Container $container) {
            $twig = new Environment($container->get('code_transform.twig_loader'), ['strict_variables' => \true, 'autoescape' => \false]);
            $renderer = new TwigRenderer($twig);
            $twig->addExtension(new TwigExtension($container->get(TextFormat::class), $container->get(WorseTypeRenderer::class)));
            return $renderer;
        });
    ...
    {
        if (!isset($attrs['name'])) {
            throw new RuntimeException(\sprintf('Generator "%s" must be registered with the "name" tag', $serviceId));
        }
    }
    private function registerRpc(ContainerBuilder $container) : void
    {
        $container->register('code_transform.rpc.handler.class_inflect', function (Container $container) {
            return new ClassInflectHandler($container->get(self::SERVICE_CLASS_INFLECTORS), $container->get(ClassToFileExtension::SERVICE_CONVERTER));
        ...
        $container->register('code_transform.rpc.handler.class_new', function (Container $container) {
            return new ClassNewHandler($container->get(self::SERVICE_CLASS_GENERATORS), $container->get(ClassToFileExtension::SERVICE_CONVERTER));
        ...
        $container->register('code_transform.rpc.handler.transform', function (Container $container) {
            return new TransformHandler($container->get(CodeTransform::class));
...
\class_alias('Phpactor202301\\Phpactor\\Extension\\CodeTransform\\CodeTransformExtension', 'Phpactor\\Extension\\CodeTransform\\CodeTransformExtension', \false);
