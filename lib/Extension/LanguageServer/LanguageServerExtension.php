...
EOT
, self::PARAM_DIAGNOSTIC_SLEEP_TIME => 'Amount of time to wait before analyzing the code again for diagnostics', self::PARAM_DIAGNOSTIC_ON_UPDATE => 'Perform diagnostics when the text document is updated', self::PARAM_DIAGNOSTIC_ON_SAVE => 'Perform diagnostics when the text document is saved', self::PARAM_DIAGNOSTIC_ON_OPEN => 'Perform diagnostics when opening a text document', self::PARAM_DIAGNOSTIC_PROVIDERS => 'Specify which diagnostic providers should be active (default to all)', self::PARAM_FILE_EVENTS => 'Register to recieve file events', self::PARAM_SHUTDOWN_GRACE_PERIOD => 'Amount of time to wait before responding to a shutdown notification', self::PARAM_SELF_DESTRUCT_TIMEOUT => 'Wait this amount of time after a shutdown request before self-destructing']);
    }
    ...
    private function registerSession(ContainerBuilder $container) : void
    {
        ...
        $container->register(DidChangeWatchedFilesListener::class, function (Container $container) {
            return new DidChangeWatchedFilesListener($container->get(ClientApi::class), $container->getParameter(self::PARAM_FILE_EVENT_GLOBS), $container->get(ClientCapabilities::class));
        }, [self::TAG_LISTENER_PROVIDER => []]);
    ...
    private function registerHandlers(ContainerBuilder $container) : void
    {
        ...
        $container->register(CodeActionHandler::class, function (Container $container) {
            return new CodeActionHandler(new AggregateCodeActionProvider(...$this->taggedServices($container, self::TAG_CODE_ACTION_PROVIDER)), $container->get(self::SERVICE_SESSION_WORKSPACE));
        }, [self::TAG_METHOD_HANDLER => []]);
    ...
    private function registerServices(ContainerBuilder $container) : void
    {
        $container->register(DiagnosticsService::class, function (Container $container) {
            return new DiagnosticsService($container->get(DiagnosticsEngine::class), $container->getParameter(self::PARAM_DIAGNOSTIC_ON_UPDATE), $container->getParameter(self::PARAM_DIAGNOSTIC_ON_SAVE), $container->get(self::SERVICE_SESSION_WORKSPACE), \true, $container->getParameter(self::PARAM_DIAGNOSTIC_ON_OPEN));
    ...
    private function registerDiagnostics(ContainerBuilder $container) : void
    {
        $container->register(DiagnosticsEngine::class, function (Container $container) {
            return new DiagnosticsEngine($container->get(ClientApi::class), $container->get(AggregateDiagnosticsProvider::class), $container->getParameter(self::PARAM_DIAGNOSTIC_SLEEP_TIME));
        });
        ...
        $container->register(CodeActionDiagnosticsProvider::class, function (Container $container) {
            return new CodeActionDiagnosticsProvider(...$this->taggedServices($container, self::TAG_CODE_ACTION_DIAGNOSTICS_PROVIDER));
        }, [self::TAG_DIAGNOSTICS_PROVIDER => ['name' => 'code-action']]);
    }
    /**
