...
class LanguageServerExtension implements Extension
{
    ...
    {
        $schema->setDefaults([self::PARAM_CATCH_ERRORS => \true, self::PARAM_ENABLE_WORKPACE => \true, self::PARAM_SESSION_PARAMETERS => [], self::PARAM_METHOD_ALIAS_MAP => [], self::PARAM_DIAGNOSTIC_SLEEP_TIME => 1000, self::PARAM_DIAGNOSTIC_ON_UPDATE => \true, self::PARAM_DIAGNOSTIC_ON_SAVE => \true, self::PARAM_DIAGNOSTIC_ON_OPEN => \true, self::PARAM_DIAGNOSTIC_PROVIDERS => null, self::PARAM_DIAGNOSTIC_OUTSOURCE => \true, self::PARAM_FILE_EVENTS => \true, self::PARAM_FILE_EVENT_GLOBS => ['**/*.php'], self::PARAM_PROFILE => \false, self::PARAM_TRACE => \false, self::PARAM_SHUTDOWN_GRACE_PERIOD => 200, self::PARAM_SELF_DESTRUCT_TIMEOUT => 2500, self::PARAM_DIAGNOSTIC_OUTSOURCE_TIMEOUT => 5]);
        $schema->setDescriptions([self::PARAM_TRACE => 'Log incoming and outgoing messages (needs log formatter to be set to ``json``)', self::PARAM_PROFILE => 'Logs timing information for incoming LSP requests', self::PARAM_METHOD_ALIAS_MAP => 'Allow method names to be re-mapped. Useful for maintaining backwards compatibility', self::PARAM_SESSION_PARAMETERS => 'Phpactor parameters (config) that apply only to the language server session', self::PARAM_ENABLE_WORKPACE => <<<'EOT'
...
, self::PARAM_DIAGNOSTIC_SLEEP_TIME => 'Amount of time to wait before analyzing the code again for diagnostics', self::PARAM_DIAGNOSTIC_ON_UPDATE => 'Perform diagnostics when the text document is updated', self::PARAM_DIAGNOSTIC_ON_SAVE => 'Perform diagnostics when the text document is saved', self::PARAM_DIAGNOSTIC_ON_OPEN => 'Perform diagnostics when opening a text document', self::PARAM_DIAGNOSTIC_PROVIDERS => 'Specify which diagnostic providers should be active (default to all)', self::PARAM_DIAGNOSTIC_OUTSOURCE => 'If applicable diagnostics should be "outsourced" to a different process', self::PARAM_DIAGNOSTIC_OUTSOURCE_TIMEOUT => 'Kill the diagnostics process if it outlives this timeout', self::PARAM_FILE_EVENTS => 'Register to receive file events', self::PARAM_SHUTDOWN_GRACE_PERIOD => 'Amount of time (in milliseconds) to wait before responding to a shutdown notification', self::PARAM_SELF_DESTRUCT_TIMEOUT => 'Wait this amount of time (in milliseconds) after a shutdown request before self-destructing']);
    ...
    private function registerServer(ContainerBuilder $container) : void
    {
        ...
        $container->register(LanguageServerBuilder::class, function (Container $container) {
            $builder = LanguageServerBuilder::create(new PhpactorDispatcherFactory($container), $this->logger($container));
            return $builder;
        });
        $container->register(ClientLogger::class, function (Container $container) {
            return new ClientLogger($container->get(ClientApi::class), $this->logger($container));
        });
    }
    ...
    private function registerSession(ContainerBuilder $container) : void
    {
        ...
        $container->register(InvalidConfigListener::class, function (Container $container) {
            return new InvalidConfigListener($container->get(ClientApi::class), $container->has(ResolverErrors::class) ? $container->get(ResolverErrors::class) : new ResolverErrors([]));
        }, [self::TAG_LISTENER_PROVIDER => []]);
        ...
        $container->register('language_server.session.handler.session', function (Container $container) {
            ...
            return new DebugHandler($container, $container->get(ClientApi::class), $container->get(self::SERVICE_SESSION_WORKSPACE), $container->get(ServerStats::class), $container->get(ServiceManager::class), $container->get(AggregateDiagnosticsProvider::class), $providers);
    ...
    private function registerEventDispatcher(ContainerBuilder $container) : void
    {
        $container->register(EventDispatcherInterface::class, function (Container $container) {
            $aggregate = new LazyAggregateProvider($container, $this->resolveListeners($container));
    ...
    private function registerCommandDispatcher(ContainerBuilder $container) : void
    {
        $container->register(CommandDispatcher::class, function (Container $container) {
            ...
            foreach ($container->getServiceIdsForTag(self::TAG_COMMAND) as $serviceId => $attrs) {
                if (!isset($attrs['name'])) {
                    throw new RuntimeException(\sprintf('Cannot register command with service ID "%s" Each command must define a "name" attribute', $serviceId));
                }
    ...
    private function registerServiceManager(ContainerBuilder $container) : void
    {
        ...
        $container->register(ServiceManager::class, function (Container $container) {
            return new ServiceManager($container->get(ServiceProviders::class), $container->get(ClientLogger::class));
        });
        $container->register(ServiceProviders::class, function (Container $container) {
            ...
            foreach ($container->getServiceIdsForTag(self::TAG_SERVICE_PROVIDER) as $serviceId => $attrs) {
                ...
                if (!$provider instanceof ServiceProvider) {
                    throw new RuntimeException(\sprintf('Tagged service provider "%s" does not implement ServiceProvider interface, is a "%s"', $serviceId, \is_object($provider) ? \get_class($provider) : \gettype($provider)));
                }
                $providers[] = $provider;
            }
    ...
    private function registerMiddleware(ContainerBuilder $container) : void
    {
        $container->register(MiddlewareDispatcher::class, function (Container $container) {
            ...
            $stack[] = new InitializeMiddleware($container->get(Handlers::class), $container->get(EventDispatcherInterface::class), $this->serverInfo());
            ...
            $stack[] = new HandlerMiddleware($container->get(MethodRunner::class));
    ...
    private function registerHandlers(ContainerBuilder $container) : void
    {
        $container->register(ArgumentResolver::class, function (Container $container) {
            return new ChainArgumentResolver(new LanguageSeverProtocolParamsResolver(), new DTLArgumentResolver());
        });
        $container->register(MethodRunner::class, function (Container $container) {
            return new HandlerMethodRunner($container->get(Handlers::class), $container->get(ArgumentResolver::class), $this->logger($container));
        });
        ...
        $container->register(StatsHandler::class, function (Container $container) {
            return new StatsHandler($container->get(ClientApi::class), $container->get(ServerStats::class));
        }, [self::TAG_METHOD_HANDLER => []]);
        $container->register(CodeActionHandler::class, function (Container $container) {
            ...
            if ($container->parameter(self::PARAM_PROFILE)->bool()) {
                $services = \array_map(fn(CodeActionProvider $provider) => new ProfilingCodeActionProvider($provider, $this->logger($container)), $services);
            }
        ...
        $container->register(FormattingHandler::class, function (Container $container) {
            ...
            return new FormattingHandler($container->get(self::SERVICE_SESSION_WORKSPACE), $formatter, $container->get(ProgressNotifier::class));
    ...
    private function registerServices(ContainerBuilder $container) : void
    {
        $container->register(DiagnosticsService::class, function (Container $container) {
            return new DiagnosticsService($container->get(DiagnosticsEngine::class), $container->parameter(self::PARAM_DIAGNOSTIC_ON_UPDATE)->bool(), $container->parameter(self::PARAM_DIAGNOSTIC_ON_SAVE)->bool(), $container->get(self::SERVICE_SESSION_WORKSPACE), \true, $container->parameter(self::PARAM_DIAGNOSTIC_ON_OPEN)->bool());
        }, [self::TAG_SERVICE_PROVIDER => [], self::TAG_LISTENER_PROVIDER => []]);
    }
    private function registerDiagnostics(ContainerBuilder $container) : void
    {
        $container->register(DiagnosticsEngine::class, function (Container $container) {
            return new DiagnosticsEngine($container->get(ClientApi::class), $container->get(AggregateDiagnosticsProvider::class), $container->parameter(self::PARAM_DIAGNOSTIC_SLEEP_TIME)->int());
        });
        $container->register(AggregateDiagnosticsProvider::class, function (Container $container) {
            $providers = $this->collectDiagnosticProviders($container, outsourced: $container->parameter(self::PARAM_DIAGNOSTIC_OUTSOURCE)->bool() ? \false : null);
            return new AggregateDiagnosticsProvider($this->logger($container, 'LSPDIAG'), ...\array_values($providers));
        });
        $container->register(AggregateDiagnosticsProvider::class . '.outsourced', function (Container $container) {
            $providers = $this->collectDiagnosticProviders($container, \true);
            return new AggregateDiagnosticsProvider($this->logger($container, 'OUTLSPDIAG'), ...\array_values($providers));
        });
        $container->register(CodeActionDiagnosticsProvider::class, function (Container $container) {
            return new CodeActionDiagnosticsProvider(...$this->taggedServices($container, self::TAG_CODE_ACTION_DIAGNOSTICS_PROVIDER, CodeActionProvider::class));
        }, [self::TAG_DIAGNOSTICS_PROVIDER => DiagnosticProviderTag::create('code-action', outsource: \true)]);
        $container->register(OutsourcedDiagnosticsProvider::class, function (Container $container) {
            ...
            return new OutsourcedDiagnosticsProvider([__DIR__ . '/../../../bin/phpactor', 'language-server:diagnostics'], $projectPath, $this->logger($container), $container->parameter(self::PARAM_DIAGNOSTIC_OUTSOURCE_TIMEOUT)->int());
    ...
    private function collectDiagnosticProviders(Container $container, ?bool $outsourced) : array
    {
        ...
        if (null !== $enabled) {
            ...
            if ($diff = \array_diff($enabled, \array_keys($providers))) {
                throw new RuntimeException(\sprintf('Unknown diagnostic provider(s) "%s", known providers: "%s"', \implode('", "', $diff), \implode('", "', \array_keys($providers))));
            }
