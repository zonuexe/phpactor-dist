    ...
    public function load(ContainerBuilder $container) : void
    {
        $container->register('language_server_psalm.suggest', function (Container $container) {
            return new PhpactorComposerSuggestor($container->expect(ConfigurationExtension::SERVICE_PHPACTOR_CONFIG_LOCAL, JsonConfig::class), $container->get(ComposerInspector::class), function (JsonConfig $config, ComposerInspector $inspector) {
                if ($config->has(\Phpactor\Extension\LanguageServerPsalm\LanguageServerPsalmExtension::PARAM_ENABLED)) {
                    return Changes::none();
                }
                if (!$inspector->package('vimeo/psalm')) {
                    return Changes::none();
                }
                return Changes::from([new PhpactorConfigChange('Psalm detected, enable the Psalm extension?', function (bool $enable) {
                    return [\Phpactor\Extension\LanguageServerPsalm\LanguageServerPsalmExtension::PARAM_ENABLED => $enable];
                })]);
            });
