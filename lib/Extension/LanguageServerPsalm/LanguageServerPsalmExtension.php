...
class LanguageServerPsalmExtension implements OptionalExtension
{
    ...
    public function load(ContainerBuilder $container) : void
    {
        ...
        $container->register(PsalmProcess::class, function (Container $container) {
            ...
            return new PsalmProcess($root, new PsalmConfig($binPath, $shouldShowInfo, $useCache, $errorLevel ? (int) $errorLevel : null), LoggingExtension::channelLogger($container, 'PSALM'));
        });
    }
    public function configure(Resolver $schema) : void
    {
        $schema->setDefaults([self::PARAM_PSALM_BIN => '%project_root%/vendor/bin/psalm', self::PARAM_PSALM_SHOW_INFO => \true, self::PARAM_PSALM_USE_CACHE => \true, self::PARAM_PSALM_ERROR_LEVEL => null]);
        $schema->setTypes([self::PARAM_PSALM_BIN => 'string', self::PARAM_PSALM_SHOW_INFO => 'boolean', self::PARAM_PSALM_USE_CACHE => 'boolean']);
        $schema->setDescriptions([self::PARAM_PSALM_BIN => 'Path to psalm if different from vendor/bin/psalm', self::PARAM_PSALM_SHOW_INFO => 'If infos from psalm should be displayed', self::PARAM_PSALM_USE_CACHE => 'If the Psalm cache should be used (see the `--no-cache` option)', self::PARAM_PSALM_ERROR_LEVEL => 'Override level at which Psalm should report errors (lower => more errors)']);
    }
