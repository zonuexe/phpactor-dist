...
class DiagnosticsParser
{
    ...
    public function parse(string $jsonString, string $filename) : array
    {
        ...
        foreach ($decoded as $psalmDiagnostic) {
            ...
            $diagnostics[] = Diagnostic::fromArray(['message' => $psalmDiagnostic['message'], 'range' => new Range(new Position($psalmDiagnostic['line_from'] - 1, $psalmDiagnostic['column_from'] - 1), new Position($psalmDiagnostic['line_to'] - 1, $psalmDiagnostic['column_to'] - 1)), 'severity' => $this->errorLevel($psalmDiagnostic), 'source' => 'psalm']);
        }
        return $diagnostics;
    }
    /**
    ...
    {
        $decoded = \json_decode($jsonString, \true, \JSON_THROW_ON_ERROR);
        if (null === $decoded) {
            throw new RuntimeException(\sprintf('Could not decode Psalm JSON output "%s": %s', $jsonString, \json_last_error_msg()));
        }
        return $decoded;
    }
