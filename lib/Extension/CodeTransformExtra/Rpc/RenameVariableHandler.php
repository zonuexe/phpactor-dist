...
class RenameVariableHandler extends AbstractHandler
{
    ...
    {
        $resolver->setDefaults([self::PARAM_NAME => null, self::PARAM_NAME_SUGGESTION => null, self::PARAM_SCOPE => null]);
        $resolver->setRequired([self::PARAM_PATH, self::PARAM_SOURCE, self::PARAM_OFFSET]);
    }
    public function handle(array $arguments)
    {
        $this->requireInput(TextInput::fromNameLabelAndDefault(self::PARAM_NAME, self::INPUT_LABEL, $arguments[self::PARAM_NAME_SUGGESTION] ?: ''));
        $this->requireInput(ChoiceInput::fromNameLabelChoices(self::PARAM_SCOPE, 'Scope: ', [RenameVariable::SCOPE_FILE => RenameVariable::SCOPE_FILE, RenameVariable::SCOPE_LOCAL => RenameVariable::SCOPE_LOCAL]));
