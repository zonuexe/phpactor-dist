...
class OverrideMethodHandler extends AbstractHandler
{
    ...
    {
        $resolver->setDefaults([self::PARAM_METHOD_NAME => null, self::PARAM_CLASS_NAME => null]);
    ...
    public function handle(array $arguments)
    {
        ...
        $this->requireInput(ListInput::fromNameLabelChoices(self::PARAM_METHOD_NAME, \sprintf('Methods from "%s"', $parentClass->name()), $this->methodChoices($parentClass))->withMultiple(\true));
        if ($this->hasMissingArguments($arguments)) {
            return $this->createInputCallback($arguments);
        }
        $newCode = $arguments[self::PARAM_SOURCE];
        foreach ((array) $arguments[self::PARAM_METHOD_NAME] as $methodName) {
            $newCode = $this->overrideMethod->overrideMethod(SourceCode::fromString((string) $newCode), (string) $class->name(), $methodName);
        }
        return UpdateFileSourceResponse::fromPathOldAndNewSource($arguments[self::PARAM_PATH], $arguments[self::PARAM_SOURCE], (string) $newCode);
    }
    private function class($source, $className = null)
    {
        ...
        if (null === $className && $classes->count() > 1) {
            throw new InvalidArgumentException('Currently will only override methods in files with one class');
        }
    ...
    private function parentClass(ReflectionClass $class)
    {
        ...
        if (null === $parentClass) {
            throw new InvalidArgumentException(\sprintf('Class "%s" has no parent', $class->name()));
        }
        return $parentClass;
    }
    private function methodChoices(ReflectionClass $parentClass)
    {
        ...
        $methodNames = \array_map(function (ReflectionMethod $method) {
            return $method->name();
        }, \iterator_to_array($parentClass->methods()->byVisibilities([Visibility::public(), Visibility::protected()])));
        \sort($methodNames);
