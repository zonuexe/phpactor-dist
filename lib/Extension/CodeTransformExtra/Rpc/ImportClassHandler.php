...
class ImportClassHandler extends AbstractHandler
{
    ...
    public function __construct(private ImportName $nameImport, private ClassSearch $classSearch, private string $filesystem)
    {
    }
    ...
    {
        $resolver->setDefaults([self::PARAM_QUALIFIED_NAME => null, self::PARAM_ALIAS => null]);
        $resolver->setRequired([self::PARAM_OFFSET, self::PARAM_SOURCE, self::PARAM_PATH]);
    }
    public function handle(array $arguments)
    {
        if (null === $arguments[self::PARAM_QUALIFIED_NAME]) {
            ...
            if (\count($suggestions) === 0) {
                return EchoResponse::fromMessage(\sprintf('No classes found with name "%s"', $name));
            }
            if (\count($suggestions) > 1) {
                $this->requireInput(ListInput::fromNameLabelChoices(self::PARAM_QUALIFIED_NAME, 'Select class:', \array_combine($suggestions, $suggestions)));
            } else {
        ...
        try {
            $sourceCode = $this->nameImport->importName(SourceCode::fromStringAndPath($arguments[self::PARAM_SOURCE], $arguments[self::PARAM_PATH]), ByteOffset::fromInt($arguments[self::PARAM_OFFSET]), NameImport::forClass($arguments[self::PARAM_QUALIFIED_NAME], $arguments[self::PARAM_ALIAS]))->apply($arguments[self::PARAM_SOURCE]);
        } catch (NameAlreadyUsedException $e) {
            if ($e instanceof NameAlreadyImportedException && $e->existingName() === $arguments[self::PARAM_QUALIFIED_NAME]) {
                return EchoResponse::fromMessage(\sprintf('Class "%s" is already imported', $arguments[self::PARAM_QUALIFIED_NAME]));
            }
            $arguments[self::PARAM_ALIAS] = null;
            $this->requireInput(TextInput::fromNameLabelAndDefault(self::PARAM_ALIAS, \sprintf('"%s" is already used, choose an alias: ', $e->name()), $e->name()));
            return $this->createInputCallback($arguments);
        } catch (TransformException $e) {
            return EchoResponse::fromMessage($e->getMessage());
        }
        return CollectionResponse::fromActions([UpdateFileSourceResponse::fromPathOldAndNewSource($arguments[self::PARAM_PATH], $arguments[self::PARAM_SOURCE], (string) $sourceCode), EchoResponse::fromMessage(\sprintf('Imported class "%s"', $arguments[self::PARAM_QUALIFIED_NAME]))]);
    }
    private function suggestions(string $name)
    {
        $suggestions = $this->classSearch->classSearch($this->filesystem, $name);
