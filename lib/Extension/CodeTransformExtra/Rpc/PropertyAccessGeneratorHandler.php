...
class PropertyAccessGeneratorHandler extends AbstractHandler
{
    ...
    public function __construct(private string $name, private Reflector $reflector, private PropertyAccessGenerator $propertyAccessGenerator)
    {
    }
    ...
    {
        $resolver->setDefaults([self::PARAM_NAMES => null]);
    ...
    private function handleClass(array $arguments) : Response
    {
        ...
        $this->requireInput(ListInput::fromNameLabelChoices(self::PARAM_NAMES, \sprintf('Properties from "%s"', $class->name()), $this->propertiesChoices($class))->withMultiple(\true));
        ...
        $edits = $this->propertyAccessGenerator->generate($newSource, (array) $arguments[self::PARAM_NAMES], $arguments[self::PARAM_OFFSET]);
        return UpdateFileSourceResponse::fromPathOldAndNewSource($arguments[self::PARAM_PATH], $originalSource, $edits->apply($originalSource));
    }
    private function class(string $source) : ReflectionClass
    {
        ...
        if ($classes->count() > 1) {
            throw new InvalidArgumentException('Currently will only generates accessor/mutators by name in files with one class');
        }
    ...
    private function propertiesChoices(ReflectionClass $class) : array
    {
        ...
        \natsort($propertiesNames);
        return \array_combine($propertiesNames, $propertiesNames);
    }
    private function handleSingle(NodeContext $context, array $arguments)
    {
        $newSource = $this->propertyAccessGenerator->generate(SourceCode::fromStringAndPath($arguments[self::PARAM_SOURCE], $arguments[self::PARAM_PATH]), [$context->symbol()->name()], $arguments[self::PARAM_OFFSET])->apply($arguments[self::PARAM_SOURCE]);
