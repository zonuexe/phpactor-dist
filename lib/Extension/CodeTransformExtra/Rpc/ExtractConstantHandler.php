...
class ExtractConstantHandler extends AbstractHandler
{
    ...
    {
        $resolver->setDefaults([self::PARAM_CONSTANT_NAME => null, self::PARAM_CONSTANT_NAME_SUGGESTION => null]);
        $resolver->setRequired([self::PARAM_PATH, self::PARAM_OFFSET, self::PARAM_SOURCE]);
    }
    public function handle(array $arguments)
    {
        $this->requireInput(TextInput::fromNameLabelAndDefault(self::PARAM_CONSTANT_NAME, self::INPUT_LABEL_NAME, $arguments[self::PARAM_CONSTANT_NAME_SUGGESTION] ?: ''));
        if ($this->hasMissingArguments($arguments)) {
            return $this->createInputCallback($arguments);
        }
        $textEdits = $this->extractConstant->extractConstant(SourceCode::fromStringAndPath($arguments[self::PARAM_SOURCE], $arguments[self::PARAM_PATH]), $arguments[self::PARAM_OFFSET], $arguments[self::PARAM_CONSTANT_NAME]);
