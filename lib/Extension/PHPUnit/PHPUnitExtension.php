...
class PHPUnitExtension implements OptionalExtension
{
    ...
    {
        $container->register(GenerateTestMethodCommand::class, function (Container $container) {
            return new GenerateTestMethodCommand($container->get(ClientApi::class), $container->expect(LanguageServerExtension::SERVICE_SESSION_WORKSPACE, Workspace::class), $container->get(GenerateTestMethods::class));
        }, [LanguageServerExtension::TAG_COMMAND => ['name' => GenerateTestMethodCommand::NAME]]);
    }
    private function registerServices(ContainerBuilder $container) : void
    {
        $container->register(GenerateTestMethodProvider::class, function (Container $container) {
            return new GenerateTestMethodProvider($container->get(GenerateTestMethods::class));
        }, [LanguageServerExtension::TAG_CODE_ACTION_PROVIDER => []]);
        $container->register(GenerateTestMethods::class, function (Container $container) {
            return new GenerateTestMethods($container->expect(WorseReflectionExtension::SERVICE_REFLECTOR, Reflector::class), $container->get(Updater::class));
        });
    }
