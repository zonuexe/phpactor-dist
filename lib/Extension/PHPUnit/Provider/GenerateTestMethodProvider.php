...
class GenerateTestMethodProvider implements CodeActionProvider
{
    ...
    {
        $methodsThatCanBeGenerated = $this->generateTestMethods->getGeneratableTestMethods(SourceCode::fromStringAndPath($textDocument->text, $textDocument->uri));
        $availableCodeActions = [];
        foreach ($methodsThatCanBeGenerated as $methodName) {
            $availableCodeActions[] = new CodeAction(title: 'Generate method ' . $methodName, kind: $this->kinds()[0], diagnostics: [], isPreferred: \false, command: new Command(title: 'Test Methods', command: GenerateTestMethodCommand::NAME, arguments: [$textDocument->uri, $methodName]));
        }
