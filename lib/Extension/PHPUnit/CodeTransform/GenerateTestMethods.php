...
{
    private const METHODS_TO_GENERATE = ['setUp', 'tearDown'];
    ...
    public function getGeneratableTestMethods(SourceCode $source) : Generator
    {
        ...
        if (!$class->isInstanceOf(ClassName::fromString('PhpactorDist\\PHPUnit\\Framework\\TestCase'))) {
            return;
        }
    ...
    {
        Assert::inArray($methodName, self::METHODS_TO_GENERATE, \sprintf('%s can not generate "%s" with class', __CLASS__, $methodName));
