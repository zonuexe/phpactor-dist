...
namespace Phpactor202301\Phpactor\ConfigLoader\Tests\Benchmark;
...
class ConfigLoaderBench extends TestCase
{
    ...
    {
        $loader = new ConfigLoader(new Deserializers(['json' => new JsonDeserializer()]), new PathCandidates([new AbsolutePathCandidate($this->config1, 'json'), new AbsolutePathCandidate($this->config2, 'json')]));
        $loader->load();
    }
    public function benchJsonLoadConfigWithBuilder() : void
    {
        ConfigLoaderBuilder::create()->enableJsonDeserializer('json')->addCandidate($this->config1, 'json')->addCandidate($this->config2, 'json')->loader()->load();
    }
    public function benchJsonLoadConfigWithNonExistingYaml() : void
    {
        $loader = new ConfigLoader(new Deserializers(['json' => new JsonDeserializer(), 'yaml' => new YamlDeserializer()]), new PathCandidates([new AbsolutePathCandidate($this->config1, 'json'), new AbsolutePathCandidate('/path/to/yaml1', 'yaml'), new AbsolutePathCandidate('/path/to/yaml2', 'yaml'), new AbsolutePathCandidate($this->config2, 'json')]));
        $loader->load();
    }
    public function benchJsonPlainPhp() : void
    {
        $config = \array_merge(\json_decode(\file_get_contents($this->config1), \true), \json_decode(\file_get_contents($this->config2), \true));
    }
    public function benchYamlLoadConfig() : void
    {
        $loader = new ConfigLoader(new Deserializers(['yaml' => new YamlDeserializer()]), new PathCandidates([new AbsolutePathCandidate($this->config1yaml, 'yaml'), new AbsolutePathCandidate($this->config2yaml, 'yaml')]));
        $loader->load();
    }
}
/**
 ...
 * @OutputTimeUnit("milliseconds")
 */
\class_alias('Phpactor202301\\Phpactor\\ConfigLoader\\Tests\\Benchmark\\ConfigLoaderBench', 'Phpactor\\ConfigLoader\\Tests\\Benchmark\\ConfigLoaderBench', \false);
