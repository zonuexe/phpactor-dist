...
class EfficientLineColsTest extends TestCase
{
    ...
    {
        (yield [[], '', function (EfficientLineCols $lineCols) : void {
            self::assertInstanceOf(EfficientLineCols::class, $lineCols);
        }]);
        (yield [[2], '01234', function (EfficientLineCols $lineCols) : void {
            self::assertEquals(3, $lineCols->get(2)->col());
            self::assertEquals(1, $lineCols->get(2)->line());
        }]);
        (yield [[2, 3, 0, 10], "01234\n5678", function (EfficientLineCols $lineCols) : void {
            self::assertEquals(3, $lineCols->get(2)->col());
            self::assertEquals(1, $lineCols->get(2)->line());
    ...
    {
        $converter = EfficientLineCols::fromByteOffsetInts($text, $offsets, \true);
        $assertion($converter);
    }
    /**
     * @return Generator<mixed>
    ...
    {
        (yield 'cat is 4 bytes' => [[5], 'a😸bc', function (EfficientLineCols $lineCols) : void {
            self::assertEquals(6, $lineCols->get(5)->col());
        }]);
    }
}
\class_alias('Phpactor202301\\Phpactor\\TextDocument\\Tests\\Unit\\EfficientLineColsTest', 'Phpactor\\TextDocument\\Tests\\Unit\\EfficientLineColsTest', \false);
