    ...
    public function apply(string $text) : string
    {
        ...
        for ($i = \count($this->textEdits) - 1; $i >= 0; $i--) {
            ...
            if ($edit->end()->toInt() > \strlen($text)) {
                throw new OutOfBoundsException(\sprintf('Text edit end (%s) exceeds length of text (%s): %s', $edit->end()->toInt(), $edit->replacement(), self::renderDebugTextEdits($edit, $this->textEdits)));
            }
