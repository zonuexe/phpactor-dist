...
final class LineCol
{
    ...
    {
        if ($line < 1) {
            throw new RuntimeException(\sprintf('Line number cannot be less than 1 (got "%s")', $line));
        }
        if ($col < 1) {
            throw new RuntimeException(\sprintf('Col number cannot be less than 1 (got "%s")', $col));
        }
        $this->line = $line;
        $this->col = $col;
    }
    ...
    public static function fromByteOffset(string $text, ByteOffset $byteOffset) : self
    {
        ...
        foreach ($lines as $lineOrDelim) {
            ...
            if ($byteOffset->toInt() >= $start && $byteOffset->toInt() <= $end) {
                $section = \substr($lineOrDelim, 0, $byteOffset->toInt() - $start);
                return new self($lineNb, \mb_strlen($section) + 1);
            }
            $offset = $end;
        }
        throw new OutOfBoundsException(\sprintf('Byte offset %s is larger than text length %s', $byteOffset->toInt(), \strlen($text)));
    }
