...
final class Parser
{
     ...
     */
    private const SCALAR_TYPES = ['int', 'float', 'bool', 'class-string', 'string', 'mixed', 'callable', 'false'];
    ...
    private function parseTypes() : ?TypeNode
    {
        ...
        $mode = null;
        while (\true) {
            if ($this->tokens->if(Token::T_BAR) || $this->tokens->if(Token::T_AMPERSAND)) {
    ...
    private function createTypeFromToken(Token $type) : TypeNode
    {
        ...
        if ($this->tokens->peekIs(0, Token::T_DOUBLE_COLON) && ($this->tokens->peekIs(1, Token::T_LABEL) || $this->tokens->peekIs(1, Token::T_ASTERISK))) {
            return new ConstantNode($classNode, $this->tokens->chomp(), $this->tokens->chomp());
        }
        return $classNode;
    }
    ...
    {
        return new DeprecatedTag($this->tokens->chomp(Token::T_TAG), $this->parseText());
    }
    ...
    {
        return $this->tokens->if(Token::T_LABEL) || $this->tokens->if(Token::T_NULLABLE) || $this->tokens->if(Token::T_QUOTED_STRING) || $this->tokens->if(Token::T_INTEGER) || $this->tokens->if(Token::T_FLOAT) || $this->tokens->if(Token::T_PAREN_OPEN);
    }
    ...
    {
        $key = $colon = $type = null;
        if ($this->tokens->if(Token::T_LABEL) && $this->tokens->peekIs(1, Token::T_COLON)) {
            $key = $this->tokens->chomp();
            $colon = $this->tokens->chomp();
        }
        if ($this->tokens->if(Token::T_LABEL) && $this->tokens->peekIs(1, Token::T_NULLABLE) && $this->tokens->peekIs(2, Token::T_COLON)) {
            $key = $this->tokens->chomp();
            $_ = $this->tokens->chomp();
            $colon = $this->tokens->chomp();
        }
