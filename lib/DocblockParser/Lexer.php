...
final class Lexer
{
    ...
    private const TOKEN_VALUE_MAP = [']' => Token::T_BRACKET_SQUARE_CLOSE, '[' => Token::T_BRACKET_SQUARE_OPEN, '>' => Token::T_BRACKET_ANGLE_CLOSE, '<' => Token::T_BRACKET_ANGLE_OPEN, '{' => Token::T_BRACKET_CURLY_OPEN, '}' => Token::T_BRACKET_CURLY_CLOSE, '(' => Token::T_PAREN_OPEN, ')' => Token::T_PAREN_CLOSE, ',' => Token::T_COMMA, '[]' => Token::T_LIST, '?' => Token::T_NULLABLE, '|' => Token::T_BAR, '&' => Token::T_AMPERSAND, '=' => Token::T_EQUALS, ':' => Token::T_COLON, '::' => Token::T_DOUBLE_COLON];
    /**
     * @var string[]
     */
    private const IGNORE_PATTERNS = ['\\s+'];
    private string $pattern;
    public function __construct()
    {
        $this->pattern = \sprintf('{(%s)|%s}', \implode(')|(', self::PATTERNS), \implode('|', self::IGNORE_PATTERNS));
    }
    public function lex(string $docblock) : Tokens
    {
        $chunks = \preg_split($this->pattern, $docblock, -1, \PREG_SPLIT_NO_EMPTY | \PREG_SPLIT_DELIM_CAPTURE | \PREG_SPLIT_OFFSET_CAPTURE);
        if (\false === $chunks) {
            throw new RuntimeException('Unexpected error from preg_split');
        }
        $tokens = [];
