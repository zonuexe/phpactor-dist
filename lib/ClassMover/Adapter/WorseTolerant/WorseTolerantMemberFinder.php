...
class WorseTolerantMemberFinder implements MemberFinder
{
    ...
    private function getMemberDeclarationReference(ReflectionClassLike $queryClass = null, Node $memberNode)
    {
        ...
        $reference = MemberReference::fromMemberNameAndPosition($memberName, Position::fromStartAndEnd($this->memberStartPosition($memberNode), $memberNode->name->start + $memberNode->name->length - 1));
    ...
    private function getScopedPropertyAccessReference(ClassMemberQuery $query, ScopedPropertyAccessExpression $memberNode)
    {
        ...
        $reference = MemberReference::fromMemberNameAndPosition(MemberName::fromString($memberName), Position::fromStartAndEnd($memberNameToken->start + $startOffset, $memberNameToken->start + $memberNameToken->length));
        $offset = $this->reflector->reflectOffset(WorseSourceCode::fromString($memberNode->getFileContents()), Offset::fromInt($memberNode->scopeResolutionQualifier->getEndPosition()));
    ...
    private function getMemberAccessReference(ClassMemberQuery $query, MemberAccessExpression $memberNode)
    {
        ...
        $reference = MemberReference::fromMemberNameAndPosition(MemberName::fromString((string) $memberNode->memberName->getText($memberNode->getFileContents())), Position::fromStartAndEnd($memberNode->memberName->start, $memberNode->memberName->start + $memberNode->memberName->length));
        $offset = $this->reflector->reflectOffset(WorseSourceCode::fromString($memberNode->getFileContents()), Offset::fromInt($memberNode->dereferencableExpression->getEndPosition()));
        return $this->attachClassInfoToReference($reference, $query, $offset);
    }
