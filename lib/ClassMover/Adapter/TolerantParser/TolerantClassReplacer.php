...
class TolerantClassReplacer implements ClassReplacer
{
    ...
    public function replaceReferences(TextDocument $source, NamespacedClassReferences $classRefList, FullyQualifiedName $originalName, FullyQualifiedName $newName) : TextEdits
    {
        ...
        foreach ($classRefList as $classRef) {
            if ($classRef->name()->wasFullyQualified()) {
                $edits[] = TextEdit::create($classRef->position()->start(), $classRef->position()->length(), '\\' . $newName->__toString());
                continue;
            }
            ...
            $edits[] = TextEdit::create($classRef->position()->start(), $classRef->position()->length(), $classRef->name()->transpose($newName)->__toString());
        }
    ...
    {
        return $this->updater->textEditsFor(SourceCodeBuilder::create()->namespace($qualifiedName->__toString())->build(), $source);
    }
}
