...
final class Visibility
{
    ...
    const VISIBILITIES = [self::PUBLIC, self::PROTECTED, self::PRIVATE];
    private string $visibility;
    private function __construct(string $visibility)
    {
        if (!\in_array($visibility, self::VISIBILITIES)) {
            throw new InvalidArgumentException(\sprintf('Invalid visibility "%s", valid visibilities: "%s"', $visibility, \implode('", "', self::VISIBILITIES)));
        }
