...
class SourceCodeBuilderTest extends TestCase
{
    ...
    {
        (yield 'new builder is modified by default' => [function (SourceCodeBuilder $builder) : void {
            $builder->class('foobar');
        }, function (SourceCodeBuilder $builder) : void {
            $this->assertTrue($builder->isModified());
        }]);
        (yield 'is not modified after snapshot' => [function (SourceCodeBuilder $builder) : void {
            $builder->class('foobar');
            $builder->snapshot();
        }, function (SourceCodeBuilder $builder) : void {
            $this->assertFalse($builder->isModified());
        }]);
        (yield 'is not modified if updated values are the same 1' => [function (SourceCodeBuilder $builder) : void {
        ...
        (yield 'is not modified if updated values are the same 2' => [function (SourceCodeBuilder $builder) : void {
        ...
        (yield 'is modified when values are different 1' => [function (SourceCodeBuilder $builder) : void {
            $builder->class('foobar')->method('foobar');
            $builder->snapshot();
        ...
        (yield 'is modified when values are different 2' => [function (SourceCodeBuilder $builder) : void {
    ...
    public function testFunctionUse() : void
    {
        ...
        $builder->useFunction('Phpactor202301\\hello\\goodbye');
        $code = $builder->build();
        $this->assertCount(2, $code->useStatements());
    ...
    {
        $builder = $this->builder();
        $classBuilder = $builder->class('Dog')->extends('Canine')->implements('Teeth')->property('one')->end()->property('two')->end()->method('method1')->end()->method('method2')->end();
    ...
    {
        $builder = $this->builder();
        $interfaceBuilder = $builder->interface('Dog')->extends('Canine')->method('method1')->end()->method('method2')->end();
    ...
    {
        $builder = $this->builder();
        $traitBuilder = $builder->trait('Dog')->property('one')->end()->property('two')->end()->method('method1')->end()->method('method2')->end();
    ...
    {
        $builder = $this->builder();
        $propertyBuilder = $builder->class('Dog')->property('one')->type('string')->defaultValue(null);
    ...
    {
        return ['Method return type' => [$this->builder()->class('Dog')->method('one')->returnType('?string')->visibility('private')->parameter('one')->type('One')->defaultValue(1)->end(), function (Method $method) : void {
            $this->assertEquals('?string', $method->returnType()->__toString());
        }], 'Method mofifiers 1' => [$this->builder()->class('Dog')->method('one')->static()->abstract(), function ($method) : void {
            $this->assertTrue($method->isStatic());
            $this->assertTrue($method->isAbstract());
        }], 'Method mofifiers 2' => [$this->builder()->class('Dog')->method('one')->abstract(), function ($method) : void {
            $this->assertFalse($method->isStatic());
            $this->assertTrue($method->isAbstract());
        }], 'Method lines' => [$this->builder()->class('Dog')->method('one')->body()->line('one')->line('two')->end(), function ($method) : void {
...
\class_alias('Phpactor202301\\Phpactor\\CodeBuilder\\Tests\\Unit\\Domain\\Builder\\SourceCodeBuilderTest', 'Phpactor\\CodeBuilder\\Tests\\Unit\\Domain\\Builder\\SourceCodeBuilderTest', \false);
