...
namespace Phpactor202301\Phpactor\Rename\Tests\Model\Renamer;
...
class ChainRenamerTest extends TestCase
{
    ...
    public function testGetFirstNonNullRename() : void
    {
        ...
        $results1 = [new LocatedTextEdit(TextDocumentUri::fromString('/foo/bar'), TextEdit::create(1, 1, 'foo'))];
    ...
    public function testFirstRenameForTwoCapableRenamers() : void
    {
        ...
        $results2 = [new LocatedTextEdit(TextDocumentUri::fromString('/foo/bar'), TextEdit::create(1, 1, 'foo'))];
     ...
     */
    private function assertResolvesRangeAndResults(array $renamers, ?ByteOffsetRange $expectedRange, array $expectedResults) : void
    {
        $textDocument = TextDocumentBuilder::create('text')->uri('file:///test1')->build();
        $byteOffset = ByteOffset::fromInt(0);
        $this->assertSame($expectedRange, $this->createRenamer($renamers)->getRenameRange($textDocument, $byteOffset), 'Returns expected range');
        $this->assertSame($expectedResults, iterator_to_array($this->createRenamer($renamers)->rename($textDocument, $byteOffset, 'foobar')), 'Returns expected results');
    }
    /**
     * @param Renamer[] $renamers
     */
...
\class_alias('Phpactor202301\\Phpactor\\Rename\\Tests\\Model\\Renamer\\ChainRenamerTest', 'Phpactor\\Rename\\Tests\\Model\\Renamer\\ChainRenamerTest', \false);
