...
final class FilePath
{
    private function __construct(private string $path)
    {
        if (\false === Path::isAbsolute($path)) {
            throw new InvalidArgumentException(\sprintf('File path must be absolute, but "%s" given', $path));
        }
    }
    ...
    public static function fromUnknown($path) : FilePath
    {
        ...
        throw new RuntimeException(\sprintf('Do not know how to create FilePath from "%s"', \is_object($path) ? \get_class($path) : \gettype($path)));
    }
    ...
    public function makeAbsoluteFromString(string $path) : FilePath
    {
        if (Path::isAbsolute($path)) {
            ...
            if (\false === $path->isWithinOrSame($this)) {
                throw new RuntimeException(\sprintf('Trying to create descendant from absolute path "%s" that does not lie within context path "%s"', (string) $path, (string) $this));
            }
            return $path;
        }
