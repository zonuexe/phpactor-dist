...
class GitFilesystem extends SimpleFilesystem
{
    ...
    public function __construct($path, FileListProvider $fileListProvider = null)
    {
        ...
        if (\false === \file_exists($path->__toString() . '/.git')) {
            throw new NotSupported('The cwd does not seem to be a git repository root (could not find .git folder)');
        }
    }
    ...
    private function exec(array $cmd) : string
    {
        ...
        if ($process->getExitCode() !== 0) {
            throw new InvalidArgumentException(\sprintf('Could not execute git command "%s", exit code "%s", output "%s"', \implode(' ', $cmd), $process->getExitCode(), $process->getOutput()));
        }
