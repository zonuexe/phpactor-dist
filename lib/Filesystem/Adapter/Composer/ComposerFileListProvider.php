...
class ComposerFileListProvider implements FileListProvider
{
    ...
    {
        $prefixes = \array_merge($this->classLoader->getPrefixes(), $this->classLoader->getPrefixesPsr4(), $this->classLoader->getClassMap(), $this->classLoader->getFallbackDirs(), $this->classLoader->getFallbackDirsPsr4());
        ...
        foreach ($prefixes as $paths) {
            ...
            foreach ($paths as $path) {
                ...
                $iterator = $this->createFileIterator($this->path->makeAbsoluteFromString($path));
