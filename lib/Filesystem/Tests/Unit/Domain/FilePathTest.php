...
class FilePathTest extends TestCase
{
    ...
    {
        (yield 'FilePath instance' => [FilePath::fromString('/foo.php'), '/foo.php']);
        (yield 'string' => ['/foo.php', '/foo.php']);
        (yield 'URI string' => ['file:///foo.php', '/foo.php']);
        (yield 'array' => [['one', 'two'], '/one/two']);
        (yield 'SplFileInfo' => [new SplFileInfo(__FILE__), __FILE__]);
    }
    /**
    ...
    {
        (yield 'object' => [new stdClass(), 'Do not know']);
        (yield 'unsupported scheme' => ['ftp://host/foo.php', 'Unsupported scheme "ftp" for path "ftp://host/foo.php"']);
        (yield 'malformed string' => ['bar:///foo.php', 'Cannot guess path from "bar:///foo.php"']);
        (yield 'URI without a path' => ['http://.?x=1&n', 'No path info from URI "http://.?x=1&n"']);
    }
    /**
...
\class_alias('Phpactor202301\\Phpactor\\Filesystem\\Tests\\Unit\\Domain\\FilePathTest', 'Phpactor\\Filesystem\\Tests\\Unit\\Domain\\FilePathTest', \false);
