...
final class LanguageServerTester
{
    ...
    {
        if ($response->error) {
            throw new RuntimeException(\sprintf('Response has error: %s' . "\n" . '%s', $response->error->message, \is_string($response->error->data) ? $response->error->data : \json_encode($response->error->data, \JSON_PRETTY_PRINT)));
        }
    }
