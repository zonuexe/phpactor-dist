...
class InitializeMiddleware implements Middleware
{
    ...
    public function process(Message $request, RequestHandler $handler) : Promise
    {
        ...
        if (\true === $this->initialized) {
            throw new RuntimeException(\sprintf('Second initialize request (id: %s) has been recieved. Can only initialize a session once', $request->id));
        }
