...
class DiagnosticsService implements ServiceProvider, ListenerProviderInterface
{
    ...
    public function __construct(DiagnosticsEngine $engine, bool $lintOnUpdate = \true, bool $lintOnSave = \true, ?Workspace $workspace = null, bool $clearOnUpdate = \true, bool $lintOnOpen = \true)
    {
        $this->engine = $engine;
    ...
    {
        $item = new TextDocumentItem($update->identifier()->uri, 'php', $update->identifier()->version, $update->updatedText());
    ...
    public function enqueueSave(TextDocumentSaved $save) : void
    {
        $item = new TextDocumentItem(
            $save->identifier()->uri,
            'php',
            $save->identifier()->version ?? 1,
            // VIM lsp client seems delivers NULL here, so just use an arbitrary identifier
