...
final class TestMessageTransmitter implements \Phpactor\LanguageServer\Core\Server\Transmitter\MessageTransmitter, \Phpactor\LanguageServer\Core\Server\Transmitter\TestMessageTransmitterStack, Countable
{
    ...
    public function shiftNotification() : ?NotificationMessage
    {
        ...
        if (!$message instanceof NotificationMessage) {
            throw new RuntimeException(\sprintf('Expected NotificationMessage, got "%s"', \get_class($message)));
        }
        return $message;
    }
    public function shiftRequest() : ?RequestMessage
    {
        ...
        if (!$message instanceof RequestMessage) {
            throw new RuntimeException(\sprintf('Expected RequestMessage, got "%s"', \get_class($message)));
        }
        return $message;
    }
