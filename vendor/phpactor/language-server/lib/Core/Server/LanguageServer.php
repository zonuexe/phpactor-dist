...
final class LanguageServer
{
    ...
    public function address() : ?string
    {
        if (!$this->streamProvider instanceof SocketStreamProvider) {
            throw new RuntimeException(\sprintf('Cannot get address on non-socket stream provider, using "%s"', \get_class($this->streamProvider)));
        }
    ...
    {
        if ($this->streamProvider instanceof SocketStreamProvider) {
            $this->logger->info(\sprintf('Listening on %s', $this->streamProvider->address()));
        }
        ...
        return \PhpactorDist\Amp\call(function () use($connection) {
            ...
            while (null !== ($request = (yield $reader->wait()))) {
                ...
                try {
                    ...
                    $transmitter->transmit(new ResponseMessage($request->body()['id'] ?? 0, []));
                    continue;
                }
                ...
                // capabilities)
                if (null === $dispatcher) {
                    $dispatcher = $this->dispatcherFactory->create($transmitter, $this->initializer->provideInitializeParams($request));
                }
        ...
        \PhpactorDist\Amp\asyncCall(function () use($transmitter, $request, $dispatcher, $connection) {
            try {
                ...
                if ($this->streamProvider instanceof ResourceStreamProvider) {
                    throw new ShutdownServer('Exit called on STDIO connection, exiting the server');
                }
                return;
            }
            if (null === $response) {
