...
final class LspMessageReader implements \Phpactor\LanguageServer\Core\Server\Parser\RequestReader
{
    ...
    private function parseRequest() : ?RawMessage
    {
        ...
        if ($this->headers === null && \substr($this->buffer, -4, 4) === "\r\n\r\n") {
            $this->headers = $this->parseHeaders(\substr($this->buffer, 0, -4));
            $this->buffer = '';
            return null;
        }
        ...
        if (!isset($this->headers[self::HEADER_CONTENT_LENGTH])) {
            throw new CouldNotParseHeader(\sprintf('Header did not contain mandatory Content-Length in "%s"', json_encode($this->headers)));
        }
