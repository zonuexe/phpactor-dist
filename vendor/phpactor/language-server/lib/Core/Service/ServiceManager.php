...
class ServiceManager
{
    ...
    public function start(string $serviceName) : void
    {
        ...
        if (isset($this->cancellations[$serviceName])) {
            throw new RuntimeException(\sprintf('Service "%s" is already running', $serviceName));
        }
        $this->logger->info(\sprintf('Starting service: %s (%s)', $serviceName, \get_class($provider)));
        if (!\method_exists($provider, $serviceName)) {
            throw new RuntimeException(\sprintf('Service provider "%s" has no service method "%s"', \get_class($provider), $serviceName));
        }
        ...
        asyncCall(function () use($provider, $serviceName, $token) {
            ...
            if (!$promise instanceof Promise) {
                throw new RuntimeException(\sprintf('Service method "%s" must return a Promise, got "%s"', \get_class($provider) . '::' . $serviceName, \is_object($promise) ? \get_class($promise) : \gettype($promise)));
            }
            try {
                (yield $promise);
            } catch (Throwable $error) {
                $this->logger->error(\sprintf('Error in service "%s" "%s:%s": %s', $serviceName, \get_class($provider), __FUNCTION__, $error->getMessage()));
            }
        });
    }
    public function stop(string $serviceName) : void
    {
        ...
        if (!isset($this->cancellations[$serviceName])) {
            throw new RuntimeException(\sprintf('Cannot stop service "%s" it has not been started, running services: "%s"', $serviceName, \implode('", "', \array_keys($this->cancellations))));
        }
