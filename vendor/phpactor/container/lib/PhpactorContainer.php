...
class PhpactorContainer implements \Phpactor\Container\Container, \Phpactor\Container\ContainerBuilder
{
    ...
    public static function fromExtensions(array $extensionClasses, array $parameters = []) : \Phpactor\Container\Container
    {
        ...
        $resolver->setDefaults([self::PARAM_EXTENSION_CLASSES => $extensionClasses]);
    ...
    public function get($id)
    {
        ...
        if (!isset($this->factories[$id])) {
            throw new RuntimeException(\sprintf('No service with ID "%s" exists', $id));
        }
    ...
    public function getParameter(string $name)
    {
        if (!\array_key_exists($name, $this->parameters)) {
            throw new RuntimeException(\sprintf('Unknown parameter "%s", known parameters "%s"', $name, \implode('", "', \array_keys($this->parameters))));
        }
