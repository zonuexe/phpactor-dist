...
class WatchmanWatcher implements Watcher, WatcherProcess
{
    ...
    public function wait() : Promise
    {
        return call(function () {
            ...
            while (null !== ($line = (yield $this->readLine()))) {
                $notification = \json_decode($line, \true);
                if (\false === $notification) {
                    throw new RuntimeException(\sprintf('Could not decode JSON from watchman: %s %s', $line, \json_last_error_msg()));
                }
                $files = \array_map(function (array $file) use($notification) {
                    $modifiedFile = ModifiedFileBuilder::fromPathSegments($notification['root'], $file['name']);
                    if ($file['type'] === 'd') {
    ...
    private function watchPaths() : Promise
    {
        return call(function () {
            foreach ($this->config->paths() as $path) {
                $process = new Process([self::WATCHMAN_CMD, 'watch', $path]);
                $pid = (yield $process->start());
                $this->logger->debug(\sprintf('Watchman: %s', $process->getCommand()));
                $exit = (yield $process->join());
                if ($exit !== 0) {
                    throw new RuntimeException(\sprintf('Watchman exited with code "%s": %s ', $exit, (yield buffer($process->getStderr()))));
                }
            }
        });
    }
    ...
    private function subscribe(string $path) : Promise
    {
        return call(function () use($path) {
            $process = new Process([self::WATCHMAN_CMD, '-j', '-p', '--no-pretty']);
            $this->logger->debug(\sprintf('Watchman: %s', $process->getCommand()));
            $pid = (yield $process->start());
            $payload = (string) \json_encode(['subscribe', $path, 'ampfs-watch', ['expression' => ['allof', ['anyof', ['type', 'f'], ['type', 'd']], ['since', \time(), 'ctime']], 'fields' => ['name', 'type']]]);
    ...
    private function readLine() : Promise
    {
        return call(function () {
            ...
            $this->logger->debug(\print_r($line, \true));
            if (null !== $line) {
                return $line;
            }
            foreach ($this->subscribers as $subscriber) {
                ...
                // handled downstream.
                if ($exitCode === 1) {
                    throw new WatcherDied(\sprintf('Watchman subscriber exited with status code "%s": %s', $exitCode, (yield buffer($subscriber->getStderr()))));
                }
            }
            return null;
        });
