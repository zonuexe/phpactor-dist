...
class FindWatcher implements Watcher, WatcherProcess
{
    ...
    private function search(string $path) : Promise
    {
        return \PhpactorDist\Amp\call(function () use($path) {
            ...
            $stop = \microtime(\true);
            $this->logger->debug(\sprintf('pid:%s Find process "%s" done in %s seconds', \getmypid(), $process->getCommand(), \number_format($stop - $start, 2)));
            if ($exitCode === 0) {
                return;
            }
            $stderr = (yield $process->getStderr()->read());
            $this->logger->error(\sprintf('Process "%s" exited with error code %s: %s', $process->getCommand(), $exitCode, $stderr));
        });
    }
    ...
    private function startProcess(string $path) : Promise
    {
        return \PhpactorDist\Amp\call(function () use($path) {
            ...
            // times.
            $process = new Process(['find', $path, '-mindepth', '1', '-newercc', $this->lastUpdateFile]);
    ...
    {
        $name = \tempnam(\sys_get_temp_dir(), 'amp-fs-watch');
        if (!$name) {
            throw new RuntimeException(\sprintf('Could not create temporary file "%s"', $name));
        }
        return $name;
    }
    /**
