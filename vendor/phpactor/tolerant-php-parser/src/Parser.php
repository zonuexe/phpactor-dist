...
use Closure;
use PhpactorDist\Microsoft\PhpParser\Node\AnonymousFunctionUseClause;
...
use PhpactorDist\Microsoft\PhpParser\Node\Expression\BracedExpression;
use PhpactorDist\Microsoft\PhpParser\Node\Expression\CallExpression;
use PhpactorDist\Microsoft\PhpParser\Node\Expression\CastExpression;
use PhpactorDist\Microsoft\PhpParser\Node\Expression\CloneExpression;
use PhpactorDist\Microsoft\PhpParser\Node\Expression\EmptyIntrinsicExpression;
use PhpactorDist\Microsoft\PhpParser\Node\Expression\ErrorControlExpression;
use PhpactorDist\Microsoft\PhpParser\Node\Expression\EvalIntrinsicExpression;
use PhpactorDist\Microsoft\PhpParser\Node\Expression\ExitIntrinsicExpression;
use PhpactorDist\Microsoft\PhpParser\Node\Expression\IssetIntrinsicExpression;
use PhpactorDist\Microsoft\PhpParser\Node\Expression\MatchExpression;
...
use PhpactorDist\Microsoft\PhpParser\Node\Expression\PrefixUpdateExpression;
use PhpactorDist\Microsoft\PhpParser\Node\Expression\PrintIntrinsicExpression;
...
use PhpactorDist\Microsoft\PhpParser\Node\Expression\ScriptInclusionExpression;
...
use PhpactorDist\Microsoft\PhpParser\Node\Expression\TernaryExpression;
...
use PhpactorDist\Microsoft\PhpParser\Node\Expression\UnaryOpExpression;
...
use PhpactorDist\Microsoft\PhpParser\Node\StaticVariableDeclaration;
use PhpactorDist\Microsoft\PhpParser\Node\ClassConstDeclaration;
use PhpactorDist\Microsoft\PhpParser\Node\DeclareDirective;
...
use PhpactorDist\Microsoft\PhpParser\Node\MissingDeclaration;
use PhpactorDist\Microsoft\PhpParser\Node\MissingMemberDeclaration;
...
use PhpactorDist\Microsoft\PhpParser\Node\NumericLiteral;
use PhpactorDist\Microsoft\PhpParser\Node\ParenthesizedIntersectionType;
use PhpactorDist\Microsoft\PhpParser\Node\PropertyDeclaration;
use PhpactorDist\Microsoft\PhpParser\Node\ReservedWord;
...
use PhpactorDist\Microsoft\PhpParser\Node\RelativeSpecifier;
...
use PhpactorDist\Microsoft\PhpParser\Node\Statement\FunctionStaticDeclaration;
use PhpactorDist\Microsoft\PhpParser\Node\Statement\GlobalDeclaration;
use PhpactorDist\Microsoft\PhpParser\Node\Statement\BreakOrContinueStatement;
use PhpactorDist\Microsoft\PhpParser\Node\Statement\DeclareStatement;
use PhpactorDist\Microsoft\PhpParser\Node\Statement\DoStatement;
use PhpactorDist\Microsoft\PhpParser\Node\Statement\EchoStatement;
use PhpactorDist\Microsoft\PhpParser\Node\Statement\EmptyStatement;
...
use PhpactorDist\Microsoft\PhpParser\Node\Statement\ForStatement;
use PhpactorDist\Microsoft\PhpParser\Node\Statement\FunctionDeclaration;
use PhpactorDist\Microsoft\PhpParser\Node\Statement\GotoStatement;
use PhpactorDist\Microsoft\PhpParser\Node\Statement\HaltCompilerStatement;
...
use PhpactorDist\Microsoft\PhpParser\Node\Statement\NamedLabelStatement;
use PhpactorDist\Microsoft\PhpParser\Node\Statement\ReturnStatement;
use PhpactorDist\Microsoft\PhpParser\Node\Statement\SwitchStatementNode;
use PhpactorDist\Microsoft\PhpParser\Node\Statement\TraitDeclaration;
use PhpactorDist\Microsoft\PhpParser\Node\Statement\TryStatement;
use PhpactorDist\Microsoft\PhpParser\Node\Statement\UnsetStatement;
use PhpactorDist\Microsoft\PhpParser\Node\Statement\WhileStatement;
...
class Parser
{
    ...
    public function __construct()
    {
        ...
        $this->parameterTypeDeclarationTokens = [TokenKind::ArrayKeyword, TokenKind::CallableKeyword, TokenKind::BoolReservedWord, TokenKind::FloatReservedWord, TokenKind::IntReservedWord, TokenKind::StringReservedWord, TokenKind::ObjectReservedWord, TokenKind::NullReservedWord, TokenKind::FalseReservedWord, TokenKind::TrueReservedWord, TokenKind::IterableReservedWord, TokenKind::MixedReservedWord, TokenKind::VoidReservedWord, TokenKind::NeverReservedWord];
        // TODO update spec
    ...
    private function parseAttributeStatement($parentNode)
    {
        ...
        if ($parentNode instanceof ClassMembersNode) {
            ...
            if (\in_array($this->token->kind, [TokenKind::ClassKeyword, TokenKind::TraitKeyword, TokenKind::InterfaceKeyword, TokenKind::AbstractKeyword, TokenKind::FinalKeyword, TokenKind::FunctionKeyword, TokenKind::FnKeyword, TokenKind::EnumKeyword], \true) || $this->token->kind === TokenKind::StaticKeyword && $this->lookahead([TokenKind::FunctionKeyword, TokenKind::FnKeyword])) {
        ...
        if ($statement instanceof FunctionLike || $statement instanceof ClassDeclaration || $statement instanceof TraitDeclaration || $statement instanceof EnumDeclaration || $statement instanceof EnumCaseDeclaration || $statement instanceof InterfaceDeclaration || $statement instanceof ClassConstDeclaration || $statement instanceof PropertyDeclaration || $statement instanceof MissingDeclaration || $statement instanceof MissingMemberDeclaration) {
    ...
    const TYPE_DELIMITER_TOKENS = [TokenKind::BarToken, TokenKind::AmpersandToken];
    /**
    ...
    {
        return $this->parseUnionTypeDeclarationList($parentNode, function ($token) : bool {
    ...
    private function parseUnionTypeDeclarationList($parentNode, Closure $isTypeStart, Closure $parseType, int $expectedTypeKind)
    {
        ...
        do {
            if ($token->kind === TokenKind::OpenParenToken || $isTypeStart($token)) {
                ...
                $openParen = \in_array(TokenKind::BarToken, $delimiter, \true) ? $this->eatOptional(TokenKind::OpenParenToken) : null;
                if ($openParen) {
        ...
        if (\in_array(\end($result->children)->kind ?? null, $delimiter, \true)) {
    ...
    private function parseParenthesizedIntersectionType($parentNode, Token $openParen, Closure $isTypeStart, Closure $parseType) : ParenthesizedIntersectionType
    {
        ...
        $node->children = $this->parseDelimitedList(DelimitedList\QualifiedNameList::class, TokenKind::AmpersandToken, $isTypeStart, $parseType, $node, \true);
        if ($node->children) {
    ...
    {
        return $this->parseUnionTypeDeclarationList($parentNode, function ($token) {
    ...
    private function checkAnyToken($expectedKind) : bool
    {
        ...
        return \is_array($expectedKind) ? \in_array($kind, $expectedKind, \true) : $kind === $expectedKind;
    }
    ...
    private function parseCatchClause($parentNode)
    {
        ...
        $catchClause->qualifiedNameList = $this->parseQualifiedNameCatchList($catchClause) ?? new MissingToken(TokenKind::QualifiedName, $this->token->fullStart);
    ...
    private function parseEchoStatement($parentNode)
    {
        ...
        $echoStatement->expressions = $this->parseExpressionList($echoStatement);
    ...
    {
        $list = $this->parseDelimitedList(DelimitedList\ArgumentExpressionList::class, TokenKind::CommaToken, $this->isArgumentExpressionStartFn(), $this->parseArgumentExpressionFn(), $parentNode);
    ...
    private function parseTraitSelectOrAliasClauseFn()
    {
        return function ($parentNode) {
            ...
            if ($traitSelectAndAliasClause->asOrInsteadOfKeyword->kind === TokenKind::InsteadOfKeyword) {
                ...
                $traitSelectAndAliasClause->targetNameList = $this->parseQualifiedNameOrScopedPropertyAccessExpression($traitSelectAndAliasClause);
            }
