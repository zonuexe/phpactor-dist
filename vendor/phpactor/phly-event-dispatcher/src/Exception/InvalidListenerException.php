...
class InvalidListenerException extends RuntimeException implements ExceptionInterface
{
    ...
    {
        return new self(\sprintf('Lazy listener of type "%s" is invalid; must be a PHP callable', \gettype($service)));
    }
    /**
    ...
    {
        return new self(\sprintf('Lazy listener of type "%s" is invalid; must be a callable, or have a method associated with it', \get_class($service)));
    }
    /**
     * @param mixed $service Should be an object.
     */
    ...
    {
        return new self(\sprintf('Lazy listener of type "%s" with associated method "%s" is invalid; not callable', \get_class($service), $method));
    }
}
