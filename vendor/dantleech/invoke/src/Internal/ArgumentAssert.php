...
class ArgumentAssert
{
    public static function types(ResolvedArguments $resolved, Parameters $parameters) : void
    {
        foreach ($resolved->resolved() as $key => $value) {
            ...
            throw new InvalidParameterType(\sprintf('Argument "%s" has type "%s" but was passed "%s" for "%s"', $parameter->getName(), $reflectionType->getName(), \is_object($value) ? \get_class($value) : \gettype($value), $parameters->describeOwner()));
        }
    }
    public static function noUnknownKeys(ResolvedArguments $resolved, Parameters $parameters) : void
    {
        ...
        throw new UnknownKeys(\sprintf('Extra keys "%s" for "%s", known keys: "%s"', \implode('", "', \array_keys($resolved->unresolved())), $parameters->describeOwner(), \implode('", "', $parameters->keys())));
    }
    public static function requiredKeys(ResolvedArguments $resolved, Parameters $parameters) : void
    {
        ...
        throw new RequiredKeysMissing(\sprintf('Required keys "%s" for "%s", are missing', \implode('", "', $diff), $parameters->describeOwner()));
    }
}
