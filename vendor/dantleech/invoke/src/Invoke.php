...
class Invoke
{
    ...
    private function doInstantiate(string $className, array $args) : object
    {
        ...
        if (!$class->hasMethod(self::METHOD_CONSTRUCT)) {
            ...
            throw new ClassHasNoConstructor(\sprintf('Class "%s" has no constructor, but was instantiated with keys "%s"', $class->getName(), \implode('", "', \array_keys($args))));
        }
     ...
     * @return mixed
     */
    private function instantiate(ReflectionClass $class, string $methodName, array $arguments, Closure $factory)
    {
        try {
        ...
        try {
            ...
            throw new InvokeException(\sprintf('Unhandled type error when invoking "%s": %s', $class->getName(), $error->getMessage()), 0, $error);
        }
    }
